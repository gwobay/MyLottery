...
public class Check23Filter4Drop extends JFrame implements Runnable 
{
/**
...
static void getSincData(){
	...
	public void myTask(){
		...
		for (int idx=testStartIndex; idx<=testLastIndex; idx++){
			...
			{
				testData.add(Arrays.copyOfRange(testBaseData.get(s), idx-90, idx));
				if (idx < iLen1)
				targets[s]=testBaseData.get(s)[idx];
				
				retS += dI.format(targets[s])+", ";
			}
			retS += ">>";
			...
			//System.out.println("Working on "+lineType);
				for (int i=0; i<dd.length; i++){
					...
					int[] predictions=Check23Filter4Drop.tryNewSample(testData);
...
public static void main(String[] args)
{
	...
	Vector<int[]> sampleData=jReaderOrder.readData(5, 39, 390);	
		...
		Check23Filter4Drop aCharter=new Check23Filter4Drop();
		int iCheckStart=iLen1-12;
		int iCheckEnd=iLen1;
		int iFitCount=0;
		int iSuccess=0;
		//Vector<String> allTestResult=new Vector<String>();
			...
			for (int ia=0; ia<11; ia++){
				Check23Filter4Drop aJob=new Check23Filter4Drop();
				aJob.testBaseData=dateAscend;
				aJob.testStartIndex=iCheckStart+ia*jobs2Assign;
				if (aJob.testStartIndex>iCheckEnd) break;
			...
			*/
			...
			while (notDone){				
				...
				notDone=false;
				for (int ith=0; ith<totalActive; ith++){
	...
	try {
		testLog=new BufferedWriter(new OutputStreamWriter(new FileOutputStream("check23Fit"+lineType+"Log.txt")));	

			int iNo=0;
			int[] hitList=new int[40];
			...
			while (itr.hasNext()){
				...
				String[] dData=dLine.split("-");
				testLog.write(dData[0]+">>++++++++++++++++++"); testLog.newLine();
				for (int i=1; i<dData.length; i++){
					hitList[Integer.parseInt(dData[i])]++;
				}
				String wasEst="(";
				String gotHit="hits (";
				int iTry=0;
				...
				for (int i=1; i<40; i++){
					...
					iTry++;
					String est=dI.format(i)+",";
					wasEst +=  (""+i+"["+hitList[i]+"], ");
					if (dData[0].indexOf(est)>0){
						gotHit += (""+i+"["+hitList[i]+"], ");
						iFound++;
					}
					
				}
				testLog.write(wasEst);testLog.newLine();
				testLog.write(gotHit+") "+iFound+" out of "+iTry);testLog.newLine();
				/*
	...
	*/	
	...
	aJob.fileBase="C:\\Users\\eric\\workspace\\GetLottery\\539\\measured\\";
		...
		Vector<double[]> atf=Check23Filter4Drop.measuredAutoCorrelation(sampleData);
