...
public class Check23Filter4OrderNt extends JFrame implements Runnable 
{
/**
	...
	double pi=Math.PI;
	 ...
	 for (int s=0; s<5; s++){
		 ...
		 { 
			 ...
			 iTry++;
			 //System.out.println("Line "+s+" use ratio "+nRatio);
		 for (int n4=1+s; n4<36+s && !bFound; n4++){
			 newData[38]=n4;
			 for (int n3=1+s; n3<36+s && !bFound; n3++){
				 newData[37]=n3;
				 for (int n2=1+s; n2<36+s && !bFound; n2++){
					 newData[36]=n2;
					 
		 for (int n1=1+s; n1<36+s && !bFound; n1++){
			 newData[35]=n1;
		...
		for (int idx=testStartIndex; idx<=testLastIndex; idx++){
			...
			//System.out.println("Working on "+lineType);
				for (int i=0; i<dd.length; i++){
					...
					int[] predictions=Check23Filter4OrderNt.tryNewSample(testData);
...
public static void main(String[] args)
{
	...
	Vector<int[]> sampleData=jReaderOrder.readData(5, 39, 390);	
		...
		Check23Filter4OrderNt aCharter=new Check23Filter4OrderNt();
		int iCheckStart=iLen1-12;
		int iCheckEnd=iLen1;
		
		BufferedWriter testLog=null;
		try {
			testLog=new BufferedWriter(new OutputStreamWriter(new FileOutputStream("check23"+lineType+JackpotReader.drawDate+".txt")));	
			...
			while (iCheckStart <= iCheckEnd){
				...
				//System.out.println("Working on "+lineType);
					for (int i=0; i<dd.length; i++){
						...
						predics += (""+predictions[p]+"["+dF.format(merit_of_solution[p])+"], ");
						}
						int iHas=0;
						...
						//iFits[iHas]++;
						testLog.write(" use "+sqz+" filter " +checkCount+" check "+predics+")"+iHas);
						testLog.newLine();
