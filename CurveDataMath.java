...
public class CurveDataMath extends JFrame implements Runnable 
{
/**
...
public void init() 
{
    ...
    for (int i=0; i<totalPoints; i++){
    	xPos -= dX;
    	g2.draw(new Line2D.Double(xPos, 0, xPos, 20*dY));	
    }
	g2.setColor(Color.black);
	g2.setStroke(stroke);
	for (int i=1; i<maxNumber+1; i += 2)
	{
        ...
        for (int i=0; i<dataSamples.size(); i++){
        	int tCount=dataSamples.get(i).length;
        	polylineD[i]=new GeneralPath(GeneralPath.WIND_EVEN_ODD, tCount);       	
...
public void drawDistribution(String fileName, int[] freqCount) 
{
        //ShapesDemo2D Demo2D = new ShapesDemo2D();
        //f.getContentPane().add("Center", Demo2D);
        //Demo2D.init();
...
//System.out.println("I am here");
	...
	//if (myQuotes.size() <1) return;
	Vector<int[]> thisSample=new Vector<int[]>();
	thisSample.add(freqCount);
		getPngFile(bi, width, height, fileName, thisSample);
		return;
...
static void checkFor(String howOpen, int level){
	...
	CurveDataMath aCharter=new CurveDataMath();
...
//---------------------------------------------------------------			
		...
		CurveDataMath toDraw=new CurveDataMath();
		toDraw.drawDistribution("C:\\Users\\eric\\workspace\\GetLottery\\539\\"+howOpen+level, dFreq);
