...
public class DataAutocorrelation extends JFrame implements Runnable 
{
/**
...
static Vector<double[]> measuredAutoCorrelation(Vector<int[]> inData, int inChkCount){
	Vector<double[]> outResult=new Vector<double[]>();
	//double[] pdf=getRangePDF(0, inData.get(0).length, inData);
	int iSet=inData.size();
	...
	//}
	noisePower=new double[iSet];
	int correlationMinLength=39;
	int chkCount=39;//inChkCount;
	for (int s=0; s<iSet; s++){
		//int k=39*7;//390;//
		//inData.get(0).length;
		//float fMax=-9999999, fMin=999999;
		//int maxAt=0, minAt=0;
		//int iCorrLen=inData.get(0).length - chkCount;
		//if (iCorrLen < 0){
			//iCorrLen=correlationMinLength;
			//chkCount=inData.get(0).length - iCorrLen;
		//} else
		//if (iCorrLen > JackpotReader.testCaseCount) iCorrLen=JackpotReader.testCaseCount;
		//else if (iCorrLen > JackpotReader.testCaseCount/2) iCorrLen=JackpotReader.testCaseCount/2;
		//else if (iCorrLen > JackpotReader.testCaseCount/4) iCorrLen=JackpotReader.testCaseCount/4;
		//
		double[] iValue=new double[inData.get(0).length - chkCount];
		Arrays.fill(iValue, 0);
		
		for (int j=1; j<iValue.length; j++)
		{			
			float value=0;
			for (int n=j; n<j+chkCount;//inData.get(0).length;//j+iCorr;
					n++){
...
int[] verifyPerformance(Vector<int[]> allSamples, String fileName, Vector<int[]> allRefD1){
	...
	try {
		aWriter=new BufferedWriter(new OutputStreamWriter(new FileOutputStream(fileName+"1.txt")));						
		String sPred="Predic:(";
	...
	//Vector<Vector<double[] > > dailyResults=new Vector<Vector<double[] > >();
		...
		int[][][] localPerformanceList=new int[1][6][JackpotReader.testCaseCount];
		int[][][] predictionTS=new int[1][JackpotReader.testCaseCount+1][];
		//Arrays.fill(predictionTS, 0);
		for (int x=0; x<6;x++)
			Arrays.fill(localPerformanceList[0][x], 0);
		int totalSample=allSamples.get(0).length;
	for (int iTest=JackpotReader.testCaseCount; iTest>=0; iTest--){
		Vector<int[]> checkSample=new Vector<int[]>();
		for (int s=0; s<5; s++){
			checkSample.add(Arrays.copyOfRange(allSamples.get(s), iTest, totalSample));			
		}
		...
		Vector<double[]> allList=measuredAutoCorrelation(checkSample, JackpotReader.testCaseCount);
			...
			for (int s=0; s<5; s++){
				if (iTest > 0)
				hits[s]=allSamples.get(s)[iTest-1];			
				double[] iList=allList.get(s);
				int[] refD=checkSample.get(s);//Arrays.copyOfRange(allRefD1.get(s), testDay, allSamples.get(0).length);
				int iMx1=0, iAt1=0;
				int iMx2=0, iAt2=0;
				int iMx3=0, iAt3=0;
			...
			int iHitC=0;
			//predictionTS[JackpotReader.testCaseCount-iTest]=new 
			int pCount=0;
			sPred="Predic:(";
			...
			predictionTS[0][JackpotReader.testCaseCount-iTest]=pList;
			String sDraw="++++++++++++++Draw:(";
			String sHits="Hits:(";
			for (int s=0; s<5; s++){
				sDraw += ""+hits[s]+", ";
			...
			hitCounts[0][iHitC]++;
			if (iTest > 0)
				localPerformanceList[0][iHitC][JackpotReader.testCaseCount-iTest]=1;
			if (iTest >0)
			lastHitLocation[0][iHitC]=iTest;
...
public static void main(String[] args)
{
	...
	Vector<int[]> sampleData=jReaderOrder.readData(5, 39, 512);//72);//390);	
		...
		//DataAutocorrelation.fileBase="C:\\Users\\eric\\workspace\\GetLottery\\539\\measured\\";
		DataAutocorrelation.fileBase="C:\\Users\\eric\\projects\\GetLottery\\539\\measured\\";
			File myDir=new File(DataAutocorrelation.fileBase);
			...
			System.out.println("Creating file "+DataAutocorrelation.fileBase+"autocorrl"+JackpotReader.drawDate+lineType);
			aJob.verifyPerformance(sampleData, "autocorrl"+JackpotReader.drawDate+lineType,
					globalSample);
