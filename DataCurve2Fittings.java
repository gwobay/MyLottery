...
public class DataCurve2Fittings extends JFrame implements Runnable 
{
/**
...
static double[] getRangePDF(int from, int to, Vector<int[]> refD){
	...
	if (iCC==iC){System.out.println("pdf verified");}
	return linePdf;
}
...
static Vector<double[]> measuredRangeAutoCorrelation(Vector<int[]> inData, int range){
	Vector<double[]> outResult=new Vector<double[]>();
	int dataLength=inData.get(0).length;
	double[] iValue=new double[dataLength];
...
static double[] measuredAvg=null;



static Vector<double[]> measuredAutoCorrelation(Vector<int[]> inData, int chkCount){
	Vector<double[]> outResult=new Vector<double[]>();
	//double[] pdf=getRangePDF(0, inData.get(0).length, inData);
	int iSet=inData.size();
	...
	for (int s=0; s<iSet; s++){
		...
		double[] iValue=new double[inData.get(0).length];
		Arrays.fill(iValue, 0);
		int iCorr=inData.get(0).length - chkCount;
		if (iCorr > JackpotReader.testCaseCount) iCorr=JackpotReader.testCaseCount;
		else if (iCorr > 5*39) iCorr=5*39;
		else if (iCorr > 4*39) iCorr=4*39;
		for (int j=1; j<chkCount; j++)
		{			
			float value=0;
			for (int n=j; n<inData.get(0).length;//j+iCorr;
					n++){
				float f1=inData.get(s)[n];
				if (n-j<0) continue;
				float f2=inData.get(s)[n-j];
				value += ((f1-measuredAvg[s])*(f2-measuredAvg[s]));//*measuredPDF[inData.get(s)[n]]);
			}
			
			iValue[j]=value;//Math.round(value);
...
static Vector<double[]> measuredAutoCorrelation(Vector<int[]> inData){
	...
	//double[] pdf=getRangePDF(0, inData.get(0).length, inData);
	int iSet=inData.size();
	if (measuredAvg.length < inData.size()){
	...
	noisePower=new double[iSet];
	for (int s=0; s<iSet; s++){
		int k=390;//inData.get(0).length;
		float fMax=-9999999, fMin=999999;
		int maxAt=0, minAt=0;
		Arrays.fill(outResult.get(s), 0);
		for (int j=1; j<inData.get(s).length; j++)
		{
			...
			for (int n=j; n<k; n++){
				...
				outResult.get(s)[j]=value;//Math.round(value);
...
static int[] projByMinMatchingPrimeError(Vector<int[]> inData, int chkCount){
	int[] retV=new int[5];
	//double[] pdf=getRangePDF(0, inData.get(0).length, inData);
	int iSet=inData.size();
	...
	for (int s=0; s<iSet; s++){
		int[] refD=inData.get(s);
		//int k=39*7;//390;//
		
		//float fMax=-9999999, fMin=999999;
		//int maxAt=0, minAt=0;
		double[] fValue=new double[iLen];
		Arrays.fill(fValue, 10000000);
		double[] dataCurve=new double[chkCount];
		Arrays.fill(dataCurve, 0);
		
		int minWhen=0;
		
			for (int x=0; x<chkCount; x++)
			{
				dataCurve[x]=inData.get(s)[iLen-chkCount+x]-inData.get(s)[iLen-chkCount+x-1];
			}
			
			double minValue=1000000;
			for (int j=iLen-2; j>=chkCount; j--)
			{	
				int[] testCurve=Arrays.copyOfRange(inData.get(s), 
										j-chkCount+1, j+1);
				testCurve[0] -= inData.get(s)[j-chkCount];
				for (int x=1; x<chkCount; x++)
				{					
					testCurve[x] -= testCurve[x - 1];
				}
				
				double errV=0;			
				for (int x=0; x<chkCount; x++){
					//checkCurve[x]=inData.get(s)[j-chkCount+1+x]-measuredAvg[s];			
					errV += Math.abs(testCurve[x] -dataCurve[x]);
				}
				fValue[j]=errV;	
				if (errV < minValue) {
					minValue=errV;
			...
			//for (int j=chkCount; j<iLen-1; j++)
			{
				//fValue[j]=100*(1+minValue)/(fValue[j]+1);
			}
			//tempBuffer.add(fValue);
			//if (minCaseValue > minValue) {
				//minCaseValue=minValue;
				//minWhen=rd;
			//}
			int sign=1;
			int pp=(refD[refD.length-1]+sign*(refD[minWhen+1]-refD[minWhen]));
			while (pp < 0) pp += 39;
			pp %= 39;
			if (pp==0) pp=39;
		retV[s]=pp;//minWhen;
...
static int[] projByMinMatchingError(Vector<int[]> inData, int chkCount){
	int[] retV=new int[5];
	//double[] pdf=getRangePDF(0, inData.get(0).length, inData);
	int iSet=inData.size();
	//if (measuredAvg==null || measuredAvg.length < inData.size()){
	...
	for (int s=0; s<iSet; s++){
		...
		double localAvg=0;
			for (int x=0; x<chkCount; x++)
			{
				dataCurve[x]=inData.get(s)[iLen-chkCount+x];//-measuredAvg[s];
				localAvg += dataCurve[x];
			}
			localAvg /= chkCount;
			for (int x=0; x<chkCount; x++)
			{
				dataCurve[x] -= localAvg;
			}
			double minValue=1000000;
			for (int j=iLen-2; j>=chkCount; j--)
			{	
				...
				for (int x=0; x<chkCount; x++){
					//checkCurve[x]=inData.get(s)[j-chkCount+1+x]-measuredAvg[s];			
					errV += Math.abs(testCurve[x] - avgJ-dataCurve[x]);
				}
				fValue[j]=errV;	
				if (errV < minValue) {
					minValue=errV;
...
static Vector<double[]> getFittingError(Vector<int[]> inData, int chkCount){
	Vector<double[]> outResult=new Vector<double[]>();
	//double[] pdf=getRangePDF(0, inData.get(0).length, inData);
	int iSet=inData.size();
	//if (measuredAvg==null || measuredAvg.length < inData.size()){
	...
	for (int s=0; s<iSet; s++){
		...
		outResult.add(tempBuffer.get(minWhen));
	}
	return outResult;
}


int[] projectByCurveFitting(Vector<int[]> allSamples, String fileName, Vector<int[]> allRefD1){
	//double[] pdf=getRangePDF(0, allSamples.get(0).length, allSamples);;

	int iLen=allSamples.get(0).length;
	int[][] checkList={{0, 12}, {1, 12}};
	int[][] hitCounts=new int[checkList.length][6];
	int[][] lastHitLocation=new int[checkList.length][6];
	JackpotReader.initPerformanceList(JackpotReader.testCaseCount, 0);
	int[][][] localPerformanceList=new int[checkList.length][6][JackpotReader.testCaseCount];
	...
	String[] showPrediction=new String[checkList.length];
	String myName=getClass().getName();
	BufferedWriter aWriter;
	try {
		aWriter=new BufferedWriter(new OutputStreamWriter(new FileOutputStream(fileName+"2All.txt")));						
		
	//Vector<Vector<double[] > > dailyResults=new Vector<Vector<double[] > >();
		int[][][] predictionTS=new int[checkList.length][JackpotReader.testCaseCount+1][];
	for (int iTest=JackpotReader.testCaseCount; iTest>=0; iTest--){
		int[] hits=new int[5];
		Arrays.fill(hits, 0);
		Vector<int[]> checkSample=new Vector<int[]>();
		for (int s=0; s<5; s++){
			if (iTest > 0)
				hits[s]=allSamples.get(s)[iLen-iTest];
			checkSample.add(Arrays.copyOfRange(allSamples.get(s), 0, iLen-iTest));			
		}
		
		for (int iCheck=0; iCheck<checkList.length; iCheck++){
			int checkCount=checkList[iCheck][1];

		//dailyResults.add(
		int[] allList=null;
		if (iCheck==1) allList=projByMinMatchingPrimeError(checkSample, checkCount);
		else allList=projByMinMatchingError(checkSample, checkCount);
				//getFittingError(checkSample, checkCount);
				//measuredAutoCorrelation(checkSample, JackpotReader.testCaseCount)
				//);

	//for (
	//int checkStart=2; 
	//checkStart<7; checkStart++)
	

			//int[] retV=new int[allRefD.get(0).length/2];
			int[] dPredict=new int[40];
			Arrays.fill(dPredict, 0);
			for (int s=0; s<allList.length; s++){	
				dPredict[allList[s]]=1;
				/*
				//double[] iList=allList.get(s);
				int[] refD=(checkSample.get(s));//, 120, iLen-iTest);
				int iAt1=allList[s];
				//int iMx2=0, iAt2=0;
				//int iMx3=0, iAt3=0;
				//for (int i=; i<allList.length; i++){
			...
			//double r=(1.0*refD[0]-1.0*refD[1])/(1.0*refD[mX[i]]-1.0*refD[mX[i]+1]);
				int pp=(refD[refD.length-1]+sign*(refD[iAt1+1]-refD[iAt1]));
				while (pp < 0) pp += 39;
				pp %= 39;
				if (pp==0) pp=39;
					//if (pp>0 && pp < 40) 
				dPredict[pp]=1;//retV[iC++]=pp;
				*/					
			}
			int iHitC=0;
			int pCount=0;
			showPrediction[iCheck]="Predic:(";
			for (int k=0; k<dPredict.length; k++) 
				{
				if (dPredict[k]==0) continue;
					showPrediction[iCheck] += ""+k+", ";
					dPredict[pCount++]=k;
				}
			int[] pList=Arrays.copyOf(dPredict, pCount);
			predictionTS[iCheck][JackpotReader.testCaseCount-iTest]=pList;
			String sDraw="++++++++++++++Draw:(";
			String sHits="Hits:(";
			if (iTest > 0)
			for (int s=0; s<5; s++){
			...
			if (iTest>0)
				localPerformanceList[iCheck][iHitC][JackpotReader.testCaseCount-iTest]=1;
...
int[] verifyPerformance(Vector<int[]> allSamples, String fileName, Vector<int[]> allRefD1){
	//double[] pdf=getRangePDF(0, allSamples.get(0).length, allSamples);;
	...
	for (int iTest=120; iTest>=0; iTest--){
		Vector<int[]> checkSample=new Vector<int[]>();
		for (int s=0; s<5; s++){
			checkSample.add(Arrays.copyOfRange(allSamples.get(s), iTest, allSamples.get(0).length));			
		}
		//measuredPDF=getRangePDF(0, checkSample.get(0).length, checkSample);;
		/*
		measuredAvg=new double[checkSample.get(0).length];
		Arrays.fill(measuredAvg, 0);
		for (int s=0; s<5; s++){
			int k=checkSample.get(s).length;
			for (int j=0; j<k; j++)
			{
				measuredAvg[s] += (checkSample.get(s)[j]);//*measuredPDF[allSamples.get(s)[j]]);
			}
			measuredAvg[s] /= k;
			//avg[i] /= k;		
		}*/
		dailyResults.add(measuredAutoCorrelation(checkSample, 
				                          JackpotReader.testCaseCount));
	}
	//for (
	int checkStart=2; 
	//checkStart<7; checkStart++)
	...
	int[] lastHitLocation=new int[6];
	Arrays.fill(hitCounts, 0);
	BufferedWriter aWriter;
	
	try {
		aWriter=new BufferedWriter(new OutputStreamWriter(new FileOutputStream(fileBase+fileName+checkStart+".txt")));						
		int testDay=120;
		for (int ix=0; ix < dailyResults.size(); ix++){
			Vector<double[]> allList=dailyResults.get(ix);
			int[] hits=new int[6];	
			Arrays.fill(hits, 0);
			//int[] retV=new int[allRefD.get(0).length/2];
				int[] dPredict=new int[5];
			Arrays.fill(dPredict, 0);
			for (int s=0; s<5; s++){
				if (120-ix > 0)
				hits[s]=allRefD1.get(s)[120-ix-1];			
				double[] iList=allList.get(s);
				int[] refD=Arrays.copyOfRange(allRefD1.get(s), testDay, allSamples.get(0).length);
				int iMx1=0, iAt1=0;
				int iMx2=0, iAt2=0;
				int iMx3=0, iAt3=0;
				for (int i=checkStart; i<iList.length; i++){
					if (iList[i]==0) break;
					if (iList[i] < 20) continue;					
					if (iList[i] > iMx1){
						iMx1=(int)Math.round(iList[i]);
			...
			//double r=(1.0*refD[0]-1.0*refD[1])/(1.0*refD[mX[i]]-1.0*refD[mX[i]+1]);
					int pp=(refD[0]+sign*(refD[iAt1-1]-refD[iAt1]));
					while (pp < 0) pp += 39;
					pp %= 39;
					if (pp==0) pp=39;
					//if (pp>0 && pp < 40) 
						dPredict[s]=pp;//retV[iC++]=pp;
					
			}
			int iHitC=0;
			String sPred="Predic:(";
			for (int k=0; k<dPredict.length; k++) sPred += ""+dPredict[k]+", ";
			String sDraw="++++++++++++++Draw:(";
			String sHits="Hits:(";
			for (int s=0; s<5; s++){
				sDraw += ""+hits[s]+", ";
			...
			hitCounts[iHitC]++;
			lastHitLocation[iHitC]=testDay;
			aWriter.write("=================== "+testDay--+" ======");aWriter.newLine();
...
/*
		*/
		...
		for (int s=0; s<6; s++){
			aWriter.write("Hits "+s+" : ["+hitCounts[s]+"]@"+lastHitLocation[s]);aWriter.newLine();
		}
		aWriter.close();
	} catch (IOException e) {
		// TODO Auto-generated catch block
...
int[] putList2File(Vector<double[]> allList, String fileName, Vector<int[]> allRefD){
	BufferedWriter aWriter;
	int[] retV=new int[allRefD.get(0).length/2];
	int iC=0;
	try {
	...
	aWriter.newLine();
			...
			//double r=(1.0*refD[0]-1.0*refD[1])/(1.0*refD[mX[i]]-1.0*refD[mX[i]+1]);
			int pp=(refD[0]+sign*(refD[iAt1-1]-refD[iAt1]));
			if (pp>0 && pp < 40) retV[iC++]=pp;
			aWriter.write(", @"+iAt1+" shift:"+(pp)+//(dF.format(refD[0]*(1.0+r))+
		...
		//double r=(1.0*refD[0]-1.0*refD[1])/(1.0*refD[mX[i]]-1.0*refD[mX[i]+1]);
		pp=refD[0]+sign*(refD[iAt1-1]-refD[iAt1]);
		if (pp>0 && pp < 40) retV[iC++]=pp;
		aWriter.write(", @"+iAt1+" shift:"+(pp)+//(dF.format(refD[0]*(1.0+r))+
				", v-ratio:"+dF.format(getProject(refD, iAt1))+", next:"+refD[iAt1-1]);
			aWriter.write("]");
...
static void evaluateAutoCorr(Vector<int[]> refData){
	...
	for (int i=iLen-78*3; i>-1; i--)
	{
		...
		for (int s=0; s<5; s++){
			...
			text += (""+hits[s]+", ");
			testData.add(Arrays.copyOfRange(refData.get(s), i+1, refData.get(0).length));
		}
		text += ">>";
		Vector<double[]> rho=measuredAutoCorrelation(testData, 3*78);
...
static double getMeanRatio(int[] inData){
	...
	int iLen=inData.length;
	 ...
	 for (int i=0; i<iLen; i++){
		 double conv=0;
		 for (int k=1; k<=iNeed; k++){
			 //if (i+k<1) continue;
			 if (i+k > iLen) break;
			 //conv += whichSinc[Math.abs(k)]*inData[i+(int)((k-1)/sqz)]*2/myPI;
			 conv += whichSinc[k]*inData[i+(k-1)]*2/myPI;
		 }
		 for (int k=1; k<=iNeed; k++){
			 if (i- k<0) break;			
			 //conv += whichSinc[Math.abs(k)]*inData[i-(int)(k/sqz)]*2/myPI;
			 conv += whichSinc[k]*inData[i-k]*2/myPI;
		 }
...
static double getConvolValue(int tau, int[] inData){
	double[] whichSinc=sinc60PI;
	int iNeed=12;
	if (sqz < 1) {
		whichSinc=sincHalfPI;
	 ...
	 for (int s=0; s<5; s++){
		 ...
		 for (int i=0; i<iLen; i++){
			 ...
			 for (int k=1; k<6+1; k++){
				 ...
				 int t=i+(int)((k-1)/sqz);
				 if (t > iBnd) break;
				 conv += sinc60PI[Math.abs(k)]*sampled.get(s)[i+(int)((k-1)/sqz)]*2/pi;
			 }
			 for (int k=1; k<6+1; k++){
				 if (i- k<0) break;
				 int t=i-(int)(k/sqz);
				 if (t<0) break;
				 //if (i+k > iLen) break;
				 conv += sinc60PI[Math.abs(k)]*sampled.get(s)[i-(int)(k/sqz)]*2/pi;
			 }
			 conv /= 2.1174;
...
static double[] inverseFit(int[] sampled, String fname){
	int iLen=sampled.length;
	double pi=Math.PI;
	 double [] retV=new double[iLen];
	 int[] bestFitPoints=new int[5];
	 int[] newData=new int[40];
	 int adjust_base_length=36;
	...
	// ratio += getMeanRatio(Arrays.copyOfRange(sampled.get(0), iLen-checkFrom, iLen));
	 ...
	 for (int pTest=iLen - 300; pTest<iLen; pTest++){
	 ...
	 for (int s=0; s<5; s++){
		 ...
		 ratio=getMeanRatio(Arrays.copyOfRange(sampled, iLen-checkFrom, iLen));
		 Arrays.fill(newData, 0);
		 for (int i=0; i<35; i++){
			 newData[i]=sampled[iLen-35+i];
		 }
		 double diff2=2000;
		 ...
		 for (int ix=0; ix<checkCount; ix++){
			 ...
			 for (int k=1; k<6+1; k++){
				 ...
				 //if (i+k >39) break;
				 conv += sinc60PI[Math.abs(k)]*newData[i+(int)((k-1)/sqz)]*2/pi;
			 }
			 for (int k=1; k<6+1; k++){
				 if (i- k < 0) break;
				 int t=i-(int)(k/sqz);
				 if (t<0) break;
				 //if (i+k > iLen) break;
				 conv += sinc60PI[Math.abs(k)]*newData[i-(int)(k/sqz)]*2/pi;
			 }
	 ...
	 return retV;//bestFitPoints;//retV;
}

static double[] merit_of_solution=new double[5];
//static double[] sinc40PI=new double[60];

static int[] tryNewSample1( Vector<int[]> sampled){
	int iLen=sampled.get(0).length;
	double pi=Math.PI;
	 Vector<int[]> retV=new  Vector<int[]>();
	 int[] bestFitPoints=new int[5];
	 ...
	 for (int s=0; s<5; s++){
		 ...
		 for (int n1=1+s; n1<36+s; n1++){
			 ...
			 double[] diffS=new double[checkCount+1];
		 for (int ix=0; ix<checkCount; ix++){
			 int i=ix+36-checkCount;
			 double conv=0;
			 for (int k=1; k<6+1; k++){
		 ...
		 good1 += " has a="+dF.format(diff3[0])+" b="+dF.format(diff3[1])+" c="+dF.format(diff3[2]);
		 bestFitPoints[s]=diffAt[0];
		 merit_of_solution[s]=diff2/checkCount;
	 }
		
	 return bestFitPoints;//retV;
}

static int[] tryNewSample( Vector<int[]> sampled){
	int iLen=sampled.get(0).length;
	double pi=Math.PI;
	 Vector<int[]> retV=new  Vector<int[]>();
	 int[] bestFitPoints=new int[5];
	 ...
	 for (int s=0; s<5; s++){
		 ...
		 { 
			 ...
			 iTry++;
			 //System.out.println("Line "+s+" use ratio "+nRatio);
		 for (int n4=1+s; n4<36+s; n4 += 60){//++){
			 newData[38]=0;//n4;
			 for (int n3=1+s; n3<36+s; n3+= 60){//++){
				 newData[37]=0;//n3;
				 for (int n2=1+s; n2<36+s; n2+= 60){//++){
					 newData[36]=0;//n2;
		 ...
		 for (int ix=0; ix<checkCount; ix++){
			 int i=ix+36-checkCount;
			 double conv=getConvolValue(i, newData);
			 /*
			 for (int k=1; k<6+1; k++){
				 //if (i+k<1) continue;
				 int t=i+(int)((k-1)/sqz);
		...
		retV.add(Arrays.copyOf(newData, iLen));
		 ...
		 good1 += " with diff="+dF.format(diff2);
		 good1 += " has a="+dF.format(diff3[0])+" b="+dF.format(diff3[1])+" c="+dF.format(diff3[2]);
		 System.out.println(good1+")"+bestRatio+"; n="+bestDt);
		 if (diff2<0.1)
		 bestFitPoints[s]=diffAt[0];
		 else
			 bestFitPoints[s]=0; 
...
static boolean getSincData(){
	
	if (sinc60PI[1]<0.0001){
		JackpotReader.getSincData();
		if (JackpotReader.sinc40PI[1] > 0.1){
			sinc60PI=Arrays.copyOf(JackpotReader.sinc40PI, 60);
			sincHalfPI=Arrays.copyOf(JackpotReader.sincHalfPI, 60);
			return true;
		}
	}
	
	System.out.println("Failed to get Sinc Data");
	return false;
}
...
static DecimalFormat dI=new DecimalFormat("000");
...
{
	String myName=System.class.getName();
	if (getSincData()==false)
		System.exit(1);
	int[] setTypes={0, 1};
	Vector<int[]> history=Lottery539PageParser.readHistory();
	if (history==null){
		System.out.println("No data to build PDF");
				 System.exit(1);
	}
	measuredPDF=new double[40];
	Arrays.fill(measuredPDF, 0);
	for (int i=0; i<history.size(); i++){
		for (int d=0; d<5; d++){
			measuredPDF[history.get(i)[d]]++;
		}		
	}
	for (int i=0; i<40; i++) measuredPDF[i] /= (5*history.size());
	
	DataCurve2Fittings.fileBase="C:\\Users\\eric\\projects\\GetLottery\\539\\measured\\";
	File myDir=new File(DataCurve2Fittings.fileBase);
	...
	for (int iType=0; iType<2; iType++){
		setType=iType;//setTypes[iq];
		
	String lineType=criteria[setType];
	JackpotReader jReaderOrder=JackpotReader.getInstance("test539.txt",lineType);//"開出順序");//"大小順序");//
	Vector<int[]> sampleData=jReaderOrder.readData(5, 39, 512);//72);//390);	
		globalSample=sampleData;
		DataCurve2Fittings aJob=new DataCurve2Fittings();
		Vector<int[]> samples=sampleData;
		...
		System.out.println(System.class.getName()+" Creating file "+DataCurve2Fittings.fileBase+"2fit"+JackpotReader.drawDate+lineType);
		
		aJob.projectByCurveFitting(dateAscend, "fit"+JackpotReader.drawDate+lineType,
					globalSample);
	}
			...
			//aJob.verifyPerformance(sampleData, "auto"+JackpotReader.drawDate+lineType,
					//globalSample);
			    //getAutoCorrelation(sampleData);
			//for (int i=0; i<5; i++){
				//aJob.putList2File(atf, "auto"+JackpotReader.drawDate+lineType,
					//globalSample);
			//}
		...
		try {
			testLog=new BufferedWriter(new OutputStreamWriter(new FileOutputStream("try23Filter"+JackpotReader.drawDate+"Log.txt")));	

		System.out.println("Using "+JackpotReader.drawDate+" with "+iLen1+" data");
		int testStartIndex=iLen1-12;//14;
		int testLastIndex=iLen1;
		for (int idx=testStartIndex; idx<=testLastIndex; idx++){
				Vector<int[]> testData=new Vector<int[]>();
				int[] targets=new int[5];
				Arrays.fill(targets,  0);
				String sTargets="******("+dI.format(idx)+")<<";
				for (int s=0; s<dateAscend.size(); s++)
				{
					testData.add(Arrays.copyOfRange(dateAscend.get(s), 0, idx));
					if (idx < iLen1)
					targets[s]=dateAscend.get(s)[idx];					
					sTargets += dI.format(targets[s])+", ";
				}
				sTargets += ">>";
				
				for (int iq=0; iq<1; iq++){
		
					int[] predCount=new int[40];
				...
				//System.out.println("Working on "+lineType);
						...
						for (int i=0; i<dd.length; i++){
							...
							int[] predictions=//refineFound(dateAscend, "filterN"+(sqz*10)+lineType);//filterOutSample(dateAscend);;//
									tryNewSample1(testData);//dateAscend, "for"+JackpotReader.drawDate+"filterN"+(sqz*10)+lineType);
					//filterOutSample(dateAscend);;//
					...
					//System.out.println("Finish low pass try with "+
							...
							for (int p=0; p<predictions.length; p++){
								...
								predCount[predictions[p]]++;
								texts += ""+predictions[p]+"["+merit_of_solution[p]+"], ";
							}
					...
					String sFound="Found (";
					for (int i=1; i<40; i++){
						if (predCount[i]<1) continue;
						iTry++;
						sTry += ""+i+"["+predCount[i]+"], ";
						for (int ig=0; ig<5; ig++){
							if (targets[ig]==0)continue;
							if (targets[ig]==i){
								iFound++;
								targets[ig]=0;
								sFound += ""+i+"["+predCount[i]+"], ";
								
								break;
							}
						}
					}
					testLog.write(sTargets);testLog.newLine();
					testLog.write(sTry+")");testLog.newLine();
					testLog.write(sFound+") "+iFound+" out of "+iTry);testLog.newLine();
				}
		}
		testLog.close();
		}catch (IOException e){}
		...
		int ttFitCount=0;
		for (int ib=iLen1-300; ib<iLen1; ib++){
	...
	*/	
		//measuredPDF=getRangePDF(0, globalSample.get(0).length, globalSample);
		//DataAutocorrelation aCharter=new DataAutocorrelation();
		...
		*/
	//DataAutocorrelation aJob=new DataAutocorrelation();
	//aJob.fileBase="C:\\Users\\eric\\workspace\\GetLottery\\539\\measured\\";
		//File myDir=new File(aJob.fileBase);
		//if (!myDir.exists()) myDir.mkdir();
		...
		//Vector<double[]> atf=DataAutocorrelation.measuredAutoCorrelation(sampleData);
		    //getAutoCorrelation(sampleData);
		//for (int i=0; i<5; i++){
			//aJob.putList2File(atf, "autolines",
				//globalSample);
		//}
