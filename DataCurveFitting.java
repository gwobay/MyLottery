...
public class DataCurveFitting extends JFrame implements Runnable 
{
/**
...
static Vector<double[]> measuredAutoCorrelation(Vector<int[]> inData, int chkCount){
	...
	for (int s=0; s<iSet; s++){
		...
		if (iCorr > testCaseCount) iCorr=testCaseCount;
...
int[] projectByCurveFitting(Vector<int[]> allSamples, String fileName, Vector<int[]> allRefD1){
	...
	int[][][] localPerformanceList=new int[checkList.length][6][testCaseCount];
	...
	try {
		aWriter=new BufferedWriter(new OutputStreamWriter(new FileOutputStream(fileName+"All.txt")));						
		
	//Vector<Vector<double[] > > dailyResults=new Vector<Vector<double[] > >();
		int[][][] predictionTS=new int[checkList.length][testCaseCount+1][];
	for (int iTest=testCaseCount; iTest>=0; iTest--){
		...
		for (int iCheck=0; iCheck<checkList.length; iCheck++){
			int checkCount=checkList[iCheck];

		//dailyResults.add(
		int[] allList=projByMinMatchingPrimeError(checkSample, checkCount);
				//projByMinMatchingError(checkSample, checkCount);
				//getFittingError(checkSample, checkCount);
				//measuredAutoCorrelation(checkSample, testCaseCount)
				//);
	...
	//checkStart<7; checkStart++)
			...
			for (int s=0; s<allList.length; s++){	
				...
				int[] refD=(checkSample.get(s));//, testCaseCount, iLen-iTest);
				int iAt1=allList[s];
				//int iMx2=0, iAt2=0;
				//int iMx3=0, iAt3=0;
			...
			predictionTS[iCheck][testCaseCount-iTest]=pList;
			String sDraw="++++++++++++++Draw:(";
			String sHits="Hits:(";
			if (iTest > 0)
			for (int s=0; s<5; s++){
...
int[] verifyPerformance(Vector<int[]> allSamples, String fileName, Vector<int[]> allRefD1){
	...
	for (int iTest=testCaseCount; iTest>=0; iTest--){
		...
		dailyResults.add(measuredAutoCorrelation(checkSample, testCaseCount));
	}
	//for (
	int checkStart=2; 
	//checkStart<7; checkStart++)
	...
	try {
		...
		for (int ix=0; ix < dailyResults.size(); ix++){
			...
			Arrays.fill(dPredict, 0);
			for (int s=0; s<5; s++){
				if (testCaseCount-ix > 0)
				hits[s]=allRefD1.get(s)[testCaseCount-ix-1];			
				double[] iList=allList.get(s);
...
public static void main(String[] args)
{
	...
	DataCurveFitting.fileBase="C:\\Users\\eric\\projects\\GetLottery\\539\\measured\\";
	File myDir=new File(DataCurveFitting.fileBase);
	...
	Vector<int[]> sampleData=jReaderOrder.readData(5, 39, 512);//72);//390);	
		...
		DataCurveFitting aJob=new DataCurveFitting();
		...
		System.out.println(System.class.getName()+" Creating file "+DataCurveFitting.fileBase+"fit"+JackpotReader.drawDate+lineType);
