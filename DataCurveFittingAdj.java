...
public class DataCurveFittingAdj extends JFrame implements Runnable 
{
/**
...
static Vector<double[]> getMatchingError(Vector<int[]> inData, int chkCount){
	Vector<double[]> outResult=new Vector<double[]>();
	//double[] pdf=getRangePDF(0, inData.get(0).length, inData);
	int iSet=inData.size();
	//if (measuredAvg==null || measuredAvg.length < inData.size()){
	...
	for (int s=0; s<iSet; s++){
		...
		double localAvg=0;
			...
			{
				checkCurve[x]=inData.get(s)[iLen-chkCount+x];//-measuredAvg[s];
				localAvg += checkCurve[x];
			}
			localAvg /= chkCount;
			...
			{				
				...
				for (int x=0; x<chkCount; x++){
					...
					errV += Math.abs(inData.get(s)[j-chkCount+1+x]-checkCurve[x]);
				}
				fValue[j]=errV;	
				//if (errV < minValue) {
					//minValue=errV;
		...
		outResult.add(fValue);//tempBuffer.get(minWhen));
	}
	return outResult;
}
...
int[] projectByCurveFitting(Vector<int[]> allSamples, String fileName, Vector<int[]> allRefD1){
	...
	JackpotReader.initPerformanceList(6*39, 0);
	int[][][] localPerformanceList=new int[checkList.length][6][6*39];
	...
	//Vector<Vector<double[] > > dailyResults=new Vector<Vector<double[] > >();
		int[][][] predictionTS=new int[checkList.length][234+1][];
	for (int iTest=6*39; iTest>=0; iTest--){
		...
		for (int s=0; s<5; s++){
			...
			checkSample.add(Arrays.copyOfRange(allSamples.get(s), iLen-iTest-256, iLen-iTest));			
		}
		...
		//dailyResults.add(
		Vector<double[]> allList=
				getMatchingError(checkSample, checkCount);
				//getFittingError(checkSample, checkCount);
				//measuredAutoCorrelation(checkSample, 6*39)
				//);
	...
	//checkStart<7; checkStart++)
			...
			predictionTS[iCheck][234-iTest]=pList;
			String sDraw="++++++++++++++Draw:(";
			String sHits="Hits:(";
			if (iTest > 0)
			for (int s=0; s<5; s++){
			...
			if (iTest>0)
				localPerformanceList[iCheck][iHitC][6*39-iTest]=1;
...
int[] verifyPerformance(Vector<int[]> allSamples, String fileName, Vector<int[]> allRefD1){
	...
	for (int iTest=120; iTest>=0; iTest--){
		...
		dailyResults.add(measuredAutoCorrelation(checkSample, 6*39));
	}
	//for (
	int checkStart=2; 
	//checkStart<7; checkStart++)
...
public static void main(String[] args)
{
	...
	DataCurveFittingAdj.fileBase="C:\\Users\\eric\\projects\\GetLottery\\539\\measured\\";
	File myDir=new File(DataCurveFittingAdj.fileBase);
	...
	Vector<int[]> sampleData=jReaderOrder.readData(5, 39, 512);//72);//390);	
		...
		DataCurveFittingAdj aJob=new DataCurveFittingAdj();
		...
		System.out.println(System.class.getName()+" Creating file "+DataCurveFittingAdj.fileBase+"fit"+JackpotReader.drawDate+lineType);
