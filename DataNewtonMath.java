...
public class DataNewtonMath extends JFrame implements Runnable 
{
/**
...
static double getFuncValue(double[] newData, double adjValue, int from, int to)
{
	...
	double retV=0;
	 for (int ix=from ; ix<to; ix++){
		 ...
		 conv /= adjValue;//ratio;//1.795;//2.1174;//
		 //newData[i]=(int)Math.round(conv);
		 retV += //conv-newData[i];//
		             //Math.sqrt
		             ((conv-newData[i])*(conv-newData[i]));
		 //double ratio=Math.abs(conv)/sampled.get(s)[i];
	 }
	 checkCount=orgCheckCount;
	 return Math.sqrt(retV);
}

static double pureBisect(double[] newData, double[] points, double startX, double endX, double startY, double endY, int checkLen){
	
	double retV=1, dx=1, dy=1, v, lastV, adj=points[0], nuX, nuY, lastY, lastX, lastPx=startX, lastNx=endX, lastPy=startY, lastNy=endY;
	double minAdj=adj/1000;
	double minDy=adj/1000;
	nuX=(startX+endX)/2;
	newData[checkLen]=nuX;
	nuY=(startY+endY)/2;
	v=getFuncValue(newData, nuY, checkLen-checkCount, checkLen);
	retV=Math.abs(v);
	
	while (retV > 0.001 && (Math.abs(dx) > 0.01 || Math.abs(dy) > minDy)){
		
		if (v > 0){
			lastPx=newData[checkLen];
			lastPy=nuY;
				
		} else
		{
			lastNx=newData[checkLen];
			lastNy=nuY;
		}
		lastX=newData[checkLen];
		lastY=nuY;
...
static double minTestV=1000;
static double minTestAtX=5;
static double minTestAtY=20;
static boolean noMinBySect=false;

static double getSolUseNewton(double[] newData, double[] nRatio, int s, int checkLen){
	double dx=10, dy=10, lastX=newData[checkLen]/2, lastY=nRatio[0]*1.2, fc=0, v=0;
	double lastPx=0, lastNx=0, lastPy=0, lastNy=0, lastV=0;
	double min=10000;
	double adj=nRatio[0];
	double adjMin=adj/200;
	double minDy=adj/1000;
	double adjUpBnd=adj*1.2;
	double adjLwBnd=adj*0.8;
	double retV=getFuncValue(newData, adj, checkLen-checkCount, checkLen);
	int iTry=0;
	double minAtX=0;
	double minAtY=0;
	double dxAtMin=0;
	double dyAtMin=0;
	  while (Math.abs(dx) > 0.01 || Math.abs(dy) > minDy){
		  	
		  if (iTry==0) v=retV;
		  else 
		  v=getFuncValue(newData, adj, checkLen-checkCount, checkLen);
		  
		  fc=Math.abs(v);
		  iTry++;
		  //retV=fc;
		  if (fc < 0.001*checkCount) 
		  {
			  System.out.println("found poss sol :"+
			
				  newData[checkLen]+" and "+adj+
				  " with fc="+dF.format(fc)+" and dx="+dF.format(dx));
		  //if (fc < 0.1){
		  ...
		  if (v*lastV < 0) {
			  double[] points={adj};
			  v=pureBisect(newData, points, lastPx, lastNx, lastPy, lastNy, checkLen);
			  nRatio[0]=points[0];
			  return Math.abs(v);
		  }
		  ...
		  if (fc < minTestV){
			  ...
			  dx= newData[checkLen];
			  newData[checkLen] += lastX;
			  newData[checkLen] /= 2;
			  dxAtMin /= 2;
			  dx=dxAtMin;
			  newData[checkLen]=(minAtX+dxAtMin);
			  //lastX=dx;
			  dy=adj;
			  adj += lastY;
			  adj /= 2;
			  //lastY=dy;
			  //dx=newData[checkLen]-lastX;
			  dy=adj-lastY;
		  ...
		  newData[checkLen] += 0.000001;
		  double fxP=(getFuncValue(newData, adj, checkLen-checkCount, checkLen)-v)*1000000;
		  //double fc=getFuncValue(newData);
		   dx=-v*fxP;
		  while (newData[checkLen]+dx > 35+s && dx > 0) dx /= 2;
		  while (newData[checkLen]+dx < 1+s && dx < 0) dx /= 2;
		  double newX=(newData[checkLen]-0.000001)+dx;
		  if (Math.abs(dx)< 0.005){
			  newX=(newData[checkLen]+lastX)/2;
		  }
		  lastX=newData[checkLen];
		  newData[checkLen]=newX;
		  
		  adj *= 1.000001;
		  double fyP=(getFuncValue(newData, adj, checkLen-checkCount, checkLen)-v)*1000000*adj;				  
		  dy=-v*fyP;
		  while (adj+dy > adjUpBnd && dy>0) dy /= 2;
		  while (adj+dy < adjLwBnd && dy<0) dy /= 2;
		  double newY=adj/(1.000001)+dy;
		  //if (Math.abs(dy) < adjMin){
			  //if (dy<0) newY=(adj+adjLwBnd)/2;
			 // else
				  //newY=(adj+adjUpBnd)/2; 
		  //}
		  lastY=adj;
		  ...
		  //dy=0;
		  dy=newY-lastY;
		  if (Math.abs(fc-minTestV)<0.000001){
			  dxAtMin=dx;
			  dyAtMin=dy;
		  }
	  }	
...
static double getSolByNewton(double[] newData, double nRatio, int s, int checkLen){
	double dx=10, dy=10, lastX=newData[checkLen]/2, lastY=newData[checkLen+1]/2, fc=0, v=0;
	double min=10000;
	int iTry=0;
	  while (Math.abs(dx)+Math.abs(dy) > 0.1){
		  						
		  v=getFuncValue(newData, nRatio, checkLen-checkCount, checkLen);
		  fc=Math.abs(v);
		  iTry++;
		  //retV=fc;
		  if (fc < 0.003*checkCount) 
		  {
			  System.out.println("found poss sol :"+
			
				  newData[checkLen]+" and "+newData[checkLen+1]+
				  " with fc="+dF.format(fc)+" and dx="+dF.format(dx));
		  //if (fc < 0.1){
		  ...
		  if (fc < min){
			  ...
			  newData[checkLen] += lastX;
			  newData[checkLen] /= 2;
			  newData[checkLen+1] += lastY;
			  newData[checkLen+1] /= 2;
			  dx=newData[checkLen]-lastX;
			  dy=newData[checkLen+1]-lastY;
			  continue;
		  }
		  newData[checkLen] += 0.000001;
		  double fxP=(getFuncValue(newData, nRatio, checkLen-checkCount, checkLen)-v)*1000000;
		  //double fc=getFuncValue(newData);
		   dx=-v*fxP;
		  while (newData[checkLen]+dx > 35+s) dx /= 2;
		  while (newData[checkLen]+dx < 1+s) dx /= 2;
		  double newX=newData[checkLen]+dx;
		  if (Math.abs(dx)< 0.005){
			  newX=(newData[checkLen]+lastX)/2;
		  }
		  lastX=newData[checkLen];
		  newData[checkLen]=newX;
		  newData[checkLen+1] += 0.000001;
		  double fyP=(getFuncValue(newData, nRatio, checkLen-checkCount, checkLen)-v)*1000000;				  
		  dy=-v*fyP;
		  while (newData[checkLen+1]+dy > 35+s) dy /= 2;
		  while (newData[checkLen+1]+dy < 1+s) dy /= 2;
		  double newY=newData[checkLen+1]+dy;
		  if (Math.abs(dy)< 0.005){
			  newY=(newData[checkLen+1]+lastY)/2;
		  }
		  lastY=newData[checkLen+1];
		  newData[checkLen+1]=newY;
		  dx=newX-lastX;
		  dy=newY-lastY;
	  }	
	  return fc;
...
static double getSolByBisect(double[] newData, double nRatio, int s, int checkLen){
	
	  double dx=10, dy=10, lastX=newData[checkLen]/2, lastY=newData[checkLen+1]/2, fc=0, v;
	  double min=20000;
	  int iTry=0;
	  double adj=nRatio;
	  double delta=adj/1000000;
	  boolean useBisect=false;
	  
	  while (Math.abs(dx)+Math.abs(dy) > 0.1){
		  						
		  fc=getFuncValue(newData, adj, checkLen-checkCount, checkLen);
		  v=Math.abs(fc);
		  iTry++;
		  
		  if (v < 0.003*checkCount) 
		  {
			  System.out.println("found poss sol :"+
		  ...
		  if (useBisect) {
			  double tmp=(newData[checkLen] + lastX)/2;
			  lastX = newData[checkLen] ;
			  newData[checkLen] = tmp;
			  tmp=(newData[checkLen+1] + lastY)/2;
			  lastY = newData[checkLen+1] ;
			  newData[checkLen+1] = tmp;
			  adj= tmp;
			  dx=newData[checkLen]-lastX;
			  dy=newData[checkLen+1]-lastY;
			  continue;
		  ...
		  newData[checkLen] += 0.000001;
		  double fxP=(getFuncValue(newData, adj, checkLen-checkCount, checkLen)-fc)*1000000;
		  //double fc=getFuncValue(newData);
		   dx=-fc*fxP;
		  while (newData[checkLen]+dx > 35+s) dx /= 2;
		  while (newData[checkLen]+dx < 1+s) dx /= 2;
		  ...
		  //adj += delta;
		  newData[checkLen+1] += 0.000001;
		  double fyP=(getFuncValue(newData, adj, checkLen-checkCount, checkLen)-fc)*1000000;				  
		  dy=-fc*fyP;
		  while (newData[checkLen+1]+dy > 35+s) dy /= 2;
		  while (newData[checkLen+1]+dy < 1+s) dy /= 2;
		  double newY=newData[checkLen+1]+dy;
...
static double getSolUseBisect(double[] newData, double[] nRatio, int s, int checkLen){
	
	  double dx=10, dy=10, lastX=newData[checkLen]/2, lastY=nRatio[0]/2, fc=0, v=0;
	  newData[checkLen+1]=0;newData[checkLen+2]=0;newData[checkLen+3]=0;
	  double min=20000;
	  int iTry=0;
	  double adj=nRatio[0];
	  double adjUPB=adj*1.2;
	  double adjLWB=adj*0.81;
	  double delta=adj/1000000;
	  boolean useBisect=false;
	  while (Math.abs(dx)+Math.abs(dy) > 0.1){
		  ...
		  if (useBisect) {
			  ...
			  lastX = newData[checkLen] ;
			  newData[checkLen] = tmp;
			  tmp=(adj + lastY)/2;//(newData[checkLen+1] + lastY)/2;
			  lastY = adj;//newData[checkLen+1] ;
			  //newData[checkLen+1] = tmp;
			  adj= tmp;
			  dx=newData[checkLen]-lastX;
			  dy=adj -lastY;//newData[checkLen+1]-lastY;
			  continue;
		  ...
		  adj += delta;
		  double fyP=(getFuncValue(newData, adj, checkLen-checkCount, checkLen)-v)*1000000;				  
		  dy=-v*fyP;
		  while (adj+dy > adjUPB) dy /= 2;
		  while (adj+dy < adjLWB) dy /= 2;
		  double newY=adj + dy;//newData[checkLen+1]+dy;
		  if (Math.abs(dy)< 0.0015){
			  newY=(newData[checkLen+1]+lastY)/2;
		  }
		  lastY=adj;//newData[checkLen+1];
		  adj = newY;//newData[checkLen+1]=newX;
		  dx=newX-lastX;
		  dy=newY-lastY;
	  }	
	  nRatio[0]=adj;
...
static double[] solutionInit=new double[5];
static int[] tryNewSample( Vector<int[]> sampled, String fname){
	...
	double pi=Math.PI;
	 ...
	 for (int s=0; s<5; s++){
		 aWriter.write("****************Checking line "+s);aWriter.newLine();
		 String good1="try line "+s+" fits at ";
		 ratio=getMeanRatio(Arrays.copyOfRange(sampled.get(s), iLen-elementsToGetAdjRef, iLen));
		 Arrays.fill(newData, 0);
		 double m=0;
		 for (int i=0; i<checkLen; i++){
			 newData[i]=sampled.get(s)[iLen-checkLen+i];
			 
			 if (i >= checkLen - elementsToGetAdjRef) m+=newData[i]; 
		 }
		 m /= elementsToGetAdjRef;
		 double diff2=2000;
		 double[] diff3=new double[3];
		 diff3[2]=0;
		 diff3[1]=10;
		...
		checkCount=6;
		 ...
		 { 
			 ...
			 //System.out.println("Line "+s+" use ratio "+nRatio);
		 //for (int n4=1+s; n4<36+s; n4++){
			 newData[38]=0;//(36+s)/2;//n4;
			 //for (int n3=1+s; n3<36+s; n3++){
				 newData[37]=0;//(36+s)/2;//n3;
				 ////for (int n2=1+s; n2<36+s; n2++){
				 int n2=(36+s)/2;
					 newData[36]=m;//n2;
				//int n1=(36+s)/2;	 
		 for (int n1=1+s; n1<36+s; n1++){
			 //newData[35]=n1;
			 if (n1 > 1) noMinBySect=true;
			  double[] diffS=new double[checkCount+1];
			  newData[checkLen]=n1;//(36+s)-solutionInit[s];
			  newData[checkLen+1]=m;//solutionInit[s];
			  boolean noAdjChange=false;//true;//false;//true;
			  double[] adjRatio={ratio};
			  tDiff=//
			  getSolUseNewton(newData, adjRatio, s, checkLen); 
			  //getSolUseBisect(newData, adjRatio, s, checkLen); 
			 // getSolByNewton(newData, nRatio, s, checkLen);
			//getSolByBisect(newData, nRatio, s, checkLen);;
			  
		 
		
			 if (noAdjChange || tDiff < diff2){
				 diff2=tDiff;
				 diffAt[0]=(int)Math.round(newData[checkLen]);
				 if (noAdjChange){
					 bestRatio=adjRatio[0];
					 break;
				 }
				 diffAt[1]=(int)Math.round(newData[checkLen+1]);
		 ...
		 //}
		  ...
		  good1 += "("+diffAt[0]+","+diffAt[1]+")";
		 good1 += " with diff="+dF.format(diff2);
		 good1 += " has a="+dF.format(diff3[0]);
		 aWriter.write(good1+")"+bestRatio+"; n="+bestDt);aWriter.newLine();
		 System.out.println(good1+")"+bestRatio);
		 bestFitPoints[s]=diffAt[0];
...
public static void main(String[] args)
{
	...
	Vector<int[]> sampleData=jReaderOrder.readData(5, 39, forHowManyDays);	
		...
		for (int s=0; s<sampleData.size(); s++){
			...
			solutionInit[s]=sampleData.get(s)[0];
		}
		System.out.println("Checking "+forHowManyDays+" data last date:"+JackpotReader.drawDate);
		...
		try {
			testLog=new BufferedWriter(new OutputStreamWriter(new FileOutputStream("checkFitLog.txt")));	

			int[] iStatistics=new int[6];
			Arrays.fill(iStatistics, 0);
			iCheckStart=iCheckEnd-2;
		...
		for (int cc=2; cc<4; cc++){
			...
			for (int i=0; i<dd.length; i++){
				...
				int[] predictions=//refineFound(dateAscend, "filterN"+(sqz*10)+lineType);//filterOutSample(dateAscend);;//
						tryNewSample(dateAscend, "for"+JackpotReader.drawDate+"filterN"+(sqz*10)+lineType);
				...
				System.out.println("Finish l"+iCheckStart+" with check last "+checkCount+" for "+
			                  (System.currentTimeMillis() - tNow)+" milli secs");
				String predics="get predictions:(";
				for (int p=0; p<predictions.length; p++){
					found[predictions[p]]++;
				predics += (""+predictions[p]+", ");
				}
		...
		String statText="try 300 cases and got ";
		for (int p=1; p<6; p++){
			statText += ""+p+":"+iStatistics[p]+", ";
		}
		testLog.write(statText+" and total fit cases:" +iSuccess);
		testLog.close();
		}
		}
		}
	...
	*/	
		...
		*/
	DataNewtonMath aJob=new DataNewtonMath();
	aJob.fileBase="C:\\Users\\eric\\workspace\\GetLottery\\539\\measured\\";
		...
		Vector<double[]> atf=DataNewtonMath.measuredAutoCorrelation(sampleData);
