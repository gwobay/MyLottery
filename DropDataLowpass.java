...
public class DropDataLowpass extends JFrame implements Runnable 
{
/**
...
static int[] tryNewSample( Vector<int[]> sampled, BufferedWriter aWriter){
	int iLen=sampled.get(0).length;
	double pi=Math.PI;
	 Vector<int[]> retV=new  Vector<int[]>();
	 int[] bestFitPoints=new int[5];
	 ...
	 for (int s=0; s<5; s++){
		...
		// int iTry=0;
		 ...
		 { 
			 ...
			 //iTry++;
			 //System.out.println("Line "+s+" use ratio "+nRatio);
		 for (int n4=1; n4<40; n4++){
			 newData[38]=n4;
			 for (int n3=1; n3<40; n3++){
				 newData[37]=n3;
				 for (int n2=1; n2<40; n2++){
					 newData[36]=n2;
					 
		 for (int n1=1; n1<40; n1++){
			 newData[35]=n1;
		 ...
		 good1 += "("+diffAt[0]+","+diffAt[1]+","+diffAt[2]+","+diffAt[3]+")";
		 good1 += " with diff="+dF.format(merit_of_solution[s]);
		 good1 += " has a="+dF.format(diff3[0])+" b="+dF.format(diff3[1])+" c="+dF.format(diff3[2]);
		 bestFitPoints[s]=0;
		 if (merit_of_solution[s] < 0.2){
		 aWriter.write(good1+")"+bestRatio+"; n="+bestDt);aWriter.newLine();
		 bestFitPoints[s]=diffAt[0];
		 }		 
	 }
...
public static void main(String[] args)
{
	...
	String fileName="for"+JackpotReader.drawDate+"filterN"+(sqz*10)+lineType+".txt";
	...
	while (iCheckStart <= iCheckEnd){
		try {
			aWriter.write("================= line "+iCheckStart+ "  =========");aWriter.newLine();
			}  catch (IOException e){				
		} 
		Vector<int[]> testData=new Vector<int[]>();
			...
			targets[s]=dateAscend.get(s)[iCheckStart];
				...
				for (int cc=2; cc<4; cc++){
					...
					for (int i=0; i<dd.length; i++){
						...
						try {
							aWriter.write("filter "+sqz+ " chkpts "+checkCount+"+++");
							}  catch (IOException e){				
						} 	
						int[] predictions=//refineFound(dateAscend, "filterN"+(sqz*10)+lineType);//filterOutSample(dateAscend);;//
								tryNewSample(testData, aWriter);
						//"for"+JackpotReader.drawDate+"filterN"+(sqz*10)+lineType);
				...
				for (int i=1; i<40; i++){
					...
					for (int ix=0; ix<5; ix++){
						if (targets[ix] != i) continue;
						goodFits += (""+ targets[ix]+"["+predCount[i]+"],");
						iFit++;
						break;
					}
				}
				...
				System.out.println(goodFits+")"+iFit);
				try {
					aWriter.write(sFound+")");aWriter.newLine();
					aWriter.write(draws+")");aWriter.newLine();
					aWriter.write(goodFits+")"+iFit);aWriter.newLine();
				}  catch (IOException e){
