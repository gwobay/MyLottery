...
public class ExpandedScanTry extends JFrame implements Runnable 
{
/**
	
	 */
	static String programName="ExpandedScanTry";
...
public static void main(String[] args)
{
	...
	Vector<int[]> sampleData=jReaderOrder.readData(5, 39, 512);	
		...
		ExpandedScanTry aJob=new ExpandedScanTry();
		...
		try {
			...
			new FileOutputStream("expand"+lineType+JackpotReader.drawDate+"6548ScanTry.txt")));	
		...
		for (int iTest=6*39; iTest>=0; iTest--){
			...
			for (int s=0; s<dateAscend.size(); s++)
			{
				testData.add(Arrays.copyOfRange(dateAscend.get(s), 
						testTarget-(512-6*39-1), testTarget));
						//0, iCheckStart));
				if (iTest >0)
	...
	for (int p=0; p<6; p++){
		testLog.write(""+p+":"+iStatistics[iFq][p]+" @"+lastHitLocation[iFq][p]+" ??"+dF.format(nextCasePdf[p]));testLog.newLine();
	}
		}
