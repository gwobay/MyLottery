...
public class FitDerivativesData extends JFrame implements Runnable 
{
/**
...
Vector<int[]> putList2File(int[] iList, String fileName, int[] refD){
	...
	try {
		...
		for (int i=0; i<iX; i++) aWriter.write(", "+refD[mX[i]]);
		aWriter.write("]");
		aWriter.newLine();
		aWriter.write(fileName+": m2x "+(iMx-1)+"; [");
		for (int i=0; i<i2; i++) aWriter.write(", "+refD[m2[i]]);
		aWriter.write("]");
		aWriter.newLine();
		aWriter.write(fileName+": m3x "+(iMx-2)+"; [");
		for (int i=0; i<i3; i++) aWriter.write(", "+refD[m3[i]]);
		aWriter.write("]");
		aWriter.newLine();
		if (iX >0)
...
//end of program
static void showMatching(Vector<int[]> sampleData, Vector<int[]> refD)
{
	int dataLength=sampleData.get(0).length;
	int[] distance=new int[dataLength];
	Arrays.fill(distance, 0);
	int mx0=100000, mx1=100000, mx2=100000;
	for (int i=1; i<dataLength; i++){
		for (int k=0; k<sampleData.size(); k++)
		distance[i] += Math.abs(sampleData.get(k)[i]-sampleData.get(k)[0]);
		if (distance[i] <= mx0) mx0=distance[i];
		else if (distance[i] <= mx1) mx1=distance[i];
		else if (distance[i] < mx2) mx2=distance[i];
	}
	int[] distance0=new int[dataLength/3];
	int i0=0, i1=0, i2=0;
	int[] distance1=new int[dataLength/3];
	int[] distance2=new int[dataLength/3];
	for (int i=1; i<distance.length; i++){
		if (distance[i] == mx0) distance0[i0++]=i;
		else if (distance[i] == mx1) distance1[i1++]=i;
		else if (distance[i] == mx2) distance2[i2++]=i;
	}
	
	
	for (int k=0; k<5; k++){
		System.out.print(""+refD.get(k)[0]+", ");
	} System.out.println(");");
	
	System.out.print("Best Matching ("+mx0+") at \n");
	for (int i=0; i<i0; i++){
		System.out.print("\t("+distance0[i]+"):(");
		for (int k=0; k<5; k++){
			System.out.print(""+refD.get(k)[distance0[i]]+", ");
		} System.out.println(");");
	} 
	System.out.print("2nd Best Matching ("+mx1+") at \n");
	for (int i=0; i<i1; i++){
		System.out.print("\t("+distance1[i]+"):(");
		for (int k=0; k<5; k++){
			System.out.print(""+refD.get(k)[distance1[i]]+", ");
		} System.out.println(");");
	} 
	System.out.print("3rd Best Matching ("+mx2+") at \n");
	for (int i=0; i<i2; i++){
		System.out.print("\t("+distance2[i]+"):(");
		for (int k=0; k<5; k++){
			System.out.print(""+refD.get(k)[distance2[i]]+", ");
		} System.out.println(");");
	} 	
}

static void showLineFit(int[] sampleData, int range, int[] refD)
{
	int dataLength=sampleData.length;
	int[] distance=new int[dataLength];
	Arrays.fill(distance, 0);
	int mx0=100000, mx1=100000, mx2=100000;
	for (int i=1; i<dataLength-range; i++){
		for (int k=0; k<range; k++)
		distance[i] += Math.abs(sampleData[i+k]-sampleData[k]);
		if (distance[i] <= mx0) mx0=distance[i];
		else if (distance[i] <= mx1) mx1=distance[i];
		else if (distance[i] < mx2) mx2=distance[i];
	}
	...
	int[] distance1=new int[dataLength/3];
	int[] distance2=new int[dataLength/3];
	for (int i=1; i<distance.length-range; i++){
		if (distance[i] == mx0) distance0[i0++]=i;
		else if (distance[i] == mx1) distance1[i1++]=i;
		else if (distance[i] == mx2) distance2[i2++]=i;
	}
	...
	//fWriter.write("Ref=(");
	//for (int k=0; k<range; k++){
		//fWriter.write(""+refD[k]+", ");
	//} //fWriter.write(");");fWriter.newLine();
	
	fWriter.write("Best Matching ("+mx0+") at \n");
	for (int i=0; i<i0; i++){
		fWriter.write("\t("+distance0[i]+"|"+refD[distance0[i]-1]+"):(");
		for (int k=0; k<range; k++){
			fWriter.write(""+refD[distance0[i]+k]+", ");
		} fWriter.write(");");fWriter.newLine();
	} 
	/*
	 * fWriter.write("2nd Best Matching ("+mx1+") at \n");
	
	for (int i=0; i<i1; i++){
		fWriter.write("\t("+distance1[i]+"):(");
		for (int k=0; k<range; k++){
			fWriter.write(""+refD[distance1[i]+k]+", ");
		} fWriter.write(");");fWriter.newLine();
	} 
	fWriter.write("3rd Best Matching ("+mx2+") at \n");
	for (int i=0; i<i2; i++){
		fWriter.write("\t("+distance2[i]+"):(");
		for (int k=0; k<range; k++){
			fWriter.write(""+refD[distance2[i]+k]+", ");
		} fWriter.write(");");fWriter.newLine();
	} 
	 */
	} catch (FileNotFoundException e){
...
public static void main(String[] args)
{
		...
		int dataLength=sampleData.get(0).length>312?312:sampleData.get(0).length;
		globalSample=new Vector<int[]>();
		for (int i=0; i<5; i++){
			globalSample.add(Arrays.copyOf(sampleData.get(i), dataLength)) ;
		}
		showMatching(sampleData, globalSample);
	
		try {
			fWriter=new BufferedWriter(new OutputStreamWriter(new FileOutputStream("fitLine.txt")));						
		
		
		Vector<int[]> workData=sampleData;
		int[] newData={10, 31, 35 ,37 , 38};
		for (int kl=0; kl<5; kl++){
			int[] lineData=Arrays.copyOf(sampleData.get(kl), dataLength);
			for (int n=1; n<7; n++){
				
				fWriter.write("working on line "+kl+": "+n+" derivative");
				fWriter.newLine();
				int[] delta=new int[dataLength];
				Arrays.fill(delta, 9990);
				for (int k=0; k<dataLength-n; k++){
					delta[k]=lineData[k] - lineData[k+1];
				}
			
					for (int j=5; j<19; j++){
						fWriter.write("working on line "+kl);
						//fWriter.newLine();
						fWriter.write(" show range "+j+": "+n+" derivative");fWriter.newLine();
						showLineFit(delta, j, globalSample.get(kl));
					}
					
					lineData=Arrays.copyOf(delta, dataLength-n);
			}
		}
		fWriter.close();
		} catch (FileNotFoundException e){
		...
		/*
		OneFitManyData aCharter=new OneFitManyData();
...
//---------------------------------------------------------------			
			...
			for (int i=0; i<12; i++){
			rngR=aCharter.rangeReplication(sampleData, i*6+5);
				for (int k=0; k<5; k++){
			aCharter.putList2File(Arrays.copyOf(rngR.get(k), 252), "rep"+(i*6+5)+"line"+(k+1),
					globalSample.get(k));
				}	
				aCharter.samples=rngR;
				aCharter.forFile="line"+new DecimalFormat("00").format(i*6+5);
				aCharter.run();	
			}
