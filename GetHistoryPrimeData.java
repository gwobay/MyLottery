...
public class GetHistoryPrimeData extends JFrame implements Runnable 
{
/**
...
static Vector<int[]> getMaxLocation(Vector<double[]> atf){
	Vector<int[]> outV=new Vector<int[]>();
	for (int k=0; k<5; k++){
		double[] aLine=atf.get(k);
		int[] aMax=new int[aLine.length/3];
		Arrays.fill(aMax, 0);
		int iC=0;
		double iMax=-9999999;
		double iMin=99999999;
		for (int i=1; i<aLine.length; i++){
			if (aLine[i]==0) continue;
			if (iMax > aLine[i]) continue;
			if (Math.abs(aLine[i]-iMax)<0.000001){ aMax[iC++]=i;}; 
			if (iMax < aLine[i]){
				iMax=aLine[i];
				aMax[0]=i;
				iC=1;
		...
		//for (int i=0; i<aLine.length; i++){
			//if (Math.abs(aLine[i]-iMax)<0.000001) aMax[iC++]=i;
		//}
		outV.add(Arrays.copyOf(aMax, iC));
	}
		return outV;
}
...
char prediction_type; //P or A
Vector<Vector<int[]> > allPrimeHistory;
Vector<Vector<int[]> > allAutoHistory;
public Vector<Vector<int[]> > getHistoryPredictionLists(){
	...
	return	createHistoryPredictionFromCorrelation();
	
	return null; 
}
public Vector<Vector<int[]> > createHistoryPredictionFromPrime()
{	
	if (allPrimeHistory != null && allPrimeHistory.size()>0)
		return allPrimeHistory;
	...
	int maxRange=16;
			...
			if (allPrimeHistory==null) allPrimeHistory=new Vector<Vector<int[]> >();
			else if (allPrimeHistory.size()>0)
				allPrimeHistory.clear();
			...
			for (int p1=300-maxRange; p1>0; p1--) {
				...
				allPrimeHistory.add(dailyHitSpots);
...
public Vector<Vector<int[]> > createHistoryPredictionFromCorrelation()
{	
	if (allAutoHistory != null && allAutoHistory.size()>0)
		return allAutoHistory;
	//JackPotReader jReaderDrop=JackPotReader.getInstance("test539.txt","開出順序");//
	if (globalSample==null){
	JackpotReader jReaderOrder=JackpotReader.getInstance("test539.txt","大小順序");//"開出順序");//
	Vector<int[]> sampleData=jReaderOrder.readData(5, 39, 390);//jReaderDrop.readData(5, 39, 330);	//
	...
	int maxRange=12;
			...
			int[] hitCounts=new int[6];
			Arrays.fill(hitCounts, 0);
			if (allAutoHistory==null) allAutoHistory=new Vector<Vector<int[]> >();
			else if (allAutoHistory.size()>0)
				allAutoHistory.clear();
			//for (int p1=42; p1>0; p1--) {
			for (int p1=300-maxRange; p1>0; p1--) {
				...
				for (int i=minRange; i<maxRange; i++){
					Vector<double[]> atf=DataAutocorrelation.measuredRangeAutoCorrelation(sampleDataP, i*6);
				  			    
				   mxList=getMaxLocation(atf);
				...
				allAutoHistory.add(dailyHitSpots);
			...
			for (int i=minRange; i<maxRange; i++){
				Vector<double[]> atf=DataAutocorrelation.measuredRangeAutoCorrelation(globalSample, i*6);			    
			   mxList=getMaxLocation(atf);
...
public static void main(String[] args)
{
	...
	Vector<int[]> sampleData=jReaderOrder.readData(5, 39, 390);//jReaderDrop.readData(5, 39, 330);	//
		...
		GetHistoryPrimeData aCharter=new GetHistoryPrimeData();
		...
		*/
		...
		measured.fileBase="C:\\Users\\eric\\workspace\\GetLottery\\539\\auto\\";
			File myDir=new File(measured.fileBase);
			if (!myDir.exists()) myDir.mkdir();//
		Vector<double[]> atf=measured.measuredAutoCorrelation(sampleData);
			...
			//for (int i=0; i<5; i++){
				int[] preList=measured.putList2File(atf, "auto_lineM",
					globalSample);
			//}
				Vector<int[]> testDataP=new Vector<int[]>();
				for (int k=0; k<5; k++){
					testDataP.add(Arrays.copyOf(sampleData.get(k), 72));
				}
		Vector<double[]> predics=measured.measuredAutoCorrelation(testDataP);
				
				//for (int i=0; i<5; i++){
		measured.putList2File(atf, "auto_line72",
						globalSample);
				//}
				/*
...
//---------------------------------------------------------------			
		...
		//for (int rg=4; rg<16; rg += 16){
				//minRange=rg;
				//maxRange=minRange+6;
				logName="autoD-"+JackpotReader.drawDate+minRange+"-"+maxRange;
				myLog=null;
			...
			Arrays.fill(hitCounts, 0);
			//for (int p1=42; p1>0; p1--) {
			for (int p1=6; p1>=0; p1--) {
				
				int[] evenList=new int[40];
				int[] oddList=new int[40];
				Arrays.fill(evenList, 0);
				
				Arrays.fill(oddList, 0);
				int testDay=p1-1;
	...
	//mxList=getMinLocation(rngR);
				  ...
				  //rngR=aCharter.getAutoCorrelationRange(sampleDataP, 3*i);
				    //logName="byAuto";
				    Vector<double[]> tmp=DataAutocorrelation.measuredRangeAutoCorrelation(sampleDataP, i*6);
	  			    
				    		//measuredAutoCorrelation(sampleDataP);
				    ...
				    */
				    ...
				    for (int n=0; n<projs.length; n++){
				    	if (projs[n]>0 && Math.round(projs[n]) <40){
					    	...
					    	vLocalProj[Math.round(projs[n])]++;
					    	if (i % 2==0)
					    		evenList[Math.round(projs[n])]++;
					    	else
					    		oddList[Math.round(projs[n])]++;
				    	}
				    }
				    projs=getPrimeProject(i, mxList, sampleDataP);
				    for (int n=0; n<projs.length; n++){
			    ...
			    String oddText="odd:(";
			    String evnHit = ") even hit (";
			    int iEven=0;
			    for (int ih=0; ih<jackPots.length; ih++){
			    	if (evenList[jackPots[ih]] > 0){
			    		evnHit += "!"+jackPots[ih]+"!";
			    		iEven++;
			    	}
			    }
			    String oddHit = ") even hit (";
			    int iOdd=0;
			    for (int ih=0; ih<jackPots.length; ih++){
			    	if (oddList[jackPots[ih]] > 0){
			    		oddHit += "!"+jackPots[ih]+"!";
			    		iOdd++;
			    	}
			    }
		    ...
		    putLogN(evenText+")*"+evnHit+"****"+iEven+"******");
		    putLogN(oddText+")*"+oddHit+"****"+iOdd+"******");
			    //putLogN("))))))))))))))))))");
			    putLogN("------------------------");
			//}
			}
	...
	//mxList=getMinLocation(rngR);
	Vector<double[]> tmp=DataAutocorrelation.measuredRangeAutoCorrelation(sampleData, i*6);
	    
	//measuredAutoCorrelation(sampleData);
    mxList=getMaxLocation1(tmp);
			    //rngR=aCharter.getAutoCorrelationRange(sampleData, 3*i);
