...
public class GetLinesAvgJumpFit {
	...
	static public GetLinesAvgJumpFit getInstance(String fileName, String whichLine){
		if (fileName==null) return null;
		GetLinesAvgJumpFit aReader=new GetLinesAvgJumpFit();
...
*/
	...
	static double guessNextOnByJumpFit(int checkCount, double[] timeLine)
	{
		...
		for (int i=0; i<n-1; i++){
			testTs[i]=localTs[2*(i+1)] - pLocal;
			skipTs[i]=localTs[2*(i+1)-1] - pLocal;
		}
		skipTs[n-1]=localTs[2*n-1] - pLocal;
		double[] whichOne=new double[2];
		for (int iTry=1; iTry >=0; iTry--){
		
			testTs[n-1]=iTry - pLocal;
			...
			for (int i=0; i< 27; i++){
				...
				for (int ik=0; ik<n; ik++){
					if (Math.abs(checkBack[ik] - testTs[ik]) > 0.01){
						System.out.println("try "+n+" found invalid inverse matrix for element "+ik+" mismatch:"+dF.format(checkBack[ik])+" != "+dF.format(testTs[ik]));
					}
				}
				//whichOne[iTry]=0;
				double app=0;			
				for (int x=1; x<n; x++){
					double r=0;
					for (int v=0; v<n; v++){
						r += pCoeff[v]*Math.sin(offSet+(2*x-1)*pI/(v+1));
					}
					app += Math.abs(r-skipTs[x]);
				}
				if (app/(n-1) < 0.01) return iTry;
				//if (app < errV) errV=app;	
				pV += app;
			}
			whichOne[iTry]=pV/27;//errV;
	...
	static double findNextOnDays(int checkCount, double[] timeLine)
	{
		...
		return predV;//mV;//predV;//predV/nCase.length;
	}
	...
	static void findDrawnTimeLine(){
		...
		Arrays.fill(allTimePDF, 0);
		try {
			aReader=new BufferedReader(new FileReader(history));
			String aLine;//=aReader.readLine();
			//String date1=aLine.substring(0, 10).replace('/', '-');
			//File pdfFile=new File("pdf"+date1);
			//if (pdfFile)
			...
			while ((aLine=aReader.readLine()) != null){
				...
				int iE=aLine.length()-1;
				while (aLine.charAt(iE) <'0' || aLine.charAt(iE) > '9') iE--;
				//int wkDay=Integer.parseInt(aLine.substring(ix+1, i9));
				String[] terms=aLine.substring(i9+2, iE+1).split("-");
				if (terms.length <5) continue;
				int[] datas=new int[terms.length];
				for (int i=0; i<5; i++) {
					datas[i]=Integer.parseInt(terms[i]);
					allTimePDF[datas[i]]++;
				}
				allLines.add(datas);
				iC++;
			...
			aReader=new BufferedReader(new FileReader(new File("permute539.txt")));
			while ((aLine=aReader.readLine()) != null){
				if (aLine.length()<5) continue;
				...
				int[] datas=new int[5];
				for (int i=0; i<5; i++) {
					datas[i]=Integer.parseInt(terms[i+1]);
				}
				all575757.addElement(datas);
			}
			aReader.close();
		} catch (FileNotFoundException e){
		System.out.println("No file");
		...
		for (int iCheck=0; iCheck<4; iCheck++){
			int checkCount=6+iCheck*3;
		
		BufferedWriter aWriter;
		try {
			aWriter=new BufferedWriter(new OutputStreamWriter(new FileOutputStream("timeLineJumpFit"+checkCount+".txt")));
			int[] hitCountistics=new int[6];
			Arrays.fill(hitCountistics, 0);
			int[] lastHitAt=new int[6];
			Arrays.fill(lastHitAt, 0);
			double allTimeMin=100000;
			
		for (int iTestCase=120; iTestCase >= 0;  iTestCase--){
			...
			double[] absPredics=new double[41];
			Arrays.fill(predBase, 0);
			...
			for (int ib=1; ib<40; ib++){			
				predBase[ib]=//Math.abs(
						...
						//);
				//absPredics[ib]=Math.abs(predBase[ib]);
			}
				...
				if (predBase[ib] >  0.75){//
					...
					for (int s=0; s<5; s++){
						if (ib==drawns[s]){
							...
							cHit++;
							if (minV > predBase[ib]) minV=predBase[ib];
							if (allTimeMin > predBase[ib]) allTimeMin=predBase[ib];
							break;
						}
					}
				}
			...
			hitCountistics[cHit]++;
			if (iTestCase != 0)
			lastHitAt[cHit]=iTestCase;
			pValue /= iFound;
			
			aWriter.write(iFound+sFound+") ; "); 
			aWriter.newLine();
		...
		for (int i=0; i<6; i++){
			aWriter.write("has "+i+"hits "+hitCountistics[i]+" times; last at "+lastHitAt[i]);
			aWriter.newLine();
		}
		aWriter.close();
		} catch (FileNotFoundException e){
...
public static int[] tryPolyFitNumber(int lineNo, int[] timeLine, BufferedWriter aWriter){
		/*
		int[] ballPdf=new int[40];//new int[40];
		...
		for (int i=0; i<timeLine.length; i++){
			ballPdf[timeLine[i]]++;
			if (ballPdf[timeLine[i]] > maxF) maxF=ballPdf[timeLine[i]];
			if (lastLoc[timeLine[i]]!=0 ) {
				if (minDist < i - lastLoc[timeLine[i]]) minDist = i - lastLoc[timeLine[i]];				
			}
			lastLoc[timeLine[i]]=i;
		}
...
*/
		...
		//Arrays.fill(bFound, false);
		double[] retV=new double[2];
		int[] nCase={6, 9, 12};//, 24, 36};
		int[] fitNumbers=new int[nCase.length];
		double[] effects={0.3, 0.25, 0.2, 0.15, 0.1};
		for (int kn=0; kn<nCase.length; kn++)
		{
			int n=nCase[kn];//checkCount;//
			//if (n==15) continue;
		double[] testTs=new double[n];
		double[] skipTs=new double[n];
		int start=len1-2*n;
		for (int i=0; i<n-1; i++){
			testTs[i]=timeLine[start+2*(i+1)] - pLocal;
			skipTs[i]=timeLine[start+2*(i+1)-1] - pLocal;
		}
		skipTs[n-1]=timeLine[start+2*n-1] - pLocal;
		double[] whichOne=new double[40];
		double min=10000000;
		int found=0;
		for (int iTry=lineNo; iTry < lineNo+35; iTry++){
			...
			whichOne[iTry]=pV/iCal;//errV;
			//System.out.println("Line "+lineNo+" try "+iTry+" get "+dF.format(whichOne[iTry]));
			if (whichOne[iTry] < min) {
				min=whichOne[iTry];
				found=iTry;
			}
		}
		fitNumbers[kn]=found;
		System.out.println("Line "+lineNo+" case "+n+ " get "+dF.format(min/n));
		}
...
public static int[] getBestFitNumber(int lineNo, int[] timeLine, BufferedWriter aWriter){
	/*
	int[] ballPdf=new int[40];//new int[40];
	int[] lastLoc=new int[40];
	Arrays.fill(ballPdf, 0);
...
public static double[][] buildPredictionErrorTS(Vector<int[]> timeLineSet){
	
	double pI=Math.PI;//*2;
	int len=timeLineSet.get(0).length;
	int checkLen=9;
	double[][] retV=new double[5][len];
	for (int s=0; s<timeLineSet.size(); s++){
		Arrays.fill(retV[s], 0);
		for (int iCheck=2*checkLen; iCheck<=len; iCheck++){
		//for (int iCheck=len1-testCaseCount; iCheck<len1; iCheck++){
			int[] timeLine=Arrays.copyOfRange(timeLineSet.get(s), iCheck-2*checkLen, iCheck);//+2*checkLen);
			double predV=0;
			int len1=timeLine.length;
			double pLocal=0;
			for (int i=0; i<len1; i++){
			...
			int n=checkLen;//nCase[kn];//checkCount;//
			//if (n==15) continue;
			double[] testTs=new double[n];
			double[] skipTs=new double[n];
			int start=0;//len1-2*n;
			for (int i=0; i<n-1; i++){
				testTs[i]=timeLine[start+2*(i+1)] - pLocal;
				skipTs[i]=timeLine[start+2*(i+1)-1] - pLocal;
			}
		...
		//fitNumbers[kn]=found;
			if (iCheck < len)
		retV[s][iCheck-1]=found - timeLineSet.get(s)[iCheck];
			else retV[s][iCheck-1]=found;
		//System.out.println("Line "+s+" case "+n+ " get "+dF.format(min/n));
		}
	}
...
static int testCaseCount=JackpotReader.testCaseCount;
	...
	static void getPredictionsByAdjustError(double[][] predictionErrors, Vector<int[]> dateAscend, Vector<double[]> fDateAscend, String forCase)
	{
		int methodCount=HalfExpandedErrorTs.fitMethods.length;
		int[][][] localPerformanceList=new int[methodCount][6][testCaseCount];
		int[] ttPredicts=new int[methodCount];
		int[] ttHits=new int[methodCount];
		Arrays.fill(ttPredicts, 0);
		Arrays.fill(ttHits, 0);
		int[][] hitCounts=new int[methodCount][6];
		int[][] lastHitLocation=new int[methodCount][6];
		for (int iMd=0; iMd<methodCount; iMd++){
		...
		String[] showPredictions=new String[methodCount];
		int[][][] predictionTS=new int[methodCount][testCaseCount+1][];
		//Arrays.fill(predictionTS, 0);
		int iLen1=dateAscend.get(0).length;
		BufferedWriter aWriter;
		try {
			aWriter=new BufferedWriter(new OutputStreamWriter(new FileOutputStream("lineJumpFitError"+JackpotReader.drawDate+forCase+".txt")));
	...
	for (int iTest=testCaseCount; iTest >=0; iTest--){
		...
		//for (int iMd=0; iMd<methodCount; iMd++){
			//int which1=HalfExpandedErrorTs.fitMethods[iMd];
			//aJob.expandedBy=which1;
		int[][] guessList=new int[methodCount][40];
		for (int k=0; k<methodCount; k++)
		Arrays.fill(guessList[k], 0);
		
		for (int s=0; s<testErrorList.length; s++){
			
			double[] predictions=HalfExpandedErrorTs.adjustBasedOnHistoryDeviation(testErrorList[s], testData.get(s));
			double originalPrediction=predictionErrors[s][iLen1-iTest-1];
			if (iTest > 0) originalPrediction += dateAscend.get(s)[iLen1-iTest];
			for (int k=0; k<predictions.length; k++){
				int adjP=(int)(originalPrediction - predictions[k]);
				//if (iTest > 0)
					//adjP=(int)(predictions[k]+testErrorList[s][iLen1-iTest]);
				if (adjP > 39) while (adjP > 39) adjP -= 39;
				else if (adjP < 1) while (adjP < 1) adjP += 39;
				guessList[k][adjP]=1;				
			}	
		}
		for (int iMd=0; iMd<methodCount; iMd++){
			...
			for (int i=1; i<40; i++){
				if (guessList[iMd][i]>0) {
					showPredictions[iMd] += " "+i+", ";
					ttPredicts[iMd] ++;
				}
			}
		
		String showHit="Freq= "+HalfExpandedErrorTs.fitMethods[iMd]+" ---found :(";

			iFound=0;
			for (int p=0; p<5; p++){
				//testTxt += ""+targets[p]+",";
				if (guessList[iMd][targets[p]]>0){
					iFound++;
					showHit += ""+targets[p]+", ";
				}
			}
			int pCount=0;
			for (int k=0; k<guessList[iMd].length; k++)
				if (guessList[iMd][k]==1) 
					{
						//sPred += ""+k+", ";
						guessList[iMd][pCount++]=k;
					}
			int[] pList=Arrays.copyOf(guessList[iMd], pCount);
	...
	String myName="GetLinesAvgJumpFit - error adjust";//getClass().getName();
	...
	public static void main(String[] args) {
		...
		JackpotReader aReader=JackpotReader.getInstance("test539.txt", drawStyle[setType]);
		...
		double[][] errorTS=buildPredictionErrorTS(dateAscend);
		
		//Vector<double[]> rSamples=new Vector<double[]>();
		System.out.println("Creating file "+"lineJumpFit"+JackpotReader.drawDate+drawStyle[setType]+".txt");
			
		String sPreds=" Predics:(";
		BufferedWriter aWriter;
		try {
			aWriter=new BufferedWriter(new OutputStreamWriter(new FileOutputStream("lineJumpFit"+JackpotReader.drawDate+drawStyle[setType]+".txt")));
			System.out.println("GetLinesAvgJumpFit Creating file "+"lineJumpFit"+JackpotReader.drawDate+drawStyle[setType]+".txt");
			//JackpotReader.initPerformanceList(testCaseCount, 0);
			//testJumpFit1Line(1, samples.get(0), aWriter);
			getPredictionsByAdjustError(errorTS, dateAscend, fDateAscend, drawStyle[setType] );
			...
			for (int iTest=testCaseCount; iTest>=0; iTest--){
				...
				for (int k=0; k<samples.size(); k++){
					...
					int copyFrom=iLen1 - iTest-273;
					int copyBefore=iLen1 - iTest;
					//testSample.add(Arrays.copyOfRange(samples.get(k), iTest, iTest+120));	
					int pp = (int)(errorTS[k][iLen1 - iTest - 1]);
					if (iTest > 0) pp += dateAscend.get(k)[iLen1 - iTest];
					if (pp > 39) while (pp > 39) pp -= 39;
					else if (pp <1) while (pp <1) pp += 39;
					int[] predicts=new int[1]; predicts[0]=pp;
							//getBestFitNumber(k+1, Arrays.copyOfRange(dateAscend.get(k), copyFrom, copyBefore), aWriter);
					
					String txt="draw is ";
					txt += ""+hits[k]+ " got predicts=(";
				...
				sPreds=" Predics:(";
				int pCount=0;
				for (int ix=0; ix<allPredic.length; ix++){
				...
				int[] pList=Arrays.copyOf(allPredic, pCount);
				predictionTS[0][testCaseCount-iTest]=pList;
				hitCounts[0][iHits]++;	
								
				aWriter.write("-----"+pCount+sPreds+"-----");aWriter.newLine();				
				aWriter.write("========================"+iHits+sHit+"=====");aWriter.newLine();
				if (iTest==0) continue;
				localPerformanceList[0][iHits][testCaseCount-iTest]=1;							
				//if (iAt > 0)
					//JackpotReader.performanceList[iHits][testCaseCount-iAt]=1;
					
				//if (iAt > 0)
				lastHitLocation[0][iHits]=iTest;
				//testJumpFit(iTest, testSample, hits, aWriter );
			}
			String myName="GetLinesAvgJumpFit";//getClass().getName();
			JackpotReader.showSummaryPage(predictionTS, hitCounts, lastHitLocation, localPerformanceList, myName, aWriter);
