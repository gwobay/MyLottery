...
public class HalfExpandedScan extends JFrame implements Runnable 
{
/**
...
double rangeByHalfSinc(int n, int range, double[] newData){
		 ...
		 //for (int n4=1+s; n4<36+s; n4 += 36){//++){
			...
			// for (int n3=1+s; n3<36+s; n3++){
			 ...
			 for (int ix=0; ix<checkCount; ix++){
				 ...
				 conv =rangeByHalfSinc(pos, sincRange, testData)/adj;
				 //newData[i]=(int)Math.round(conv);
				 diffS[ix]=Math.abs(conv-testData[pos]);
				 //double ratio=Math.abs(conv)/sampled.get(s)[i];
			 }
...
double getMeanError(int from, int to, double[] testList, double withAdjust){
	...
	for (int i=from; i<to; i++){
		 ...
		 double expect=	rangeByHalfSinc(pos, useSincRange, testData)*withAdjust;			 
...
double getDistance(double adjust, double[] testList){
	...
	for (int ix=0; ix<checkCount; ix++){
		 ...
		 tDiff += Math.abs(testList[i] - rangeByHalfSinc(pos, useSincRange, testData)/adjust);
	 }
	return tDiff;
}
...
double getDiff(double adjust, double[] testList){
	...
	for (int ix=0; ix<checkCount; ix++){
		 ...
		 tDiff += (testList[i] - rangeByHalfSinc(pos, useSincRange, testData)/adjust);
	 }
	return tDiff;
}
	...
	// int orgCheckLen=36;
	 ...
	 for (int s=0; s<5; s++){
		 ...
		 for (int v=(checkLen-checkCount); v<checkLen; v++){			 
			 ...
			 double tmp=rangeByHalfSinc(pos, sincRange, testData);
		 ...
		 for (int v=(checkLen-checkCount)+1; v<checkLen+1; v++){			 
			 int pos=v*expandFactor;
			 double tmp=rangeByHalfSinc(pos, sincRange, newData);
...
public static void main(String[] args)
{
	...
	Vector<int[]> sampleData=jReaderOrder.readData(5, 39, 512);	
		...
		int[] testFreqs={3,4,6,8};
		
		HalfExpandedScan aJob=new HalfExpandedScan();
		...
		BufferedWriter chLog=null;
		try {
			testLog=new BufferedWriter(new OutputStreamWriter(
					new FileOutputStream("halfN"+lineType+JackpotReader.drawDate+"Log.txt")));	
			...
			//			
			int[][][] localPerformanceList=new int[testFreqs.length][6][6*39];
			int[] ttPredicts=new int[testFreqs.length];
			int[] ttHits=new int[testFreqs.length];
			Arrays.fill(ttPredicts, 0);
			Arrays.fill(ttHits, 0);
			int[][] iStatistics=new int[testFreqs.length][6];
			int[][] lastFoundLocation=new int[testFreqs.length][6];
			for (int iqs=0; iqs<testFreqs.length; iqs++){
				Arrays.fill(iStatistics[iqs], 0);
				Arrays.fill(lastFoundLocation[iqs], 0);
				
				for (int x=0; x<6;x++)
					Arrays.fill(localPerformanceList[iqs][x], 0);
			}
			iCheckStart=iCheckEnd-39*6;//cTryCases+1;
			
			String[] showPredictions=new String[testFreqs.length];
			int[][] predictionTS=new int[234+1][];
		...
		for (int iTest=6*39; iTest >=0; iTest--){
			...
			{
				testData.add(Arrays.copyOfRange(dateAscend.get(s), targetLoc-256, targetLoc));//0, iCheckStart));
				if (iTest > 0)
			...
			testLog.write(showDraw);
			testLog.newLine();
			
			for (int iqs=0; iqs<testFreqs.length; iqs++){
				int freqs=testFreqs[iqs];
				String sFreq="";
				aJob.expandedBy=freqs;
				int[] foundList=new int[40];
				Arrays.fill(foundList, 0);
				for (int cc=0; cc<ccVal.length; cc++){
				//minCheckCount; cc<minCheckCount+1; cc++){
			
				checkCount=ccVal[cc];//cc+5;//(2*cc+4);//*minCheckCount;
				
				long tNow=System.currentTimeMillis();
				int[] predictions=//refineFound(dateAscend, "filterN"+(sqz*10)+lineType);//filterOutSample(dateAscend);;//
							aJob.tryHalfSincFilter(testData, freqs, checkCount);//dateAscend, "filterA"+checkCount);//filterOutSample(dateAscend);;//
				...
				//hasPredic += predictions.toString();
					...
					foundList[predictions[k]]=1;
					}
					
				}
				showPredictions[iqs]="has predic=(";
				for (int i=1; i<40; i++){
					if (foundList[i]>0) {
						showPredictions[iqs] += " "+i+", ";
						ttPredicts[iqs] ++;
					}
				}
				String showHit="Freq= "+freqs+" ---found :(";
				String foundTxt="good pred: (";
				int goodOne=0;
				iFound=0;
				for (int p=0; p<5; p++){
					//testTxt += ""+targets[p]+",";
					if (foundList[targets[p]]>0){
						iFound++;
						showHit += ""+targets[p]+", ";
					}
				}
				ttHits[iqs] += iFound;
				testLog.write(showHit+">>>");
				testLog.newLine();
				
				testLog.write(showPredictions[iqs]+")");
				testLog.newLine();
			...
			iStatistics[iqs][iFound]++;
			if (iTest > 0 )
				lastFoundLocation[iqs][iFound]=	iTest;
			if (iTest>0)
				localPerformanceList[iqs][iFound][6*39-iTest]=1;
		}
		}
		
		//testLog.write("try "+iCheckStart+" cases and got");
		//testLog.newLine();
		BufferedWriter predictBase=new BufferedWriter(new OutputStreamWriter(new FileOutputStream("toPredictBase.txt", true)));						
		testLog.write("========= Half Expanded========");testLog.newLine();
		predictBase.write("========= Half Expanded========");predictBase.newLine();
		
		for (int iqs=0; iqs<testFreqs.length; iqs++){
			testLog.write("Freq= "+testFreqs[iqs]+"===="+myName+"========");
			testLog.newLine();
			testLog.write( showPredictions[iqs]);
			testLog.newLine();
			predictBase.write("Freq= "+testFreqs[iqs]+"===="+myName+"========");
			predictBase.newLine();
			predictBase.write( showPredictions[iqs]);
			predictBase.newLine();
			JackpotReader.performanceList=localPerformanceList[iqs];
			...
			for (int s=0; s<6; s++){
				...
				int iLL=localPerformanceList[iqs][s].length-10;
				for (int ih=0; ih<10; ih++){
					showOnOffSeq += ""+localPerformanceList[iqs][s][iLL+ih];
				}
				predictBase.write(showOnOffSeq+"--- ");
				testLog.write(showOnOffSeq+"--- ");
				predictBase.write("Hits "+s+" : ["+iStatistics[iqs][s]+"]@"+lastFoundLocation[iqs][s]);
				predictBase.write("   	??"+dF.format(nextCasePdf[s]));
				testLog.write("Hits "+s+" : ["+iStatistics[iqs][s]+"]@"+lastFoundLocation[iqs][s]);
				testLog.write("   	??"+dF.format(nextCasePdf[s]));
				//int days=hitCounts[s]*lastHitLocation[s];
				if (caseMeanVar[s] > 0 && lastFoundLocation[iqs][s] > caseMeanVar[s]) {
					double ovdToVar=(lastFoundLocation[iqs][s] - caseMeanVar[s])/caseMeanVar[s+6];
			...
			predictBase.write("------ Performance predics/case="+ttPredicts[iqs]/234+" predicts per hits="+ttPredicts[iqs]/ttHits[iqs]);
			predictBase.newLine();
			testLog.write("------ Performance predics/case="+ttPredicts[iqs]/234+" predicts per hits="+ttPredicts[iqs]/ttHits[iqs]);
			testLog.newLine();
		}
		testLog.close();
		predictBase.close();
