...
public class HalfExpanedDrop extends JFrame implements Runnable 
{
/**
...
int[] tryHalfSincFilter(Vector<int[]> oldSample){
	//int byFactor=32;
	...
	// int orgCheckLen=36;
	 ...
	 for (int s=0; s<5; s++){
		 ...
		 //newData[checkLen+1]=m;
		 double[] testData=expandSeries(expandFactor,newData); 
		 //for (int i=checkLen*expandFactor+1; i<testData.length; i++){
			 //testData[i]=0;
		 //}
		 ...
		 for (int v=(checkLen-checkCount); v<checkLen; v++){			 
			 int pos=v*expandedBy;
			 double tmp=rangeByHalfSinc(pos, sincRange, testData);
			 adj += Math.abs(tmp/newData[v]);
			 iCC++;
		 }
		 adj /=iCC;
		 double errorRef=getMeanError(0, checkCount, Arrays.copyOfRange(newData, checkLen-checkCount, checkLen), adj);
		 checkCount=1;
		 newData[checkLen+1]=m;
		 double diff2=200000;
		 double pLBND=1;
		 ...
		 double v1=getSolution(newData, checkLen, 39/4, pUBND, pLBND, adj);
		 double sol1=newData[checkLen];
		 double v2=getSolution(newData, checkLen, 39*3/4, pUBND, pLBND, adj);
		 double sol2=newData[checkLen];
		...
		// for (int n4=1+s; n4<36+s; n4 += 36){//n4++){
		 ...
		 checkCount=orgCheckCount;//verifyLen;
...
public static void main(String[] args)
{
	...
	Vector<int[]> sampleData=jReaderOrder.readData(5, 39, 390);	
		...
		HalfExpanedDrop aJob=new HalfExpanedDrop();
		
		int[] ccVal={3,4,5};//5, 6 ,7, 8};//4, 5, 6};//2, 3, 4, 5, 6};//,7,8,9};
		int[][] mFreqs={{4,5,6,8}, {4,8}, {4,5,8}};//2, 3, 4, 5, 6};
		for (int iqs=0; iqs<mFreqs.length; iqs++){
			int[] freqs=mFreqs[iqs];
			String sFreq="";
			for (int iss=0; iss<freqs.length; iss++) sFreq += freqs[iss];
		BufferedWriter testLog=null;
		BufferedWriter chLog=null;
		try {
			testLog=new BufferedWriter(new OutputStreamWriter(
					new FileOutputStream("half"+lineType+JackpotReader.drawDate+"Fit"+sFreq+"-"+ccVal[0]+"Log.txt")));	
			chLog=new BufferedWriter(new OutputStreamWriter(
					new FileOutputStream("half"+lineType+JackpotReader.drawDate+"Fit"+sFreq+"-"+ccVal[0]+"TaiLog.txt"), "utf-8"));
			...
			int[] lastFoundLocation=new int[6];
			Arrays.fill(lastFoundLocation, 0);
			Arrays.fill(iStatistics, 0);
			int cTryCases=39*6;// 79 is Q 24;
			iCheckStart=iCheckEnd-39*6;//cTryCases+1;
			
			double avgCandidates=0;
		while (iCheckStart <= iCheckEnd){
			testLog.write(" ******** "+iCheckStart+" ********");
			testLog.newLine();
			chLog.write(" ******** "+iCheckStart+" ********");
			chLog.newLine();
			...
			int minCheckCount=5;
			int history_count = 6;
			String[] hasPredic=new String[history_count];//{"fit 6pts:", "fit 7pts:", "fit 8pts", "fit 9pts"};
			...
			//System.out.println("Working on "+lineType);
				...
				for (int i=0; i<freqs.length; i++){
					...
					int[] predictions=//refineFound(dateAscend, "filterN"+(sqz*10)+lineType);//filterOutSample(dateAscend);;//
							aJob.tryHalfSincFilter(testData);//dateAscend, "filterA"+checkCount);//filterOutSample(dateAscend);;//
				...
				//hasPredic += predictions.toString();
					String key=""+(checkCount*100+aJob.expandedBy);
					String foundTxt="found (";
					iFound=0;
					for (int p=0; p<5; p++){
						for (int k=0; k<predictions.length; k++){
						if (targets[p] != predictions[k]) continue;
							iFound++;
							break;
						}
					}
					Integer k=caseCount.get(key);
					if (k != null) iFound += k;
					caseCount.put(key, new Integer(iFound));
					
					System.out.println("Finish "+iCheckStart+" use "+freqs[i]+" filter with check last "+checkCount+" for "+
				                  (System.currentTimeMillis() - tNow)+" milli secs");
					String predics="get (";
					String predicsTw="預估:(";
					for (int p=0; p<predictions.length; p++){
						...
						found[predictions[p]]++;
						//hasPredic[cc] += ""+predictions[p]+",";
					predics += (""+predictions[p]+"["+dF.format(aJob.merit_of_solution[p])+"], ");
					predicsTw += (""+predictions[p]+", ");
					}
					int iHas=0;
					...
					//iFits[iHas]++;
					testLog.write(" use "+freqs[i]+" filter " +checkCount+" check "+predics+")"+iHas);
					testLog.newLine();
					chLog.write(" 利用 "+freqs[i]+" 取樣 " +checkCount+" 參考 "+predicsTw+")"+iHas);
					chLog.newLine();
			...
			testLog.newLine();
			chLog.write(foundTxt+") "+iFound+" out of "+iPred);
			chLog.newLine();
			...
			testLog.newLine();
			chLog.write(predTxtTw+") ");
			chLog.newLine();
			
			if (iFound > 0) iSuccess++;
			iStatistics[iFound]++;
			if (iLen1 > iCheckStart)
				lastFoundLocation[iFound]=	iCheckStart;
			iCheckStart++;
		}
		testLog.write("try "+cTryCases+" cases and got");
		testLog.newLine();
		for (int p=0; p<6; p++){
			testLog.write( ""+p+":"+iStatistics[p]+", @"+(iLen1-lastFoundLocation[p]));
			testLog.newLine();
		}
		//testLog.write(statText+" and total fit cases:" +iSuccess);testLog.newLine();
		//testLog.write("avg predictions=" +avgCandidates/cTryCases);testLog.newLine();
		/*
		Iterator itr=caseCount.keySet().iterator();
		while (itr.hasNext()){
			String key=(String)itr.next();
			Integer k=caseCount.get(key);
			System.out.println(key +" has "+k);
			testLog.write(key +" has "+k);testLog.newLine();
			chLog.write(key +" has "+k);chLog.newLine();
