...
public class IdleBallOnDayTS {
	...
	static public IdleBallOnDayTS getInstance(String fileName, String whichLine){
		if (fileName==null) return null;
		IdleBallOnDayTS aReader=new IdleBallOnDayTS();
...
*/
	...
	static void createDistribGrid(){
		...
		IdleBallOnDayTS aJob=new IdleBallOnDayTS();
		for (int i=0; i<4; i++){
...
PRNG is the value returned from the portable random number generator (PRNG). 
	...
	static double[] predictionByVariationAlphaFit(int[][] delayCurves, String chartName){
		...
		for (int s=0; s<5; s++){
			...
			new IdleBallOnDayTS().drawFitCurve(dataToPlot, plotStatements, chartName+s);
			dataToPlot.clear();
	...
	static double[] getMovingAvg(double[] orgData, int n){
		double[] avg=new double[orgData.length-n+1];
		Arrays.fill(avg, 0);
		for (int x=n-1; x< orgData.length; x++){
			for (int j=0; j<n; j++){
				avg[x-n+1] += orgData[x-j];
			}
			avg[x-n] /= n;			
		}
		return avg;
	...
	static double[] getMovingAvg(int[] orgData, int n){
		double[] avg=new double[orgData.length-n+1];
		Arrays.fill(avg, 0);
		for (int x=n-1; x< orgData.length; x++){
			for (int j=0; j<n; j++){
	...
	static double[] getOnDaysByAlphaMCFit(int which1, int from, int to, int testPoints){
		...
		//for (int s=0; s<5; s++){
			...
			double lowerBound=(0-0.5 - mu)/sigma;
			...
			double[] nextV=PdfTSByMC.getAlphaPredictionsByMC(alphaCurveMR[2], means, sigmas, upperBound, lowerBound);//, testPoints);
			//getNewAlphaByMC(alphaCurveMR[2], sigma, upperBound, lowerBound);//, testPoints);
			//getAlphaPredictionsByMC(alphaCurveMR[2], sigma, upperBound, lowerBound);//, testPoints);
			//retV[s]=new double[nextV.length];
			//for (int k=0; k<nextV.length; k++)
			retV[0]= mu + nextV[0]*sigma - ballIdleTS[which1][to - 1];
			retV[1]=nextV[1];
		//}
		return retV;
	}
			...
			*/
			...
			for (int ic=0; ic<caseId.length; ic++){				
				...
				for (int ib=1; ib<40; ib++){
					...
					predictions[ib]=(int)Math.round(100+predicts[0])*100+ib;												
				}
				Arrays.sort(predictions);
				sPredictions="predictions:( ";
				int ttPredicts=5;
				int[] subPredictions=new int[ttPredicts];
				Arrays.fill(subPredictions, 0);				
				for (int i=0; i<ttPredicts; i++){
					subPredictions[i]=predictions[i+1] % 100;
					sPredictions += ""+subPredictions[i]+", ";
				}
				predictionTS[ic][testCaseCount-iTest]=Arrays.copyOf(subPredictions, ttPredicts);
				
				Arrays.fill(predictions, 0);
				for (int i=0; i<ttPredicts; i++){
					predictions[subPredictions[i]]=1;
				}
				String sHit="<<<hits:( ";
