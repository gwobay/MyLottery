...
public class JackpotCompressBalls {
	...
	static public JackpotCompressBalls getInstance(String fileName, String whichLine){
		if (fileName==null) return null;
		JackpotCompressBalls aReader=new JackpotCompressBalls();
...
*/
	...
	static double findNextOnDays(double[] timeLine)
	{
		double pI=Math.PI;//*2;
		...
		for (int kn=0; kn<nCase.length; kn++){
			int n=nCase[kn];
			//if (n==15) continue;
			double[] localTs=Arrays.copyOfRange(timeLine, len1-n*3, len1);
			double[] compressTs=new double[n];
			for (int i=0; i<n; i++){
				compressTs[i]=localTs[i*3]+2*localTs[i*3+1]+4*localTs[i*3+2];			
			}
			...
			while (offSet < 89){
				...
				double[] pCoeff=iv.multiply(compressTs);
				//double[] checkBack=mFit.multiply(pCoeff);
				//for (int ik=0; ik<n; ik++){
					//if (Math.abs(checkBack[ik] - compressTs[ik]) > 0.01){
						//System.out.println("try "+n+" found invalid inverse matrix for element "+ik+" mismatch:"+dF.format(checkBack[ik])+" != "+dF.format(compressTs[ik]));
					//}
				//}
				double probV=0;
				for (int v=0; v<n; v++){
				...
				//apprP[i-1]=probV;
				probV += pLocal;
				if (probV < -0.5 || probV > 8.5) continue;
				pV = (Math.round(probV) % 2);
				break;
			}
			//pV /= 6; 
			//pV += pLocal;
		...
		return predV;//(predV/nCase.length);// % 2);
	}
	...
	static void findDrawnTimeLine(){
		...
		BufferedWriter aWriter;
		try {
			aWriter=new BufferedWriter(new OutputStreamWriter(new FileOutputStream("timeLineCompressResults.txt")));
			int[] hitStatistics=new int[6];
			Arrays.fill(hitStatistics, 0);
			int[] lastHitAt=new int[6];
			Arrays.fill(lastHitAt, 0);
			double allTimeMin=100000;
			
		for (int iTestCase=iC-300; iTestCase >= 0;  iTestCase--){
			System.out.println("Total Samples:"+iC+" working on "+iTestCase);
			...
			for (int ib=1; ib<40; ib++){			
				predBase[ib]=Math.abs(findNextOnDays(Arrays.copyOfRange(timeLine[ib], testDataFrom, testDataTo)));
			}
			...
			int[] sols=new int[40];
			for (int ib=1; ib<40; ib++){
				//if (predBase[ib] > predBase[ib-1] && predBase[ib] > predBase[ib+1]){
				if (predBase[ib] > 0){//Math.round(predBase[ib]) % 2 == 1){  //for 3 bit compress
			...
			aWriter.write(""+iFound+sFound+") ; avg="+dF.format(pValue));aWriter.newLine();//+" hit min ratio:"+dF.format(minV/pValue)); aWriter.newLine();
			aWriter.write(sDraw+") ------>"+cHit+sHits);aWriter.newLine();
			aWriter.write(")///////////////");aWriter.newLine();
		...
		aWriter.write("Total "+(iC-300)+" cases");aWriter.newLine();
		for (int i=0; i<6; i++){
			aWriter.write("has "+i+"hits "+hitStatistics[i]+" times; last at "+lastHitAt[i]);
			aWriter.newLine();
		}
		aWriter.close();
		} catch (FileNotFoundException e){
	...
	public static void main(String[] args) {
		...
		int select_type=0;
		JackpotCompressBalls aReader=JackpotCompressBalls.getInstance("test539.txt", drawStyle[select_type]);
		...
		BufferedWriter aWriter;
		try {
			aWriter=new BufferedWriter(new OutputStreamWriter(new FileOutputStream("Ball"+drawDate+"S"+checkCase+"B"+trySize+"Y"+checkCount+".txt")));
