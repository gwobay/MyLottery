...
public class JackpotListBestTriple {
	...
	static DecimalFormat dI=new DecimalFormat("000000");
	...
	static public JackpotListBestTriple getInstance(String fileName, String whichLine){
		if (fileName==null) return null;
		JackpotListBestTriple aReader=new JackpotListBestTriple();
...
*/
	...
	public static String[] projByAvgDiff(int forDay, Vector<int[]> samples, int[] hits, BufferedWriter aWriter, int[][][] localPerformanceList){
		int dataLength=samples.get(0).length;
		...
		Vector<double[]> rSamples=new Vector<double[]>();
		double[] nextProj=new double[samples.size()];
		double[][] expectDistance=new double[samples.size()][4];
		
		double[][] prjDiffs=new double[samples.size()][iEnsamble];
		String[] showPredics=new String[3];//"Preds:(";
		for (int k=0; k<samples.size(); k++){
			//aWriter.write("/////////////// LINE  "+(k+1)+"//////////////////////");
			//aWriter.newLine();
			//no need to reverse already ascending
			...
			double[] rSample1=new double[samples.get(k).length];
			rSample1[0]=samples.get(k)[0];
			rSample1[1]=samples.get(k)[1];
			rSample1[2]=samples.get(k)[2];
			
			for (int i=3; i<samples.get(k).length; i++){
				rSample1[i]=samples.get(k)[i];
				int d=samples.get(k)[i] - samples.get(k)[i-1];
				if (d <=0) d += 39;
				mvDistanceCount[0][d]++;
				d=samples.get(k)[i] - samples.get(k)[i-2];
				if (d <=0) d += 39;
				mvDistanceCount[1][d]++;
				d=samples.get(k)[i] - samples.get(k)[i-3];
				if (d <=0) d += 39;
				mvDistanceCount[2][d]++;				
			}
		...
		for (int dis=0; dis<3; dis++){
			...
			aWriter.write(">>>"+found);
				...
				if (forDay > 0)
				localPerformanceList[dis][found][6*39-forDay]=1;
		}
			aWriter.newLine();
			aWriter.newLine();	
		} catch (IOException e){
	...
	static void readHistoryFile(Vector<int[]> samples){
			...
			try {
				...
				//if (pdfFile)
				int[][] lines=new int[5][5000];
				//int[] line2=new int[3000];
				//int[] line3=new int[3000];
				//int[] line4=new int[3000];
				//int[] line5=new int[3000];
				for (int i=0; i<5; i++) Arrays.fill(lines[i], -1);
				//int iCc=0;
				int iC=0;
				...
				while ((aLine=aReader.readLine()) != null){
					...
					for (int i=0; i<5; i++) {
						...
						int data1=Integer.parseInt(terms[i]);
				...
				aReader.close();
				for (int i=0; i<5; i++) {
					samples.add(Arrays.copyOf(lines[i], iC));
					//allTimePDF[datas[i]]++;
				}
				
				/*
				aReader=new BufferedReader(new FileReader(new File(dataCenter+"permute539.txt")));
				while ((aLine=aReader.readLine()) != null){
					if (aLine.length()<5) continue;
	...
	static void getQuarTriple(Vector<int[]> samples)
	{
		...
		for (int x=samples.get(0).length-1; x>=0; x--){
			for (int n1=0; n1<5-3; n1++){
				for (int n2=n1+1; n2<5-2; n2++){
					for (int n3=n2+1; n3<5-1; n3++){
						for (int n4=n3+1; n4<5-0; n4++){
						tripleCount[samples.get(n1)[x]][samples.get(n2)[x]][samples.get(n3)[x]][samples.get(n4)[x]]++;
						lastLoc[samples.get(n4)[x]][samples.get(n1)[x]*10000+samples.get(n2)[x]*100+samples.get(n3)[x]]=x+1;
					}
					}
				}
			}
		...
		BufferedWriter aWriter;
		try {
			aWriter=new BufferedWriter(new OutputStreamWriter(new FileOutputStream("qtripleList.txt")));
			...
			for (int i=0; i<40; i++){
				for (int j=0; j<40; j++){
					for (int k=0; k<40; k++){
						...
						sTripCount += dI2.format(i)+"x";
						sTripCount += dI2.format(j)+"x";
						sTripCount += dI2.format(k)+"x";
			...
			while (itr.hasNext()){
				...
				{ 
					...
					//if (lastLocation > samples.get(0).length/3)
					{
					...
					if ( lastLocation > samples.get(0).length/3)lastAt += "<---";
					aWriter.write(aLine+"  -----> "+iCount+lastAt);aWriter.newLine();	
					}
				}		
			}
			aWriter.write("====== total Days="+samples.get(0).length+ "=========================");aWriter.newLine();
	...
	static void getTriple(Vector<int[]> samples)
	{
			...
			for (int n1=0; n1<5-2; n1++){
				for (int n2=n1+1; n2<5-1; n2++){
					for (int n3=n2+1; n3<5-0; n3++){
						tripleCount[samples.get(n1)[x]][samples.get(n2)[x]][samples.get(n3)[x]]++;
						lastLoc[samples.get(n1)[x]*10000+samples.get(n2)[x]*100+samples.get(n3)[x]]=x+1;
					}
				}
			}
		}
		...
		BufferedWriter aWriter;
		try {
			aWriter=new BufferedWriter(new OutputStreamWriter(new FileOutputStream("tripleList.txt")));
			...
			while (itr.hasNext()){
				...
				if (iCount > 3){ lastAt="@"+(lastLoc[Integer.parseInt(terms[1])*10000+Integer.parseInt(terms[2])*100+Integer.parseInt(terms[3])]);
				aWriter.write(aLine+"  -----> "+iCount+lastAt);aWriter.newLine();	}		
			}
			aWriter.write("====== total Days="+samples.get(0).length+ "=========================");aWriter.newLine();
			for (int i=0; i<summary.length; i++){
	...
	static int[][] lastResults=new int[4][6];
	static int[][] testResults=new int[4][6];
	
public static void main(String[] args) {
		// TODO Auto-generated method stub
		final String[] criteria={"開出順序", "大小順序"};
		//for (int iType=0; iType<2; iType++){
			int setType=1;//iType;//setTypes[iq];
			String lineType=criteria[setType];
		
		JackpotReader aReader=JackpotReader.getInstance(dataCenter+"test539.txt", lineType);
		Vector<int[]> samples=new Vector<int[]>();//aReader.readData(5, 39, 512);
		readHistoryFile(samples);
		int[][][] tripleCount=new int[40][40][40];
		for (int i=0; i<40; i++){
			for (int j=0; i<40; i++){
				for (int k=0; i<40; i++){
		...
		try {
			...
			while (itr.hasNext()){
				...
				{ 
					...
					{
					lastAt="@"+lastLocation;//(lastLoc[Integer.parseInt(terms[4])][Integer.parseInt(terms[1])*10000+Integer.parseInt(terms[2])*100+Integer.parseInt(terms[3])]);
					if ( lastLocation > samples.get(0).length/d)
						{
						lastAt += "<---";
						if (iCount > 6)
						vipList.add(terms[1]);
						}
					if ( lastLocation*d > samples.get(0).length*2.5){
						if (iCount > 4)
							vipList.add(terms[1]);
						lastAt += "	ovd";
				...
				//if (iCount > samples.get(0).length/7){ lastAt="@"+(lastLoc[Integer.parseInt(terms[1])*10000+Integer.parseInt(terms[2])*100+Integer.parseInt(terms[3])]);
				//aWriter.write(aLine+"  -----> "+iCount+lastAt);aWriter.newLine();	}		
			}
			aWriter.write("====== total Days="+samples.get(0).length+ "=========================");aWriter.newLine();
			for (int i=0; i<summary.length; i++){
