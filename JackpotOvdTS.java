...
public class JackpotOvdTS {
	...
	static public JackpotOvdTS getInstance(String fileName, String whichLine){
		if (fileName==null) return null;
		JackpotOvdTS aReader=new JackpotOvdTS();
	...
	//int dmY=d.height;
        ...
        */
		int segSize=myChartWidth*8/predictionList.size()/10;
        for (int i=0; i<predictionList.size(); i++){
...
*/
	...
	static void createDistribGrid(){
		...
		JackpotOvdTS aJob=new JackpotOvdTS();
		for (int i=0; i<4; i++){
	...
	static int[][] allDrawIdDateDescending=null;
	...
	static int[][] getHistoryData(){
		...
		allDrawIdDateDescending=new int[allLines.size()][5];
		...
		for (int i=0; i<allLines.size(); i++){
			...
			for (int s=0; s<5; s++) {
				...
				allDrawIdDateDescending[loc][s]=data[s];
			}
			for (int s=0; s<5; s++) {
...
PRNG is the value returned from the portable random number generator (PRNG). 
	...
	static int[][] buildHitBallDelayCountCurves(){
		if (allBallTS==null || allDrawIdDateDescending==null) 
			allBallTS=getHistoryData();
		int[][] refData=allDrawIdDateDescending;
		int[][] retV=new int[5][allBallTS[0].length];
		for (int k=0; k<5; k++ )
			Arrays.fill(retV[k], 0);
		for (int loc=10; loc<allDrawIdDateDescending.length; loc++){
			
			int[] sortedTemp=new int[5];
			//if (loc < allDrawIdDateDescending.length)
			for (int k=0; k<5; k++){
				int j=loc-1;
				int bHit=refData[loc][k];
				sortedTemp[k]=allBallTS[bHit][j];
	...
	static BufferedWriter getPredictionByStatistics(){
		...
		int ttHitCount=0;
		//int curveLengthUsed4Predict=6;
		//double[] ovd=new double[sampleSize+1];
		int[][] refData=allDrawIdDateDescending;
		for (int iTest=sampleSize; iTest>=0; iTest--){
			int loc=refData.length-iTest;
			/*
			int[] sortedTemp=new int[5];
			...
			for (int k=0; k<5; k++){
				int j=loc;
				int bHit=refData[loc][k];
				sortedTemp[0]=allBallTS[bHit][j];
				if (sortedTemp[0] == 1) sortedTemp[0]=0;
				else {
					while (allBallTS[bHit][j--] < 1) {
						sortedTemp[0]++;
					}
				}
			}
	...
	static double getPredictionByCurve(double[] alphaCurve, int checkMaxMinCount){
		double retV=0;
		double avg=0;
		double maxV=-10000;
		double minV=100000;
	...
	static double[] predictionByVariationAlphaFit(int[][] delayCurves){
		...
		for (int s=0; s<5; s++){
			...
			double nextV=getPredictionByCurve(alphaCurve, 6);
			...
			plotStatements.add("0:"+dF.format(zero)+"/d="+dF.format(delta)+"   next="+dF.format(nextV));
			new JackpotOvdTS().drawFitCurve(dataToPlot, plotStatements, "delayCurvesFitAlpha"+s);
			dataToPlot.clear();
	...
	public static void main(String[] args) {
		...
		//Vector<int[]> samples=Lottery539DataReader.getAllData();//aReader.readData(5, 39, 512);
		//Vector<double[]> rSamples=new Vector<double[]>();
		...
		for (int k=0; k<delayCurves.length; k++){
			...
			new JackpotOvdTS().drawFitCurve(dataToPlot, plotStatements, "delayCurves"+k);
			dataToPlot.clear();
			plotStatements.clear();
		}
		...
		/*
		BufferedWriter aWriter;
		int[] timerBuildDays={6*39};//18, 36, 54, 72, 90, 108, 120};
		...
		try {
			...
			for (int iTest=6*39; iTest>=0; iTest--){
				int[] hits=new int[5];
				Arrays.fill(hits, 0);
				...
				if (iTest>0)
					localPerformanceList[foundHit][234-iTest]=1;
				aWriter.write("==========================================="); aWriter.newLine();
			}
