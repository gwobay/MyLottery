...
public class JackpotReadBallsAvg {
	...
	static public JackpotReadBallsAvg getInstance(String fileName, String whichLine){
		if (fileName==null) return null;
		JackpotReadBallsAvg aReader=new JackpotReadBallsAvg();
...
*/
	...
	static void findDrawnTimeLine(){
		...
		BufferedWriter aWriter;
		try {
			aWriter=new BufferedWriter(new OutputStreamWriter(new FileOutputStream("ballTimeLineByAvgAll.txt")));
			System.out.println("JackpotReadBallsAvg Creating file "+"ballTimeLineByAvgAll.txt");
			...
			String[] showPredictions=new String[3];
			int[][][] predictionTS=new int[3][testCaseCount+1][];
		...
		for (int iTestCase=testCaseCount; iTestCase >= 0;  iTestCase--){
			...
			int cHit=0;
			int[] sols=new int[40];
			int[] predictionList=new int[40];
			Arrays.fill(predictionList, 0);
			...
			for (int ib=1; ib<40; ib++){
				if (iTestCase==0){
					if (predBase[ib] > maxData) maxData=predBase[ib];
					else if (predBase[ib] < minData) minData=predBase[ib];
					if (absPredics[ib] > maxAbs) maxAbs=absPredics[ib];
					else if (absPredics[ib] < minAbs) minAbs=absPredics[ib];
				}
			...
			if (iTestCase==0){
				...
				maxData -= minData;
				int[] plotAbsData=new int[40];
				Arrays.fill(plotAbsData, 0);
				maxAbs -= minAbs;
				showPredictions[iCheck]="check "+iCheck+" has predics:( ";
				for (int ib=1; ib<40; ib++){
					if (predictionList[ib]>0) showPredictions[iCheck] += ""+ib+", ";
					double r=100*(predBase[ib]-minData)/maxData;
					plotData[ib]=(int)Math.round(r);
					r=100*(absPredics[ib]-minAbs)/maxAbs;
					plotAbsData[ib]=(int)Math.round(r);
				}
				dataToPlot.add(plotData);
				dataToPlot.add(plotAbsData);
				
				//createPngFile("timerStrengthAvg"+checkCount, dataToPlot, 100, 40);
			}
			...
			for (int ib=1; ib<40; ib++){
				if (predBase[ib] > lowB) stdby += (""+ib+"["+dF.format(predBase[ib])+">,");
			}
		...
		String myName="JackpotReadBallsAvg";
...
/*
		...
		predictBase.write("=================== JackpotReadBallsAvg ======");predictBase.newLine();
		aWriter.write("=================== JackpotReadBallsAvg ======");aWriter.newLine();
		for (int ij=0; ij<3; ij++){
			aWriter.write(showPredictions[ij]);aWriter.newLine();	
			predictBase.write(showPredictions[ij]);predictBase.newLine();	
