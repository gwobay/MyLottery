...
public class JackpotReadBallsAvgJumpFit {
	...
	static public JackpotReadBallsAvgJumpFit getInstance(String fileName, String whichLine){
		if (fileName==null) return null;
		JackpotReadBallsAvgJumpFit aReader=new JackpotReadBallsAvgJumpFit();
...
*/
	...
	static void findDrawnTimeLine(){
		...
		File history=new File(JackpotReader.dataCenter+"History.txt");
		if (!history.exists()) {
			aParser.readFromWeb();
			findDrawnTimeLine();
			return;
		...
		BufferedWriter aWriter;
		try {
			aWriter=new BufferedWriter(new OutputStreamWriter(new FileOutputStream("ballTimeLineJumpFit"+".txt")));
			System.out.println("JackpotReadBallsAvgJumpFit Creating file "+"ballTimeLineJumpFit"+".txt");
			int[] fitRange={9, 12, 15};
			int[][] lastHitAt=new int[fitRange.length][6];
			int[][] hitStatistics=new int[fitRange.length][6];
			int[][][] localPerformanceList=new int[fitRange.length][6][testCaseCount];
			String[] showPredictions=new String[fitRange.length];
			for (int i=0; i<fitRange.length; i++){
				Arrays.fill(hitStatistics[i], 0);
				Arrays.fill(lastHitAt[i], 0);
				for (int x=0; x<6;x++)
					Arrays.fill(localPerformanceList[i][x], 0);
			}
			int[][][] predictionTS=new int[fitRange.length][testCaseCount+1][];
		...
		for (int iTestCase=testCaseCount; iTestCase >= 0;  iTestCase--){
			...
			for (int iCheck=0; iCheck<3; iCheck++){
				...
				aWriter.write("........ checkcount."+iTestCase+".... "); aWriter.newLine();
			...
			predictionTS[iCheck][testCaseCount-iTestCase]=Arrays.copyOf(subPredictions, iSol);
			
			hitStatistics[iCheck][cHit]++;
			if (iTestCase != 0)
			lastHitAt[iCheck][cHit]=iTestCase;
			pValue /= iFound;
			if (iTestCase>0)
				localPerformanceList[iCheck][cHit][testCaseCount-iTestCase]=1;
			aWriter.write(iFound+sFound+") ; "); 
			aWriter.newLine();
			showPredictions[iCheck]=sFound;
		...
		String myName="JackpotReadBallsAvgJFT";
		JackpotReader.showSummaryPage(predictionTS, hitStatistics, lastHitAt, localPerformanceList, myName, aWriter);
/*
		BufferedWriter predictBase=new BufferedWriter(new OutputStreamWriter(new FileOutputStream("toPredictBase.txt", true)));						
		
		predictBase.write("=================== JackpotReadBallsAvgJFT ======");predictBase.newLine();
		aWriter.write("=================== JackpotReadBallsAvg JFT======");aWriter.newLine();
		
		for (int iCheck=0; iCheck<fitRange.length; iCheck++){
			aWriter.write("check count="+(9+iCheck*3)+".............");aWriter.newLine();
			aWriter.write(showPredictions[iCheck]);aWriter.newLine();
			predictBase.write("check count="+(9+iCheck*3)+".............");predictBase.newLine();
			predictBase.write(showPredictions[iCheck]);predictBase.newLine();
			
			JackpotReader.performanceList=localPerformanceList[iCheck];
			...
			for (int i=0; i<6; i++){
				...
				int iLL=localPerformanceList[iCheck][i].length-10;
				for (int ia=0; ia<10; ia++){
					showOnOffSeq += ""+localPerformanceList[iCheck][i][iLL+ia];
				}
				predictBase.write(showOnOffSeq+"--- ");
				aWriter.write(showOnOffSeq+"--- ");
				
				predictBase.write("Hits "+i+" : ["+hitStatistics[iCheck][i]+"]@ "+lastHitAt[iCheck][i]+" ??"+dF.format(nextCasePdf[i]));				
				aWriter.write("Hits "+i+" : ["+hitStatistics[iCheck][i]+"]@ "+lastHitAt[iCheck][i]+" ??"+dF.format(nextCasePdf[i]));
				if (caseMeanVar[i] > 0 && lastHitAt[iCheck][i] > caseMeanVar[i]) {
					double ovdToVar=(lastHitAt[iCheck][i] - caseMeanVar[i])/caseMeanVar[i+6];
	...
	public static void main(String[] args) {
		...
		int select_type=0;
		JackpotReadBallsAvgJumpFit aReader=JackpotReadBallsAvgJumpFit.getInstance("test539.txt", drawStyle[select_type]);
		...
		int[] trySizes={39*3, 39*4, 39*5};//52, 13*5};
		int[] checkCases={39, 39*2, 39*3, 39*4};
		for (int ic=0; ic<checkCases.length; ic++){
			int checkCase=checkCases[ic];
			for (int iBase=0; iBase<trySizes.length; iBase++){
				trySize=trySizes[iBase];
		...
		BufferedWriter aWriter;
		try {
			aWriter=new BufferedWriter(new OutputStreamWriter(new FileOutputStream("jfBall"+drawDate+"S"+checkCase+"B"+trySize+"Y"+checkCount+".txt")));
			...
			for (int iTest=testCaseCount; iTest>=0; iTest--){
				...
				for (int k=0; k<samples.size(); k++){
					...
					int[] predicts=populateCircle(k, Arrays.copyOfRange(samples.get(k), iTest, dataCount), aWriter);
