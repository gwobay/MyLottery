...
public class LotteryNoiseData {

	LotteryNoiseData(int x, int y){
	...
	LotteryNoiseData(Vector<double[]> data){
		this.data=data;
		numRows=data.size();
		numCols=0;
	...
	LotteryNoiseData(LotteryNoiseData copy){
     ...
     */
    public LotteryNoiseData subMatrix( int row, int col ){
    	Vector<double[]> retMx=new Vector<double[]>();
    	for (int x=row-1; x<numRows; x++){
    		retMx.add(Arrays.copyOfRange(data.get(x), col-1, numCols));
    	}
    	return new LotteryNoiseData(retMx);
    };

    public LotteryNoiseData subMatrix( int row, int col, int range ){
    ...
    public LotteryNoiseData subOrderMatrix( int row, int col){ 
    	Vector<double[]> retMx=new Vector<double[]>();
    ...
    public double determined(LotteryNoiseData a){
...
int[] index2Scale;
    public LotteryNoiseData multiply(LotteryNoiseData a){
    	...
    	LotteryNoiseData b=new LotteryNoiseData(retV);
    	b.index2Scale=index2Scale;
    	return b;
    }
    
    public LotteryNoiseData multiplyBy(LotteryNoiseData a){
    	Vector<double[]> retV=new Vector<double[]>();
    	...
    	return new LotteryNoiseData(retV);
    }
    ...
    public LotteryNoiseData transpose(){
    	Vector<double[]> retV=new Vector<double[]>();
    ...
    LotteryNoiseData getI(int dime){
    	Vector<double[]> retV=new Vector<double[]>();
    ...
    LotteryNoiseData scalerMatrix(double scale, int forLine, int dime){
    ...
    LotteryNoiseData forwardElimitor(LotteryNoiseData forThis, int whichCol){
    	Vector<double[]> retV=new Vector<double[]>();
    ...
    static void cleanError(LotteryNoiseData a){
...
//......................... unrelated ...........................
static double[] getCoeff(LotteryNoiseData dMtx, int startFrom, int range, int[] refD){
...
static double[] getCoeff(LotteryNoiseData dMtx, int startFrom, int range, double[] refD){
	double[] retC=new double[range];
...
static LotteryNoiseData getSinValuesMatrix(int startFrom, int range){
...
//--------------------------------------------------
	public static void main(String[] args) {
		...
		//------------- function test ------
		LotteryNoiseData b=new LotteryNoiseData(2, 2);
		b.update(1, 1, 1);
		b.update(2, 2, 4);
		b.update(2, 1, 2);
		b.update(1, 2, 3);
		double d=b.determined();
		LotteryNoiseData b_1=b.inverse();
		
		LotteryNoiseData a=new LotteryNoiseData(7, 7);
		/*
		a.update(1, 1, 1);
		a.update(2, 2, 4);
		a.update(3, 3, 8);
		...
		*/
		...
		d=a.determined();
		LotteryNoiseData a_1=a.inverse();
		LotteryNoiseData m1;
		...
		LotteryNoiseData eche=new LotteryNoiseData(a);
		for (int i=7; i>0; i--){
			//eche=b.forwardElimitor(eche, i).multiply(eche);
			LotteryNoiseData tmp=b.backwardElimitor(eche, i);
			if (tmp==null){
		...
		for (int range=5; range < 11; range++){
	       ...
	       LotteryNoiseData test39=getSinValuesMatrix(iLen1-range, range);
	       long tNow=System.currentTimeMillis();
	       //d=test39.determined();
	       LotteryNoiseData work39=test39.inverse();
	       if (work39 == null){
