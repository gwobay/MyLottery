...
public class LowPassData extends JFrame implements Runnable 
{
/**
...
static boolean sincDataSet=false;
...
static int[] tryNewSample( Vector<int[]> sampled, String fname){
	...
	// ratio += getMeanRatio(Arrays.copyOfRange(sampled.get(0), iLen-checkFrom, iLen));
	 ...
	 for (int s=0; s<5; s++){
		 ...
		 good1 += " with diff="+dF.format(diff2);
		 good1 += " has a="+dF.format(diff3[0])+" b="+dF.format(diff3[1])+" c="+dF.format(diff3[2]);
		 if (aWriter != null){ aWriter.write(good1+")"+bestRatio+"; n="+bestDt);aWriter.newLine();}
		 bestFitPoints[s]=diffAt[0];
	 }
	 if (aWriter != null){
		aWriter.close();}
...
public int[] findPredictions(Vector<int[]> dateAscend){
	
	if (!sincDataSet)
	getSincData();
	int[] predCount=new int[40];
	Arrays.fill(predCount, 0);
	for (int cc=2; cc<4; cc++){
		...
		for (int i=0; i<dd.length; i++){
			...
			int[] orgF=//refineFound(dateAscend, "filterN"+(sqz*10)+lineType);//filterOutSample(dateAscend);;//
					tryNewSample(dateAscend, null);//"for"+JackPotReader.drawDate+
							//"filterN"+(sqz*10)+lineType);
