...
public class LowPassDataMath extends JFrame implements Runnable 
{
/**
...
static double getMeanRatio1(int[] inData){
		int iLen=inData.length;
		double meanRatio=0;
	 
	 for (int i=0; i<iLen; i++){
		 double conv=0;
		 for (int k=1; k<3*sqz+1; k++){
			 //if (i+k<1) continue;
			 if (i+k > iLen) break;
			 conv += sinc40PI[Math.abs(k)]*inData[i+(k-1)/sqz]*2/myPI;
		 }
		 for (int k=1; k<3*sqz+1; k++){
			 if (i- k<0) break;
			 //if (i+k > iLen) break;
			 conv += sinc40PI[Math.abs(k)]*inData[i-k/sqz]*2/myPI;
		 }
		 //conv /= 2.1174;
...
static double[] getMeanRatioG(Vector<int[]> aData){
	
	
	double[] meanRatios=new double[aData.size()];
	
	for (int s=0; s<aData.size(); s++){
		int[] inData=aData.get(s);
		int iLen=inData.length;
		double meanRatio=0;
	 int iC=0;
	 ...
	 meanRatio /= iC;
	 meanRatios[s]=meanRatio;
	}
	 return meanRatios;
}

static double[] getMeanRatio(Vector<int[]> aData){
	 ...
	 for (int s=0; s<5; s++){
		 ...
		 for (int i=0; i<iLen; i++){
			 ...
			 for (int k=1; k<3*sqz+1; k++){
				 //if (i+k<1) continue;
				 if (i+k > iLen) break;
				 conv += sinc40PI[Math.abs(k)]*sampled.get(s)[i+(k-1)/sqz]*2/pi;
			 }
			 for (int k=1; k<3*sqz+1; k++){
				 if (i- k<0) break;
				 //if (i+k > iLen) break;
				 conv += sinc40PI[Math.abs(k)]*sampled.get(s)[i-k/sqz]*2/pi;
			 }
			 conv /= 2.1174;
			 newData[i]=(int)Math.round(conv);
			 double diff=conv-sampled.get(s)[i];
...
static int[] tryNewSample( Vector<int[]> sampled, String fname){
	int[] fitPoint=new int[sampled.size()];
	int iLen=sampled.get(0).length;
	double pi=Math.PI;
	 Vector<int[]> retV=new  Vector<int[]>();
	 for (int s=0; s<sampled.size(); s++){
		 retV.add(Arrays.copyOfRange(sampled.get(0), iLen-40, iLen));
	 }
	 double[] ratio=getMeanRatioG(retV);//getMeanRatio(retV);
	
	  BufferedWriter aWriter=null;
	 String fileName="C:\\Users\\eric\\workspace\\GetLottery\\539\\";
	 if (fname != null)
		 fileName += fname;
	 try {
		 if (fname != null)
	 ...
	 for (int s=0; s<5; s++){
		 ...
		 for (int n1=1+s; n1<36+s; n1++){
			 newData[35]=n1;
			 //double ratio1=getMeanRatio1(newData);
		 ...
		 for (int ix=0; ix<2; ix++){
			 ...
			 for (int k=1; k<3*sqz+1; k++){
				 //if (i+k<1) continue;
				 if (i+k > 40) break;
				 conv += sinc40PI[Math.abs(k)]*newData[i+(k-1)/sqz]*2/pi;
			 }
			 for (int k=1; k<3*sqz+1; k++){
				 if (i- k < 0) break;
				 //if (i+k > iLen) break;
				 conv += sinc40PI[Math.abs(k)]*newData[i-k/sqz]*2/pi;
			 }
			 conv /= ratio[s];//ratio1;//1.795;//2.1174;//
			 //newData[i]=(int)Math.round(conv);
			 diffS[ix]=Math.abs(conv-newData[i]);
			 //double ratio=Math.abs(conv)/sampled.get(s)[i];
		 }
		 double tDiff=diffS[0]+diffS[1];
		 if (tDiff < diff2){
			 diff2=tDiff;
			 diff12[0]=diffS[0];
			 diff12[1]=diffS[1];
			 ...
			 if (diff2<0.01){
				 if (aWriter != null){
				 aWriter.write("fit AT:("+fitAt[0]+","+fitAt[1]+","+fitAt[2]+","+fitAt[3]+")");aWriter.newLine(); 
				 }
			 }
		 }
					 }
		 ...
		 //meanRatio /= iLen;
		 retV.add(Arrays.copyOf(newData, iLen));
		 good1 += "("+fitAt[0]+","+fitAt[1]+","+fitAt[2]+","+fitAt[3]+")";
		 good1 += " with diff="+dF.format(diff2);
		 good1 += " has a="+diff12[0]+" b="+diff12[1];
		 if (aWriter != null){aWriter.write(good1);aWriter.newLine();}
...
public static void main(String[] args)
{
	...
	Vector<int[]> sampleData=jReaderOrder.readData(5, 39, 390);	
		...
		sqz=2;
		String fileName="C:\\Users\\eric\\workspace\\GetLottery\\539\\tryFilter"+lineType+".txt";
		int[] orgF=tryNewSample(dateAscend, "filterFor"+lineType);
		/*
		BufferedWriter aWriter;
		 try {
		...
		for (int ix=60; ix<iLen1; ix++){
			Vector<int[]> testData=new Vector<int[]>();
			for (int s=0; s<5; s++){
				
				testData.add(Arrays.copyOfRange(dateAscend.get(s), ix-40, ix));
			}
			int[] orgF=tryNewSample(testData, null);//"filterFor"+lineType);//filterOutSample(dateAscend);;//
			String found="found (";
			String projs=" try (";
			String test="test (";
			int tt=0;			
				for (int t=0; t<testData.size(); t++){
					test += ""+dateAscend.get(t)[ix]+", ";
					projs += ""+orgF[t]+", ";
					for (int i=0; i<orgF.length; i++){
					if (dateAscend.get(t)[ix]!=orgF[i]) continue;
						tt++;
						found += ""+orgF[i]+",";
						break;
					}
				...
				aWriter.write(test+")"+projs+")"+found+")"+tt);
				aWriter.newLine();
			}
		aWriter.close();
		 }  catch (IOException e){}
		*/
		...
		//measuredPDF=getRangePDF(0, globalSample.get(0).length, globalSample);
		LowPassDataMath aCharter=new LowPassDataMath();
		...
		*/
		...
		LowPassDataMath.fileBase="C:\\Users\\eric\\workspace\\GetLottery\\539\\";
		...
		//aCharter.putList2File(atf, "autolines",
				//globalSample);
		//}
		Vector<double[]> pDup=LowPassDataMath.getRangePrimeFit(8, sampleData);
	    //getAutoCorrelation(sampleData);
	//for (int i=0; i<5; i++){
	aCharter.putList2File(pDup, "dupPrime"+lineType,
			globalSample);
	//}
