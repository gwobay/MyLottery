...
public class MyMatrix {
	...
	MyMatrix(Vector<double[]> data1){
		numRows=data1.size();
		numCols=0;
		for (int i=0; i<numRows; i++){
			if (data1.get(i).length > numCols)
				numCols=data1.get(i).length;
		}
		data=new Vector<double[]>();
		
		for (int i=0; i<numRows; i++){
			data.add(Arrays.copyOf(data1.get(i), numCols));
		}
     ...
     */
    public MyMatrix subMatrix( int row, int col ){
    	Vector<double[]> retMx=new Vector<double[]>();
    	for (int x=row-1; x<numRows; x++){
    		retMx.add(Arrays.copyOfRange(data.get(x), col-1, numCols));
    	}
    	return new MyMatrix(retMx);
    };

    public MyMatrix subMatrix( int row, int col, int range ){
    ...
    public MyMatrix subOrderMatrix( int row, int col){ 
    	Vector<double[]> retMx=new Vector<double[]>();
    	for (int x=0; x<numRows; x++){
    		if (x==row-1) continue;
    ...
    public double determined(MyMatrix a){
...
//int[] index2Scale;
    public MyMatrix multiply(MyMatrix a){
    	...
    	MyMatrix b=new MyMatrix(retV);
    	//b.index2Scale=index2Scale;
    	return b;
    }
    
    public MyMatrix multiplyBy(MyMatrix a){
    	Vector<double[]> retV=new Vector<double[]>();
    ...
    MyMatrix scalerMatrix(double[] scales, int dime){
    ...
    MyMatrix scanForScaleMatrix(MyMatrix forThis){
    ...
    MyMatrix forwardElimitor(MyMatrix forThis, int whichCol){
    	...
    	MyMatrix retM=getI(forThis.numCols);
    ...
    MyMatrix backwardElimitor(MyMatrix forThis, int whichCol){
    ...
    static public MyMatrix inverseLU(MyMatrix a){
		...
		for (int i=a.numCols; i>1; i--){
			...
			MyMatrix tmp=a.backwardElimitor(eche, i);
			if (tmp==null){
		...
		for (int i=1; i<a.numCols; i++){
			...
			MyMatrix tmp=a.forwardElimitor(eche, i);
			if (tmp==null){
				System.out.println("Failed in "+i+" for 0 diagonal value");
				break;
			}
    ...
    static void cleanError(MyMatrix a){
...
static int[] predictFromFitOld(int n_points, int[] refV){
...
public static void main(String[] args) {
		...
		*/
		...
		MyMatrix b_1=inverseLU(b);
		
		int dimen=12;
		MyMatrix a=new MyMatrix(dimen, dimen);
		/*
		a.update(1, 1, 1);
		a.update(2, 2, 4);
		a.update(3, 3, 8);
		...
		*/
		...
		int[] predicStatics=new int[40];
		Arrays.fill(predicStatics, 0);
		...
		for (int range=6; range < iRange_B; range++){
			for (int n=0; n<meanedData.size(); n++){ 
				...
				dataPeriod = 1+0.125*delta;//meanPeriod[n]+testPeriod*delta;
			double angleBase=dataPeriod;//twoPi/dataPeriod;
					
	       MyMatrix test39=getSinValuesMatrix(iLen1-range+1, range);
	       //tNow=System.currentTimeMillis();
	       //d=test39.determined();
	       MyMatrix work39=inverseLU(test39);
	       if (work39 == null){
		...
		String sDelta="overAll predictions:(";
		for (int i=0; i<predicStatics.length; i++){
			if (predicStatics[i] > 0)
			sDelta += (""+i+"["+predicStatics[i]+"],");
			//goodNumber[i]=0;
		}
		System.out.println(sDelta);
}
