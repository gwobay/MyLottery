...
public class OneFitManyData extends JFrame implements Runnable 
{
/**
...
public static void main(String[] args)
{
		...
		int dataLength=sampleData.get(0).length;
		Vector<int[]> sampleDataDelta=new Vector<int[]>();
		for (int k=0; k<sampleData.size(); k++){
			int[] delta=new int[dataLength];
			Arrays.fill(delta, 0);
			for (int i=0; i<dataLength-1; i++)
			{
				delta[i]=sampleData.get(k)[i] - sampleData.get(k)[i+1];
			}
			sampleDataDelta.add(delta);
		}
		showMatching(sampleDataDelta, globalSample);
		
		dataLength=sampleDataDelta.get(0).length;
		Vector<int[]> sampleData2ndDelta=new Vector<int[]>();
		for (int k=0; k<sampleData.size(); k++){
			int[] delta=new int[dataLength];
			Arrays.fill(delta, 0);
			for (int i=0; i<dataLength-1; i++)
			{
				delta[i]=sampleDataDelta.get(k)[i] - sampleDataDelta.get(k)[i+1];
			}
			sampleData2ndDelta.add(delta);
		}
		showMatching(sampleData2ndDelta, globalSample);
