...
public class PdfDistTSByMC {
	...
	static public PdfDistTSByMC getInstance(String fileName, String whichLine){
		if (fileName==null) return null;
		PdfDistTSByMC aReader=new PdfDistTSByMC();
...
*/
	...
	static void createDistribGrid(){
		...
		PdfDistTSByMC aJob=new PdfDistTSByMC();
		for (int i=0; i<4; i++){
...
PRNG is the value returned from the portable random number generator (PRNG). 
	...
	static double[] predictionByPdfAlphaMCFit(int[][] delayCurves, int maxV, int minV, int testPoints){
		...
		for (int s=0; s<5; s++){
			...
			double lowerBound=(minV - mu)/sigma;
	...
	static double[] predictionByVariationAlphaFit(int[][] delayCurves, String chartName){
		...
		for (int s=0; s<5; s++){
			...
			new PdfDistTSByMC().drawFitCurve(dataToPlot, plotStatements, chartName+s);
			dataToPlot.clear();
