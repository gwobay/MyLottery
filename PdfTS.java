...
public class PdfTS {
	...
	static public PdfTS getInstance(String fileName, String whichLine){
		if (fileName==null) return null;
		PdfTS aReader=new PdfTS();
...
PRNG is the value returned from the portable random number generator (PRNG). 
	...
	static int[][] buildHitBallDelayCountCurves(int from, int to){
		...
		for (int loc=from; loc<to; loc++){
			...
			for (int k=0; k<5; k++){
				int bHit=refData[loc][k];
				sortedTemp[k]=ballIdleTS[bHit][loc-1];
			}
			Arrays.sort(sortedTemp);
			for (int k=0; k<5; k++) retV[k][idx]=sortedTemp[k];
		}
	...
	static void predictByPdfAlphaMC(){
		...
		for (int iTest=testCaseCount; iTest>=0; iTest--){
			...
			double[] predicts=predictionByPdfAlphaMCFit(pdfCurves, 1);
	...
	static double[] monteCarlorNext10Point(double current1, double sigma, double upperBound, double lowerBound){
	...
	static double[] getAlphaPredictionsByMC(double[] alphaCurve, double sigma, double upperBound, double lowerBound){		
		...
		//for (int j=nPoints; j>0; j--){
			...
			//for (int s=0; s<iTry; s++){
				double[] newPath=monteCarlorNext10Point(current1, sigma, upperBound, lowerBound);
	...
	static double[] predictionByPdfAlphaMCFit(int[][] delayCurves, int testPoints){
		...
		for (int s=0; s<5; s++){
			...
			double[] nextV=getAlphaPredictionsByMC(alphaCurve, sigma, upperBound, lowerBound);//, testPoints);
	...
	static double[] predictionByVariationAlphaFit(int[][] delayCurves, String chartName){
		...
		for (int s=0; s<5; s++){
			...
			new PdfTS().drawFitCurve(dataToPlot, plotStatements, chartName+s);
			dataToPlot.clear();
	...
	public static void main(String[] args) {
		...
		for (int k=0; k<pdfHitCurves.length; k++){
			...
			new PdfTS().drawFitCurve(dataToPlot, plotStatements, "pdfCurves"+k);
			dataToPlot.clear();
			plotStatements.clear();
		}
		...
		for (int k=0; k<delayCurves.length; k++){
			...
			new PdfTS().drawFitCurve(dataToPlot, plotStatements, "delayCurves"+k);
			dataToPlot.clear();
			plotStatements.clear();
		}
