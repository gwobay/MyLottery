...
public class PdfTSByMC {
	...
	static public PdfTSByMC getInstance(String fileName, String whichLine){
		if (fileName==null) return null;
		PdfTSByMC aReader=new PdfTSByMC();
...
PRNG is the value returned from the portable random number generator (PRNG). 
	...
	static int[][] buildHitBallPdfCurves(int from, int to){
		...
		int[][] retV=new int[5][sampleCount];
		for (int k=0; k<5; k++ )
		...
		for (int loc=from; loc<to; loc++){
			...
			for (int k=0; k<5; k++){
				...
				sortedTemp[k]=ballSumTS[bHit][loc-1];//(int)Math.round(ballSumTS[bHit][j])-avg);
			}
	...
	static void predictByDelayAlphaMC(){
		...
		BufferedWriter aWriter=null;
		try {
			aWriter=new BufferedWriter(new OutputStreamWriter(new FileOutputStream("tsIdleAlphaMC"+prediction4WkDay+".txt")));
			int[] caseId={1};//, 2, 3, 4};
			int[][] hitCases=new int[caseId.length][6];
			int[][] lastHitCases=new int[caseId.length][6];
	...
	static double[][] buildStatisticsCurves(int[] delayTS){
	...
	static double[] monteCarloNext10Point(double current1, double mean, double sigma, int calculatedPoints, double upperBound, double lowerBound){
		double[] retV=new double[10+1];
		double new1=current1;
		double newMean=mean;
		double newSigma=sigma;
		int iC=calculatedPoints;
		for (int i=0; i<10; i++){
			retV[i]=Math.random();
			...
			double newV=(new1*newSigma+newMean);
			newMean=iC*newMean+newV;
			double tmp=newSigma*newSigma;
			tmp *= iC;
			iC++;
			newMean /= iC;
			tmp += (newV - newMean)*(newV - newMean);
			tmp /= iC;
			newSigma=Math.sqrt(tmp);						
		}
		retV[10]=new1;
		return retV;
	}
	static double[] getAlphaPredictionsByMC(double[] alphaCurve, double[] means, double[] sigmas, double upperBound, double lowerBound){		
		double current1=alphaCurve[alphaCurve.length-2];
		double error=100000;
		int iC=alphaCurve.length;
		...
		//for (int j=nPoints; j>0; j--){
			...
			error=100000;
			int iTry=1000;
			double errorMax=0.1/sigmas[0];
			while (error > errorMax && iTry < 10000){
			//for (int s=0; s<iTry; s++){
				double[] newPath=monteCarloNext10Point(current1, means[0], sigmas[0], iC-1, upperBound, lowerBound);
		...
		double newMean=means[1];
		double newSigma=sigmas[1];
		current1=alphaCurve[alphaCurve.length-1];
		for (int p=0; p<path2Keep.length; p++){
			double delta=path2Keep[p]-(current1-lowerBound)/(upperBound-lowerBound);
			delta *= newSigma;
			current1 += delta;
			if (current1 > upperBound)current1=upperBound;
			if (current1 < lowerBound) current1=lowerBound;
			double newV=(current1*newSigma+newMean);
			newMean=iC*newMean+newV;
			double tmp=newSigma*newSigma;
			tmp *= iC;
			iC++;
	...
	static double[] predictionByPdfAlphaMCFit(int[] delayCurves, int maxV, int minV, int testPoints){
		...
		double[] retV=new double[2];
		//for (int s=0; s<5; s++){
			int[] hitBallCountTS=delayCurves;
			//double minV=10000;
			//double maxV=-10000;
			//for (int x=0; x<delayTS.length; x++){
				//for (int ib=1; ib<40; ib++){
				//if (ballSumTS[ib][x] > maxV) maxV=ballSumTS[ib][x];
				//if (ballSumTS[ib][x] < minV) minV=ballSumTS[ib][x];
				//}
			//}
			//System.out.print("line "+s+" : ");
			double[][] alphaCurveMR=buildStatisticsCurves(hitBallCountTS);//buildAlphaCurve(hitBallCountTS);
			...
			double upperBound=(maxV+0.5 - mu)/sigma;
			double lowerBound=(minV-0.5 - mu)/sigma;
			...
			double[] nextV=getAlphaPredictionsByMC(alphaCurve, means, sigmas, upperBound, lowerBound);//, testPoints);
			//retV[s]=new double[nextV.length];
			//for (int k=0; k<nextV.length; k++)
			retV[0]= mu + nextV[0]*sigma;
			retV[1]=nextV[1];
		//}
		return retV;
	}
	...
	static double[] predictionByPdfAlphaMCFit(int[][] delayCurves, int maxV, int minV, int testPoints){
		...
		for (int s=0; s<5; s++){
			int[] hitBallCountTS=delayCurves[s];
			//double minV=10000;
			//double maxV=-10000;
			//for (int x=0; x<delayTS.length; x++){
				//for (int ib=1; ib<40; ib++){
	...
	static double[] predictionByVariationAlphaMCFit(int[][] delayCurves, int testPoints){
		...
		double[] retV=new double[5*2];
		for (int s=0; s<5; s++){
			int[] ballCountTS=delayCurves[s];
			double minV=10000;
			double maxV=-10000;
			for (int x=0; x<ballCountTS.length; x++){
				for (int ib=1; ib<40; ib++){
			...
			//System.out.print("line "+s+" : ");
			double[][] alphaCurveMR=buildStatisticsCurves(ballCountTS);//buildAlphaCurve(delayTS);
			double[] alphaCurve=Arrays.copyOfRange(alphaCurveMR[2], 0, alphaCurveMR[2].length);
			double mu=alphaCurveMR[0][alphaCurveMR[0].length-1];
			double sigma=alphaCurveMR[1][alphaCurveMR[1].length-1];
			double upperBound=(maxV - mu)/sigma;
			double lowerBound=(0 - mu)/sigma;
			double[] means=Arrays.copyOfRange(alphaCurveMR[0], alphaCurveMR[0].length-2, alphaCurveMR[0].length);
			double[] sigmas=Arrays.copyOfRange(alphaCurveMR[1], alphaCurveMR[1].length-2, alphaCurveMR[1].length);			
			double[] nextV=getAlphaPredictionsByMC(alphaCurve, means, sigmas, upperBound, lowerBound);//, testPoints);
			//retV[s]=new double[nextV.length];
			//for (int k=0; k<nextV.length; k++)
			retV[2*s]= mu	+ nextV[0]*sigma;
	...
	static double[] predictionByVariationAlphaFit(int[][] delayCurves, String chartName){
		...
		for (int s=0; s<5; s++){
			...
			new PdfTSByMC().drawFitCurve(dataToPlot, plotStatements, chartName+s);
			dataToPlot.clear();
	...
	static void predictByPdfAlphaMC(){
		if (allBallTS==null)
			allBallTS=getHistoryData(); //ascending
		...
		BufferedWriter aWriter=null;
		try {
			aWriter=new BufferedWriter(new OutputStreamWriter(new FileOutputStream("tsPdfAlphaMC"+prediction4WkDay+"-"+(tNow %1000)+".txt")));
			int[] caseId={1};//, 2, 3, 4};
			int[][] hitCases=new int[caseId.length][6];
			int[][] lastHitCases=new int[caseId.length][6];
		...
		for (int iTest=testCaseCount; iTest>=0; iTest--){
			...
			if (iTest > 0) draws=drawnBallDateAscending[tsPos];
			for (int s=0; s<5; s++) sDraws += draws[s]+", ";
			int[][] pdfCurves=buildHitBallPdfCurves(39*6, tsPos);
			double[] predicts=predictionByPdfAlphaMCFit(pdfCurves, maxD, minD, 1);
		...
		String myName="PDF Alpha curve fit";
		JackpotReader.showSummaryPage(predictionTS, hitCases, lastHitCases, localPerformanceList, myName, aWriter);
		/*
		int iTest=0; 
			int tsPos=iLen;
			...
			int[][] pdfCurves=buildHitBallPdfCurves(39*6, iLen);
			double[] predicts=predictionByPdfAlphaMCFit(pdfCurves, maxD, minD, 1);
...
*/
	...
	public static void main(String[] args) {
		...
		for (int k=0; k<pdfHitCurves.length; k++){
			...
			new PdfTSByMC().drawFitCurve(dataToPlot, plotStatements, "pdfCurves"+k);
			dataToPlot.clear();
			plotStatements.clear();
		}
		...
		for (int k=0; k<delayCurves.length; k++){
			...
			new PdfTSByMC().drawFitCurve(dataToPlot, plotStatements, "delayCurves"+k);
			dataToPlot.clear();
			plotStatements.clear();
		}
