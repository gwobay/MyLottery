...
public class PowerBallDataMathNewTest extends JFrame implements Runnable 
{
/**
...
static int getProject(int[] refD, int loc){
	int avgH=0;
	int avgL=0;
	float areaH=0;
	float areaL=0;
...
static int[] getAvgProject(int onRange, Vector<int[]> maxS, Vector<int[]> refD){
	//System.out.print("range"+onRange+" has avg projs (");
	int tt=0;
	for (int k=0; k<refD.size(); k++){ tt += maxS.get(k).length; }
	int[] retList=new int[tt];
	tt=-1;
	for (int k=0; k<refD.size(); k++){
		int[] loc=maxS.get(k);
		for (int i=0; i<loc.length; i++){
			retList[++tt]=getProject(refD.get(k), loc[i]);
			//System.out.print(""+retList[tt]+",");
		}
...
static int[]  getPrimeProject(int onRange, Vector<int[]> maxS, Vector<int[]> refD){
	Vector<int[]> prime1=getPrimeData(refD);
	int tt=0;
	for (int k=0; k<refD.size(); k++){ tt += maxS.get(k).length; }
	int[] retList=new int[tt];
	tt=-1;
	//System.out.print("range"+onRange+" has prime projs (");
	for (int k=0; k<refD.size(); k++){
		int[] loc=maxS.get(k);
		for (int i=0; i<loc.length; i++){
...
static int[]  getNextProject(int onRange, Vector<int[]> maxS, Vector<int[]> refD){
	int tt=0;
	for (int k=0; k<refD.size(); k++){ tt += maxS.get(k).length; }
	int[] retList=new int[tt];
	tt=-1;
	//System.out.print("range"+onRange+" has next projs (");
	for (int k=0; k<refD.size(); k++){
		int[] loc=maxS.get(k);
		for (int i=0; i<loc.length; i++){
...
public static void main(String[] args)
{
	...
	Vector<int[]> sampleData=jReaderOrder.readData(7, 39, 330);//jReaderDrop.readData(5, 39, 330);	//
		...
		int[] bonus=sampleData.remove(6);
		
		PowerBallDataMathNewTest aCharter=new PowerBallDataMathNewTest();
...
//---------------------------------------------------------------			
			...
			int[] nCount=new int[40];
			for (int p=0; p<24; p++) {
				String txt="<<<<<<["+p+"/"+((24-p)%6+1)+"]checking (";
				Vector<int[]> sampleDataP=new Vector<int[]>();
				for (int l=0; l<globalSample.size(); l++)
				{
					sampleDataP.add(Arrays.copyOfRange(globalSample.get(l), p+1, 172));
				...
				for (int i=9; i<20; i++){
				    ...
				    int[] projs=getNextProject(i, mxList, sampleDataP);
			    ...
			    for (int n=0; n<40; n++)
			    {
			    	...
			    	//System.out.print(","+n+"["+nCount[n]+"]");
			    }
			    //System.out.println(")*************");
			    for (int n=0; n<40; n++)
			    {
			    	if (nCount[n]<7) continue;
			    	System.out.print(","+n+"["+nCount[n]+"]");
			    }
			    System.out.println(")");
			}
			...
			for (int i=9; i<20; i++){
			    ...
			    int[] projs=getNextProject(i, mxList, sampleData);
			    for (int n=0; n<projs.length; n++){
			    	if (projs[n]>0 && projs[n] <40)
				    	nCount[projs[n]]++;
			    }
