...
public class ScanUsingExpandedData extends JFrame implements Runnable 
{
/**
...
static void evaluateAutoCorr(Vector<int[]> refData){
	...
	for (int i=iLen-78*3; i>-1; i--)
	{
		...
		for (int ip=1; ip<40; ip++){
			...
			if (allPredictList[ip] > 0) found_count++;
			text += (", "+ip+"["+preds[ip]+"],");
			if (preds[ip] > iBest){
				iBest = preds[ip];
				iBet=ip;
...
int[] tryTriangleSample(Vector<int[]> oldSample, int checkCount1, int expandF){
	...
	// int selected=((int)sqz % 2);
	 ...
	 for (int s=0; s<5; s++){
		 ...
		 double adj=0;
		 int iCC=0;
		 for (int v=checkLen-9; v<checkLen; v++){
		...
		// rdt=-0.2;
		 ...
		 for (int n4=1+s; n4<36+s; n4 += 36){//n4++){
			 ...
			 */
			 //adj *= 2;
			 double[] diffS=new double[checkCount1];
			 for (int ix=0; ix<checkCount1; ix++){
...
static boolean getStatisticsFromYesterdayFile(BufferedWriter aWriter, String lineType, 
		int[][] hitCount, int[][]lastHitLocation, int[][][]localPerformanceList){
	...
	try {
		...
		while(	(aLine=mReader.readLine()) != null){
			...
			if (i0 >0) {
				...
				i9=aLine.indexOf(")");
				currentTestCaseNo=Integer.parseInt(aLine.substring(i0+4, i9));
				currentSubSetNo=0;
				if (currentTestCaseNo<testCaseCount){
					aWriter.write("=======("+(currentTestCaseNo+1)+")===");aWriter.newLine();
				}
				continue;
			}
			...
			if (i0 >0) {
				...
				if (newCaseNo > testCaseCount) continue;
				int iFound=Integer.parseInt(aLine.substring(i0-1, i0));
				hitCount[currentSubSetNo][iFound]++;
				//int loc=iCheckEnd-iCheckStart;
				lastHitLocation[currentSubSetNo][iFound]=newCaseNo;	
				localPerformanceList[currentSubSetNo][iFound][testCaseCount-newCaseNo]=1;
			...
			if (i0 >0) {				
					if (currentTestCaseNo<testCaseCount){
					aWriter.write("Line ("+(currentTestCaseNo+1)+aLine.substring(i0));aWriter.newLine();
				}
				continue;
			}
			if (currentTestCaseNo<testCaseCount)
...
public static void main(String[] args)
{
	...
	Vector<int[]> sampleData=jReaderOrder.readData(5, 39, 512);	
		...
		ScanUsingExpandedData aJob=new ScanUsingExpandedData();
		...
		int[] ccVal={3,6};//4, 5, 6};//2, 3, 4, 5, 6};//,7,8,9};
		int[] freqs={6,5,4,8};//2, 3, 4, 5, 6};
		BufferedWriter aWriter=null;
		String fileName="expand"+lineType+JackpotReader.todayDrawID+"Scan.txt";//drawDate+"Scan.txt";
		try {
			aWriter=new BufferedWriter(new OutputStreamWriter(new FileOutputStream(fileName)));	
			System.out.println(fileName);
			//double avgCandidates=0;
			int[][] hitCount=new int[freqs.length][6];			
			int[][] lastHitLocation=new int[freqs.length][6];
			int[][][] localPerformanceList=new int[freqs.length][6][testCaseCount];
			
			String[] foundList=new String[freqs.length];
			for (int i=0; i<freqs.length; i++){
				Arrays.fill(hitCount[i], 0);
			...
			//boolean ok=getStatisticsFromYesterdayFile(aWriter,lineType, hitCount, lastHitLocation, localPerformanceList);
			int startJobCount=1;
			//if (!ok){
				//System.out.println("No previous data file");
				//System.exit(0);
				startJobCount=testCaseCount;
			//}
			int[][][] predictionTS=new int[freqs.length][testCaseCount+1][];
			int cTryCase=0;
			iCheckStart=iCheckEnd-testCaseCount;
			int iTestLen=512-testCaseCount;
		//while (iCheckStart <= iCheckEnd){
		for (int iTest=testCaseCount; iTest>=0; iTest--){
			int iFound=0;
			int testTarget=iLen1 - iTest;
			aWriter.write("===================("+iTest+") ================");
			aWriter.newLine();
			Vector<int[]> testData=new Vector<int[]>();
		...
		for (int p=0; p<5; p++){
			testTxt += ""+targets[p]+",";
			if (allPredictList[targets[p]]>0){
				iFound++;
				foundTxt += ""+targets[p]+"["+allPredictList[targets[p]]+"], ";
			}
		}
		aWriter.write(testTxt+">>>");
		aWriter.newLine();
		foundList[iFq]="Predic:(";
		int iPred=0;
		for (int p=0; p<40; p++){
			if (allPredictList[p]>0) {
				iPred++;
				foundList[iFq] += " "+p+",";
			}
		...
		int pCount=0;
		for (int k=0; k<foundList.length; k++)
			if (allPredictList[k]==1) 
				{
					//sPred += ""+k+", ";
				allPredictList[pCount++]=k;
				}
		int[] pList=Arrays.copyOf(allPredictList, pCount);
		predictionTS[iFq][testCaseCount-iTest]=pList;
		//iFits[iFound]++;
		aWriter.write(foundTxt+") "+iFound+" out of "+iPred);
		//avgCandidates += iPred;
		aWriter.newLine();
		//aWriter.write(hasPredic[0]+" and "+hasPredic[1]+" and "+hasPredic[2]+" and "+hasPredic[3]);
		//aWriter.newLine();
		
		aWriter.write(foundList[iFq]+") ");
		aWriter.newLine();
		...
		if (iTest>0)
			localPerformanceList[iFq][iFound][testCaseCount-iTest]=1;
		}
	}
		JackpotReader.showSummaryPage(predictionTS, hitCount, lastHitLocation, localPerformanceList, myName, aWriter);
			
	aWriter.write("Last date: "+JackpotReader.drawDate+" on "+iLen1+" data");aWriter.newLine();
	aWriter.close();
		}catch (IOException e){}
	}
/*
