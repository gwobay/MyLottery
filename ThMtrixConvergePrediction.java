...
public class ThMtrixConvergePrediction extends JFrame implements Runnable 
{
/**
	
	 */
	static String programName="ScanUsingExpandedData";
...
double fineTuneTrianglePrediction(int s, int[] oldSample, double variableRatio, int expandFactor, int sincRange, int[] pWas, double vWas){
	...
	int checkLen=35;
		...
		// rdt=-0.2;
		 ...
		 //aWriter.newLine();
		 //if (diff2 > 0.45) return 0;
		 if (diff2 > vWas) {
			 //pWas;
			 return vWas;
		 }
		 else pWas[0]=diffAt[0];
		 return diff2;	
}

static double[] merit_of_solutions=new double[5];

int[] predictByFitFocus(Vector<int[]> oldSample, int checkCount1, int expandF, int[] draws){
	//int byFactor=32;
	...
	// int selected=((int)sqz % 2);
	 ...
	 for (int s=0; s<5; s++){
		 ...
		 for (int im1=0; im1<3; im1++){
			 ...
			 double[] pointData=new double[im];
			 double[] sin=new double[im];
				for (int row=0; row<im; row++){
					pointData[row]=oldSample.get(s)[iLen-im+row];
					double x=(row+1)/expandF;
					
					for (int y=0; y<im; y++){
						sin[y]=Math.sin((y+1)*pI*x+pI/4);
					}
					mtx.add(sin);
				}
				MyMatrixFit t=new MyMatrixFit(mtx);
				MyMatrixFit iv=MyMatrixFit.inverseLU(t);
				if (iv == null) proj[im1]=0;
				else {
					double[] coeff=iv.multiply(pointData);
					for (int y=0; y<im; y++){
						sin[y]=Math.sin((y+1)*pI*(im+1)+pI/4);
						proj[im1] += coeff[y]*sin[y];
					}					
				}
				bestFitPoints[s] += proj[im1];
		 }
		  bestFitPoints[s] /= proj.length;
		  double var=0;
		  for (int i=0; i<3; i++){
			  var += (proj[i] - bestFitPoints[s])*(proj[i] - bestFitPoints[s]);
		  }
		  var = Math.sqrt(var);
		  var /= 3;		 
		 merit_of_solutions[s]=var;
	 }
		
	 return bestFitPoints;//retV;
}

int[] tryTriangleSample(Vector<int[]> oldSample, int checkCount1, int expandF, int[] draws){
	//int byFactor=32;
	//Vector<double[]> sampled=oldSample;//expandSeries(byFactor,oldSample);
	//int iLen=sampled.get(0).length;
	//double pi=Math.PI;
	...
	// int selected=((int)sqz % 2);
	 ...
	 for (int s=0; s<5; s++){
		 ...
		 int expandFactor=expandF;//8;
		 double[] testData1=expandSeries(expandFactor,newData); 
		 double adj=0;
		 int iCC=0;
		 double[] adjs=new double[3];
		 for (int v=1; v<4; v++){//checkLen-9; v<checkLen; v++){
			 int pos=(checkLen-v)*expandFactor;
			 double tmp=convolBySinc(pos, 54, testData1);
			 adjs[v-1] = (tmp/testData1[pos]);
			 adj += adjs[v-1];
			 iCC++;
		 }
		 adj /= iCC;
		...
		// rdt=-0.2;
		 ...
		 int bestFactor=0;
		 int bestRange=0;
		 expandFactor = expandF;
		 boolean found=false;
		 while (!found && diff2>0.1 && expandFactor<expandF+3)
		 { 
		 ...
		 //for (int n4=1+s0; n4<36+s9; n4 += 36){//n4++){
		for (int m4=0; m4<39; m4+=50){//+){//n4<36+s9; n4 += 36){//n4++){
			 double n4=(draws[s]+m4) % 39;
			 if (n4 == 0) n4=39;
			 newData[checkLen+3]=draws[s];//n4;			 
			 for (int m3=0; m3<39*2; m3++){//for (int n3=1+s0; n3<36+s9; n3++){
				 if (found)break;
				 double n3=(draws[s]+m3*0.5);
				 if (n3 > 39) n3 -= 39;
				 newData[checkLen+2]=n3;				 
				 for (int m2=0; m2<39*2; m2++){
					 if (found)break;
					 double n2=(draws[s]+m2*0.5);
					 if (n2 > 39) n2 -= 39;
					 newData[checkLen+1]=n2;					 
					 for (int m1=0; m1<39*2; m1++){//for (int n1=1+s0; n1<36+s9; n1++){
						 if (found)break;
						 double n1=(draws[s]+m1*0.5);
						 if (n1 > 39) n1 -= 39;
						 newData[checkLen]=n1;
			 ...
			 */
			 ...
			 for (int ix=1; ix<4; ix++){
				 //int i=ix+36-checkCount1;
				 //int 
				 int pos=(checkLen+2-ix)*expandFactor;
				 double conv=0;
				 
				 conv =convolBySinc(pos, sincRange, testData)/adjs[ix-1];
				 //newData[i]=(int)Math.round(conv);
				 diffS[ix-1]=Math.abs(conv-testData[pos]);
				 //diffS[0]=Math.abs(conv-testData[pos]);
				 //double ratio=Math.abs(conv)/sampled.get(s)[i];
			 }
		...
		expandFactor += 1;	  
		 ...
		 solution[0]=diffAt[0];
		// if (diff2 > 0.1) diff2=fineTuneTrianglePrediction(s, oldSample.get(s), bestRatio, bestFactor, bestRange, solution, diff2);
		//bestFitPoints[s]=0;
		//if (diff2 < 0.11 || diffAt[0]!=0)
		 merit_of_solutions[s]=diff2;
		 bestFitPoints[s]=solution[0];//;
	 }
...
static boolean getStatisticsFromYesterdayFile(BufferedWriter testLog, String lineType, 
		int[][] iStatistics, int[][]lastHitLocation, int[][][]localPerformanceList){
	String fileName="expand"+lineType+JackpotReader.yesterdayDrawID+"Scan.txt";//drawDate+"Scan.txt";
	BufferedReader mReader;
	try {
		mReader=new BufferedReader(new InputStreamReader(new FileInputStream(fileName),  "utf-8"));						
		String aLine=null;
		int currentTestCaseNo=9999;
		int currentSubSetNo=-1;
		int i0=-1;
		int i9=-1;
		while(	(aLine=mReader.readLine()) != null){
			i0=aLine.indexOf("====(");
			if (i0 >0) {
				if (currentTestCaseNo==1) break;				
				i9=aLine.indexOf(")");
				currentTestCaseNo=Integer.parseInt(aLine.substring(i0+5, i9));
				currentSubSetNo=0;
				if (currentTestCaseNo<234){
					testLog.write("====("+(currentTestCaseNo+1)+")===");testLog.newLine();
				}
				continue;
			}
			i0=aLine.indexOf(" out of");
			if (i0 >0) {
				int newCaseNo=currentTestCaseNo+1;
				if (newCaseNo > 6*39) continue;
				int iFound=Integer.parseInt(aLine.substring(i0-1, i0));
				iStatistics[currentSubSetNo][iFound]++;
				//int loc=iCheckEnd-iCheckStart;
				lastHitLocation[currentSubSetNo][iFound]=newCaseNo;	
				localPerformanceList[currentSubSetNo][iFound][6*39-newCaseNo]=1;
				testLog.write(aLine);testLog.newLine();
				currentSubSetNo++;
				continue;
			}
			i0=aLine.indexOf(") <<");
			if (i0 >0) {				
					if (currentTestCaseNo<234){
					testLog.write("Line ("+(currentTestCaseNo+1)+aLine.substring(i0));testLog.newLine();
				}
				continue;
			}
			if (currentTestCaseNo<234)
...
static int dataDrawType=0;
static String myName="";
public static void main(String[] args)
{
	getSincData();
	String[] criteria={"開出順序", "大小順序"};
	String[] drawType={"drop", "order"};
	
	int[] setTypes={0, 1};
	double[] dd={1, 1.5};//2,3};//, 0.3, 0.5, 1, 1.5};
	for (int iq=1; iq>=0; iq--){
	...
	Vector<int[]> sampleData=jReaderOrder.readData(5, 39, 612);	
		globalSample=sampleData;
		int sampleLength=sampleData.get(0).length;
		while (sampleData.get(0)[sampleLength-1]==0) sampleLength--;
		int[] testSeq=new int[sampleLength];
		Vector<int[]> dateAscend=new Vector<int[]>();
		for (int s=0; s<5; s++){
			for (int i=0; i<sampleLength; i++){
				testSeq[i]=
						sampleData.get(s)[sampleLength-1-i];
			}
			dateAscend.add(Arrays.copyOf(testSeq, sampleLength));
		}
		ThMtrixConvergePrediction aJob=new ThMtrixConvergePrediction();
		myName=aJob.getClass().getName();
		int iCheckStart=sampleLength-300;
		int iCheckEnd=sampleLength;
		int iFitCount=0;
		int iSuccess=0;
		int[] iFits={0, 0, 0, 0, 0, 0};
		int[] ccVal={1};//3,6};//4, 5, 6};//2, 3, 4, 5, 6};//,7,8,9};
		int[] freqs={3, 6, 9};//,4,8};//2, 3, 4, 5, 6};
		BufferedWriter testLog=null;
		String fileName="convergeFitLineTh"+lineType+JackpotReader.todayDrawID+".txt";//drawDate+"Scan.txt";
		try {
			testLog=new BufferedWriter(new OutputStreamWriter(new FileOutputStream(fileName)));	
			System.out.println(fileName);
			//double avgCandidates=0;
			int[][] iStatistics=new int[freqs.length][6];			
			int[][] lastHitLocation=new int[freqs.length][6];
			int[][][] localPerformanceList=new int[freqs.length][6][6*39];
			
			String[] foundList=new String[freqs.length];
			for (int i=0; i<freqs.length; i++){
				Arrays.fill(iStatistics[i], 0);
				Arrays.fill(lastHitLocation[i], 0);
				for (int x=0; x<6;x++)
					Arrays.fill(localPerformanceList[i][x], 0);					
			}
			/*
			boolean ok=getStatisticsFromYesterdayFile(testLog,lineType, iStatistics, lastHitLocation, localPerformanceList);
			if (!ok){
			...
			*/
			int cTryCase=0;
			iCheckStart=iCheckEnd-6*39;
			int iTestLen=sampleLength-6*39;
			int[][][] solDiff=new int[freqs.length][5][6*39];
		//while (iCheckStart <= iCheckEnd){
		for (int iTest=234; iTest>=0; iTest--){
			long tStarted=System.currentTimeMillis();
			int iFound=0;
			int testTarget=sampleLength - iTest;
			testLog.write("===================("+iTest+") ================");
			testLog.newLine();
			Vector<int[]> testData=new Vector<int[]>();
			...
			{
				testData.add(Arrays.copyOfRange(dateAscend.get(s), 
						testTarget-(sampleLength-6*39-1), testTarget));
						//0, iCheckStart));
				if (iTest ==0) targets[s]=dateAscend.get(s)[testTarget-1];
				else targets[s]=dateAscend.get(s)[testTarget];
				for (int d=1; d<2; d++)
				initValues[s] += dateAscend.get(s)[testTarget-d];
				initValues[s] /= 1;
			}
			Thread[][] useThread=new Thread[freqs.length][ccVal.length];
			int[][][] predictionsTh=new int[freqs.length][ccVal.length][];
			int[] found=new int[40];
			for (int iFq=0; iFq<freqs.length; iFq++){
				//sqz=dd[i];
				
				int filterBy=freqs[iFq];//yFactor[i];
			...
			//int minCheckCount=3;
				...
				//Thread[] useThread=new Thread[ccVal.length];
			for (int cc=0; cc<ccVal.length; cc++){
				//minCheckCount; cc<minCheckCount+1; cc++){
			
				int checkCount1=ccVal[cc];//cc*minCheckCount;
			...
			//System.out.println("Working on "+lineType);
				...
				//for (int i=1; i<dd.length; i++){
					//sqz=dd[i];
				final int idx=cc;
				final int iCase=iTest;
				final int ifq=iFq;
				useThread[iFq][cc]=new Thread(new Runnable(){
					public void run(){
						
					
						
					predictionsTh[ifq][idx]=//refineFound(dateAscend, "filterN"+(sqz*10)+lineType);//filterOutSample(dateAscend);;//
						aJob.tryTriangleSample(testData, checkCount1, filterBy, initValues);//dateAscend, "filterA"+checkCount);//filterOutSample(dateAscend);;//
				...
				//hasPredic += predictions.toString();
					//System.out.println("Finish "+iCase+" use "+filterBy+" filter with check last "+checkCount1+" for "+
			                 // (System.currentTimeMillis() - tStarted)+" milli secs");

				}
				}
				);
			
				useThread[iFq][cc].start();
				try {
					useThread[iFq][cc].join();
			...
			for (int iFq=0; iFq<freqs.length; iFq++){
				...
				for (int cc=0; cc<ccVal.length; cc++){
					...
					for (int p=0; p<predictionsTh[iFq][cc].length; p++){
						if (predictionsTh[iFq][cc][p] < 1 || predictionsTh[iFq][cc][p] > 39) continue;
						found[predictionsTh[iFq][cc][p]]=1;
						//hasPredic[cc-1] += ""+predictions[p]+",";
					predics += (""+predictionsTh[iFq][cc][p]+", ");//"["+dF.format(merit_of_solutions[p])+"], ");
					}
					testLog.write(predics+") ");
					testLog.newLine();
				}
				for (int ss=0; ss<5; ss++){
					if (iTest > 0)
					solDiff[iFq][ss][234-iTest]=targets[ss]-predictionsTh[iFq][0][ss];
				}
		...
		testLog.write(foundList[iFq]+") ");
		testLog.newLine();
		
		if (iFound > 0) iSuccess++;
		iStatistics[iFq][iFound]++;
		//int loc=iCheckEnd-iCheckStart;
		if (iTest >0)
		lastHitLocation[iFq][iFound]=iTest;	
		if (iTest>0)
			localPerformanceList[iFq][iFound][6*39-iTest]=1;
		}
		System.out.println("Finish "+iTest+" using "+(System.currentTimeMillis() - tStarted)+" milli secs");
	}
		testLog.write("try "+6*39+" cases and got ");testLog.newLine();
		
		BufferedWriter predictBase=new BufferedWriter(new OutputStreamWriter(new FileOutputStream("toPredictBase.txt", true)));						
		
		for (int iFq=0; iFq<freqs.length; iFq++){
			testLog.write("Freq="+freqs[iFq]+"===== "+myName+" ===========================");
			testLog.newLine();
			testLog.write(foundList[iFq]+") ");
			testLog.newLine();
			predictBase.write("Freq="+freqs[iFq]+"===== "+myName+" ===========================");
			predictBase.newLine();
			predictBase.write(foundList[iFq]+") ");
			predictBase.newLine();
			JackpotReader.performanceList=localPerformanceList[iFq];
			...
			for (int s=0; s<6; s++){
				...
				int iLL=localPerformanceList[iFq][s].length-10;
				for (int ia=0; ia<10; ia++){
					showOnOffSeq += ""+localPerformanceList[iFq][s][iLL+ia];
				}
				predictBase.write(showOnOffSeq+"--- ");
				testLog.write(showOnOffSeq+"--- ");
				testLog.write("Hits "+s+" : ["+iStatistics[iFq][s]+"]@"+lastHitLocation[iFq][s]);
				testLog.write("   	??"+dF.format(iStatistics[iFq][s]*nextCasePdf[s]/234));
				
				predictBase.write("Hits "+s+" : ["+iStatistics[iFq][s]+"]@"+lastHitLocation[iFq][s]);
				predictBase.write("   	??"+dF.format(iStatistics[iFq][s]*nextCasePdf[s]/234));
				//int days=hitCounts[s]*lastHitLocation[s];
				if (caseMeanVar[s] > 0 && lastHitLocation[iFq][s] > caseMeanVar[s]) {
					double ovdToVar=(lastHitLocation[iFq][s] - caseMeanVar[s])/caseMeanVar[s+6];
					testLog.write(" OVD by "+dF.format(ovdToVar));
			...
			for (int s=0; s<5; s++)
			for (int i=0; i<234; i++){
				testLog.write(solDiff[iFq][s][i]+",");
				if ((i+1) % 20==0) testLog.newLine();
			}
			testLog.newLine();
			predictBase.newLine();
		}
	...
	testLog.write("Last date: "+JackpotReader.drawDate+" on "+sampleLength+" data");testLog.newLine();
