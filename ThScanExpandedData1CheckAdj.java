...
public class ThScanExpandedData1CheckAdj extends JFrame implements Runnable 
{
/**
...
double convolBySinc(int n, int range1, double[] newData){
	//f(n-x)g(x); let sinc be g and newData be f
	//case 1 : n > data length: sum over x=data_lenght to x=0
	//case 2 : n =0 to n < data length; sum 2 parts one positive and one negative
	//case 3 : n <0 ; 0
	// divided into 2 parts; one from n-x=0 (i.e., i=x=n) to n-x < data_length (i=x>n-data_length)
	// the second from n-x = -1 to n-x 
	int range=54;
	int uBnd=newData.length;
	int multiplyBnd=sinc40PI.length-1;
	double retV=0;
	double forward=0;
	if (n >= uBnd-1){
		for (int i=1; i<range && i <= uBnd; i++){
			//if (n-i==newData.length) continue;
			if (n+i > multiplyBnd) break;
			forward += newData[uBnd-i]*sinc40PI[n-uBnd+i];//sinc[0]=0, sinc gives the integral
		}
	}
	else if (n > 0){
		...
		for (int i=1; i< uBnd-n && i < range; i++){
			try {
			forward += newData[n + i]*sinc40PI[i];
			} catch (ArrayIndexOutOfBoundsException e){
				System.out.println("n+i="+(n+i)+", i="+i);
			}
		}
	}
...
int[] tryTriangleSample(Vector<int[]> oldSample, int checkCount1, int expandF){
	//int byFactor=32;
	...
	// int selected=((int)sqz % 2);
	 ...
	 checkLen=checkCount1+54/expandF+1; 
	 for (int s=0; s<5; s++){
		 //aWriter.write("****************Checking line "+s);aWriter.newLine();
		 //String good1="try line "+s+" fits at ";
		 //ratio=getMeanRatio(Arrays.copyOfRange(sampled.get(s), iLen-checkLen, iLen));
		 ...
		 double adj=0;
		 int iCC=0;
		 //for (int v=1; v<2; v++){//checkLen-9; v<checkLen; v++){
			 int pos=(checkLen-2)*expandFactor;
			 double tmp=convolBySinc(pos, 54, testData1);
			 adj += (tmp/testData1[pos]);
			 iCC++;
		// }
		 adj /= iCC;
		 double diff2=200000;
		...
		// rdt=-0.2;
		 ...
		 int bestRange=0;
		 expandFactor = 1;
		 while (diff2>0.1 && expandFactor<7)
		 { 
			 expandFactor += 1;
		 ...
		 for (int n4=1+s; n4<36+s; n4 += 36){//n4++){
			 ...
			 */
			 //adj *= 2;
			 double[] diffS=new double[1];//checkCount1];
			 for (int ix=0; ix<1; ix++){//checkCount1; ix++){
				 int i=checkLen-2;//ix+36-checkCount1;
				 pos=i*expandFactor;
				 double conv=0;
		 ...
		 double tDiff=0;
		 for (int c=0; c<1; c++)//{//checkCount1; c++)
		 {
		 	tDiff += diffS[c];
		 }
		 //tDiff /= checkCount1;
...
static void buildAdjRef(Vector<int[]> dateAscendingSampleData){
	// from data older than 6*39*2=468
...
public static void main(String[] args)
{
	...
	Vector<int[]> sampleData=jReaderOrder.readData(5, 39, 612);	
		...
		ThScanExpandedData1CheckAdj aJob=new ThScanExpandedData1CheckAdj();
		...
		int[] freqs={6,5,4,8};//2, 3, 4, 5, 6};
		BufferedWriter testLog=null;
		String fileName="expandTh"+ccVal[0]+lineType+JackpotReader.todayDrawID+"Scan.txt";//drawDate+"Scan.txt";
		try {
		...
		for (int iTest=234; iTest>=0; iTest--){
			...
			for (int s=0; s<dateAscend.size(); s++)
			{
				testData.add(Arrays.copyOfRange(dateAscend.get(s), 
						...
						//0, iCheckStart));
				if (iTest >0)
				targets[s]=dateAscend.get(s)[testTarget];
			}
			...
			//System.out.println("Working on "+lineType);
				...
				useThread[iFq][cc]=new Thread(new Runnable(){
					...
					predictionsTh[ifq][idx]=//refineFound(dateAscend, "filterN"+(sqz*10)+lineType);//filterOutSample(dateAscend);;//
						aJob.tryTriangleSample(testData, checkCount1, filterBy);//dateAscend, "filterA"+checkCount);//filterOutSample(dateAscend);;//
