...
public class TryPAMFilter extends JFrame implements Runnable 
{
/**
...
static double getPAMSincConvolution(int tau, int range, double[] inData){
	double retV=0;
	double PI=Math.PI;
	int uBnd=inData.length-1;
	double tmp=0;
	for (int i=0; i<range; i++){
...
public static void main(String[] args)
{
	...
	Vector<int[]> sampleData=jReaderOrder.readData(5, 39, 390);	
		...
		TryPAMFilter aCharter=new TryPAMFilter();
		/*
		Vector<int[]> testData1=new Vector<int[]>();
		int[] targets1=new int[5];
		Arrays.fill(target1s, 0);
		int[] found1=new int[40];
		Arrays.fill(foun1d, 0);
		for (int s=0; s<dateAscend.size(); s++)
		{
			testData1.add(Arrays.copyOfRange(dateAscend.get(s), 0, iLen));
			//if (iCheckStart < iLen1)
			//targets[s]=dateAscend.get(s)[0];				
		}
		for (int cc=2; cc<4; cc++){
			
			checkCount=cc;
		int[] predictionN=aCharter.tryPAMExpandedSeries(data1, "tryPAM");
		
		System.out.print("Data fits "+cc+" for tomorrow:(");
		for (int i=0; i<predictionN.length; i++){
			System.out.print(""+predictionN[i]+", ");
		}
		System.out.println(")");
		}
		*/
		...
		while (iCheckStart <= iCheckEnd){
			...
			//System.out.println("Working on "+lineType);
				...
				//for (int i=1; i<dd.length; i++){
					//sqz=dd[i];
					long tNow=System.currentTimeMillis();
					int[] predictions=aCharter.tryPAMExpandedSeries(testData, "tryPAM");
							//refineFound(dateAscend, "filterN"+(sqz*10)+lineType);//filterOutSample(dateAscend);;//
						//aCharter.tryTriangleSample(testData);//dateAscend, "filterA"+checkCount);//filterOutSample(dateAscend);;//
