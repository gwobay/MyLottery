...
public class TryPAMFilterNew extends JFrame implements Runnable 
{
/**
...
int fineTuneTrianglePrediction(int s, int[] oldSample, double variableRatio, 
		int expandFactor, int sincRange, int pWas, double vWas){
	int iLen=oldSample.length;
	int checkLen=35;
		 double[] newData=new double[37];
			Arrays.fill(newData, 0);
		 for (int i=0; i<checkLen; i++){
	...
	// int selected=((int)sqz % 2);
	 ...
	 for (int s=0; s<5; s++){
		...
		// rdt=-0.2;
		 ...
		 if (diff2 > 0.001) diffAt[0]=fineTuneTrianglePrediction(s, oldSample.get(s), 
				 bestRatio, bestFactor, bestRange, diffAt[0], diff2);
		 bestFitPoints[s]=diffAt[0];
	 }
...
double getPAMSincConvolution(int tau, int range, double[] inData){
	double retV=0;
	double PI=Math.PI;
	final int uBnd=inData.length-1;
	synchronized(this){
	double tmp=0;
	for (int i=0; i<range; i++){
		if (tau+i+1 > uBnd) break;
		int sgn=1;
		if (i % 2==1) sgn=-1;
		tmp=2/PI;
		tmp *= (inData[tau+i+1]-inData[tau+i])*sgn;
		tmp += ((i+1)*inData[tau+i]-i*inData[tau+i+1])*sinc40PI[i+1];
		retV += tmp;
	}
	...
	for (int i=1; i<=range; i++){
		...
		tmp=2/PI;
		tmp *= (inData[tau-i+1]-inData[tau-i])*sgn;
		tmp += ((1-i)*inData[tau-i]+i*inData[tau-i+1])*sinc40PI[i];
		retV += tmp;
	}
	}
	return retV;
...
double getAdjustNorm(int n, int range, double[] newData){
	//mx+b
	//m=(f[n+1]-f[n])
	//b=(n+1)*f(n)-n*f(n+1)
	
	return getPAMSincConvolution(n, range, newData);
}

int fineTunePAMPrediction(int s, int[] oldSample, double variableRatio, 
		int expandFactor, int sincRange, int pWas, double vWas){
	int iLen=oldSample.length;
	int checkLen=35;
		...
		// rdt=-0.2;
		 ...
		 for (int n4=1+s; n4<36+s; n4 += 36){//n4++){
			 ...
			 for (int ix=0; ix<checkCount; ix++){
				 ...
				 conv =getPAMSincConvolution(pos, sincRange, testData)/adj;
				 //newData[i]=(int)Math.round(conv);
				 diffS[ix]=Math.abs(conv-testData[pos]);
				 //double ratio=Math.abs(conv)/sampled.get(s)[i];
			 }
...
int[] tryPAMExpandedSeries(Vector<int[]> oldSample){
	int byFactor=32;
	//Vector<double[]> sampled=oldSample;//expandSeries(byFactor,oldSample);
	//int iLen=sampled.get(0).length;
	//double pi=Math.PI;
	...
	// int orgCheckLen=36;
	 ...
	 for (int s=0; s<5; s++){
		...
		// rdt=-0.2;
		 ...
		 int bestFactor=0;
		 int bestRange=0;
		 while (diff2>0.02 && expandFactor<17)
		 { 
		 ...
		 for (int n4=1+s; n4<36+s; n4 += 36){//n4++){
			 ...
			 double adj=0;
			 int iCC=0;
			 for (int v=testData.length/32; v<testData.length*31/32; v++){
				 double tmp=getAdjustNorm(v, sincRange, testData);
				 adj += (tmp/testData[v]);
				 iCC++;
			 }
			 adj /= iCC;
		 ...
		 if (tDiff < diff2){
			 ...
			 bestRange=sincRange;
				 ...
				 //System.out.println
				 //("Line "+s+" f="+expandFactor+" diff="
						 		//+dF.format(diff2)+" bf="+bestFactor+" range="+bestRange+ " r="+dF.format(bestRatio)+" @ "+diffAt[0]+","+diffAt[1]);	 
		 expandFactor++;	  
		 }
		 ...
		 if (diff2 > 0.1) diffAt[0]=fineTunePAMPrediction(s, oldSample.get(s), 
				 bestRatio, bestFactor, bestRange, diffAt[0], diff2);
		 bestFitPoints[s]=diffAt[0];
	 }
	 return bestFitPoints;//retV;
...
int[] tryPAMExpandedSeries(Vector<int[]> oldSample, String fname){
	int byFactor=32;
	//Vector<double[]> sampled=oldSample;//expandSeries(byFactor,oldSample);
	//int iLen=sampled.get(0).length;
	//double pi=Math.PI;
	...
	// int selected=((int)sqz % 2);
	 BufferedWriter aWriter;
	 String fileName="tryPAMExpandLog"+JackpotReader.drawDate+".txt";
	 try {
			aWriter=new BufferedWriter(new OutputStreamWriter(new FileOutputStream(fileName)));	
...
Vector<int[]> testBaseData;
int testStartIndex;
int testLastIndex;
static DecimalFormat dI=new DecimalFormat("0000");
public void myTask(){
	System.out.println("I started with "+testStartIndex+"->"+testLastIndex);
	int iLen1=testBaseData.get(0).length;
	for (int idx=testStartIndex; idx<=testLastIndex; idx++){
		Vector<int[]> testData=new Vector<int[]>();
		int[] targets=new int[5];
		Arrays.fill(targets,  0);;
		String retS="("+dI.format(idx)+")<<";
		for (int s=0; s<testBaseData.size(); s++)
		{
			testData.add(Arrays.copyOfRange(testBaseData.get(s), idx-60, idx));
			if (idx < iLen1)
			targets[s]=testBaseData.get(s)[idx];
			
			retS += ""+targets[s]+", ";
		}
		retS += ") has predictions (";
		String[] hasPredic={"fit 2pts:", "fit 3pts:"};
		for (int cc=2; cc<4; cc++){	
			checkCount=cc;
		//System.out.println("Working on "+lineType);
			
			long tNow=System.currentTimeMillis();
			int[] predictions=tryPAMExpandedSeries(testData);
					
			//System.out.println("Finish low pass try with check last "+checkCount+" for "+
		                  //(System.currentTimeMillis() - tNow)+" milli secs");
			//String predics="get predictions:(";
			for (int p=0; p<predictions.length; p++){
				//found[predictions[p]]++;
				hasPredic[cc-2] += (""+predictions[p]+", ");
			}			
		}
		retS += hasPredic[0]+" and "+hasPredic[1];
		try {
			allTestResult.put(retS);
		} catch (InterruptedException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		System.out.println("Finish "+retS);
	}
...
//int dmY=d.height;
	...
	for (int i=0; i<predictionList.size(); i++){
		g2.setColor(useColors[i % 3 ]);
		//else
			//g2.setColor(Color.orange);
		if (i % 3==0){
			g2.draw(new Line2D.Double(showWidth, showHeight+i*dY+dY/3, myChartWidth-showWidth/2, showHeight+i*dY+dY/3)); 
			g2.draw(new Line2D.Double(showWidth, showHeight+i*dY+dY/3, myChartWidth-showWidth/2, showHeight+i*dY+dY/3)); 
		}
		...
		for (int ip=0; ip<maxBall+1; ip++){
			...
			if (i % 3==0){
				g2.draw(new Line2D.Double(showWidth+ip*dX+dX/2, showHeight+i*dY+dY/3, showWidth+ip*dX+dX/2, showHeight+i*dY+dY*3)); 
			}
		}
	}
	/*
...
//System.out.println("I am here");
	...
	String dFF="PAM"+projFileNo+JackpotReader.drawDate;
...
String cleanNumberString(String inString){
	int i0=0;
	int i9=inString.length();
	while (i0 < i9 && (inString.charAt(i0)<'0' || inString.charAt(i0)>'9') ) i0++;
	if (i0==i9 ) return null;
	i9--;
	while (inString.charAt(i9)<'0' || inString.charAt(i9)>'9' ) i9--;
	return inString.substring(i0, ++i9);
}
public void tmpTaskFromfile(String fileName){
...
String aLine=null;
	...
	while (itr.hasNext()){
		String dLine=(String)itr.next();
		int idx=dLine.indexOf("<<");
		String[] dData=dLine.substring(idx+2).split(":");
		
		for (int i=0; i<dData.length; i++){
			String[] numbs=dData[i].split(",");
			int[] aN=new int[40];
			Arrays.fill(aN, 0);
			for (int inm=0; inm<numbs.length-1; inm++){
				String tmp=cleanNumberString(numbs[inm]);
				if (tmp!=null)
				aN[Integer.parseInt(tmp)]++;
			}
		hitSpots.add(Arrays.copyOf(aN, 40));
		}
		/*
		numbs=dData[1].split(",");
		aN=new int[numbs.length];
		Arrays.fill(aN, 0);
		for (int inm=0; inm<aN.length-1; inm++){
			String tmp=cleanNumberString(numbs[inm]);
			if (tmp!=null)
			aN[inm]=Integer.parseInt(tmp);
		}
		hitSpots.add(aN);
		numbs=dData[2].split(",");
		aN=new int[numbs.length];
...
static ArrayBlockingQueue<String> allTestResult=new ArrayBlockingQueue<String>(390);
...
{
	TryPAMFilterNew tJob=new TryPAMFilterNew();
	//tJob.tmpTaskFromfile("pamcheckDropData.txt");
	getSincData();
	...
	Vector<int[]> sampleData=jReaderOrder.readData(5, 39, 390);	
		...
		TryPAMFilterNew aCharter=new TryPAMFilterNew();
		int iCheckStart=iLen1-72;
		int iCheckEnd=iLen1;
		int iFitCount=0;
		int iSuccess=0;
		//Vector<String> allTestResult=new Vector<String>();
		
			int[] iStatistics=new int[6];
			Arrays.fill(iStatistics, 0);
			//iCheckStart=iCheckEnd-2;
			int jobs2Assign=iCheckEnd-iCheckStart+1;
			jobs2Assign /=10;
			Thread[] pool=new Thread[11];
			for (int ia=0; ia<11; ia++){
				TryPAMFilterNew aJob=new TryPAMFilterNew();
				aJob.testBaseData=dateAscend;
				aJob.testStartIndex=iCheckStart+ia*jobs2Assign;
				aJob.testLastIndex=iCheckStart+(ia+1)*jobs2Assign-1;
				if (aJob.testLastIndex >iCheckEnd)
					aJob.testLastIndex=iCheckEnd;
				pool[ia]=new Thread(aJob);
				pool[ia].start();	
			}
			/*
			for (int ij=0; ij<pool.length; ij++){
				try {
					pool[ij].join();
				} catch (InterruptedException e) {
					// TODO Auto-generated catch block
			...
			*/
			...
			while (notDone){				
				try {
					String aLine=null;
					do{
						aLine=allTestResult.poll(10, TimeUnit.SECONDS);
						if (aLine==null) break;
						tempSet.add(aLine);						
					} while (notDone);
					//Thread.sleep(3000);
				...
				notDone=false;
				for (int ith=0; ith<pool.length; ith++){
					if (!pool[ith].isAlive()) continue;
					notDone=true;
					break;
				}
			}
			String aLine=null;
		while (!allTestResult.isEmpty()){
			try {
				aLine=allTestResult.poll(1, TimeUnit.SECONDS);
				if (aLine==null) break;
				tempSet.add(aLine);	
			} catch (InterruptedException e) {
				// TODO Auto-generated catch block
	...
	try {
		testLog=new BufferedWriter(new OutputStreamWriter(new FileOutputStream("checkPAMFitLog.txt")));	
			...
			while (itr.hasNext()){
				...
				for (int i=0; i<dData.length; i++){
					...
					Arrays.fill(aN, 0);
					for (int inm=0; inm<numbs.length-1; inm++){
						String tmp=tJob.cleanNumberString(numbs[inm]);
						if (tmp!=null)
						aN[Integer.parseInt(tmp)]++;
					}
				hitSpots.add(Arrays.copyOf(aN, 40));
			...
			testLog.write(foundTxt+") "+iFound+" out of "+iPred+" "+foundPredictions);
			testLog.newLine();
