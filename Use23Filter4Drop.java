...
public class Use23Filter4Drop extends JFrame implements Runnable 
{
/**
		...
		for (int idx=testStartIndex; idx<=testLastIndex; idx++){
			...
			//System.out.println("Working on "+lineType);
				for (int i=0; i<dd.length; i++){
					...
					int[] predictions=Use23Filter4Drop.tryNewSample(testData);
...
public static void main(String[] args)
{
	...
	Vector<int[]> sampleData=jReaderOrder.readData(5, 39, 390);	
		...
		Use23Filter4Drop aJob=new Use23Filter4Drop();
		int iCheckStart=iLen1-300;
		int iCheckEnd=iLen1;
		int iFitCount=0;
		int iSuccess=0;
		int[] iFits={0, 0, 0, 0, 0, 0};
		int[] ccVal={5, 6 ,7, 8};//4, 5, 6};//2, 3, 4, 5, 6};//,7,8,9};
		int[] freqs={4,6,8};//2, 3, 4, 5, 6};
		...
		try {
			...
			new FileOutputStream("Try23"+lineType+JackpotReader.drawDate+"Log.txt")));	
		...
		while (iCheckStart <= iCheckEnd){
			...
			for (int cc=2; cc<4; cc++){
				...
				checkCount=cc;//Val[cc-1];//cc*minCheckCount;
			...
			//System.out.println("Working on "+lineType);
				...
				for (int i=0; i<dd.length; i++){
					...
					int filterBy=(int)Math.round(sqz);//freqs[i];//yFactor[i];
				...
				//for (int i=1; i<dd.length; i++){
					...
					int[] predictions=//refineFound(dateAscend, "filterN"+(sqz*10)+lineType);//filterOutSample(dateAscend);;//
						Use23Filter4Drop.tryNewSample(testData);//(testData);//dateAscend, "filterA"+checkCount);//filterOutSample(dateAscend);;//
				...
				predics += (""+predictions[p]+"["+dF.format(0)+"], ");
				}
				int iHas=0;
