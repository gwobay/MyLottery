...
public class UseExpanedDataMathOld extends JFrame implements Runnable 
{
/**
	...
	// int selected=((int)sqz % 2);
	 ...
	 for (int s=0; s<5; s++){
		...
		// rdt=-0.2;
		 ...
		 int bestFactor=0;
		 int bestRange=0;
		 while (diff2>0.01 && expandFactor<17)
		 { 
			  sincRange=expandFactor*2;
		 ...
		 if (diff2 > 0.1) diffAt[0]=fineTuneTrianglePrediction(s, oldSample.get(s), 
	 ...
	 for (int s=0; s<5; s++){
		...
		// rdt=-0.2;
		 ...
		 ("!!!!Line "+s+" Best fit with diff="
				 		+dF.format(diff2)+" factor="+bestFactor+" range="+bestRange);
		 aWriter.newLine();
		 if (diff2 > 0.1) diffAt[0]=fineTuneTrianglePrediction(s, oldSample.get(s), bestRatio, 
				 bestFactor, bestRange, diffAt[0], diff2);
		 bestFitPoints[s]=diffAt[0];
	 }
		aWriter.close();
	} catch(IOException e){}
...
public static void main(String[] args)
{
	...
	Vector<int[]> sampleData=jReaderOrder.readData(5, 39, 390);	
		...
		UseExpanedDataMathOld aCharter=new UseExpanedDataMathOld();
		...
		while (iCheckStart <= iCheckEnd){
			...
			//System.out.println("Working on "+lineType);
				...
				//aCharter.tryExpanedSample(4, dateAscend, "filter"+byFactor+"X"+(sqz*10)+lineType);//filterOutSample(dateAscend);;//
					...
					System.out.println("Finish "+iCheckStart+" with check last "+checkCount+" for "+
				                  (System.currentTimeMillis() - tNow)+" milli secs");
