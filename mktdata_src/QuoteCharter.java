...
String dataDir;
public QuoteCharter(Vector<String> myTradeData, String... dir)
{
...
public QuoteCharter(Vector<String> myTradeData, MktQuote thisQuote, String... dir)
{
	lastQuoteTime=new String("00:00");
	myQuotes=myTradeData;
	priceMax=0;
...
void readChartData() 
{
	...
	do 
	{
		...
		if (aQuote.time.indexOf(":") < 0) {i++; continue;}
		mySymbol=aQuote.symbol;
		if (lastQuote.vol==0) {lastQuote=aQuote;i++; samplingStartFrom=aQuote.time; continue;}
		
//System.out.println(aQuote.time+" --> "+lastQuote.time);
		int j=time2Int(aQuote.time) - time2Int(lastQuote.time);	
		if (j==0) {i++; continue;}			
		if (aQuote.vol<0 || aQuote.vol != lastQuote.vol)// < 0 when no vol data
		{
			double pDiff=(aQuote.price - lastQuote.price)/j;
			for (int h=0; h<j; h++)
			{
			aXY=new DrawData(lastQuote.price+(h+1)*pDiff, (aQuote.vol-lastQuote.vol)/j);
...
*/			
		...
		i++;				
	} while(i<myQuotes.size());
	String last1=myQuotes.get(i-1);
...
//time
	...
	g2.drawString("Last="+lastPrice, myChartWidth/5, showHeight);
...
System.out.println("Total data count : "+myQuotes.size());
		QuoteCharter aCharter=new QuoteCharter(myQuotes);
